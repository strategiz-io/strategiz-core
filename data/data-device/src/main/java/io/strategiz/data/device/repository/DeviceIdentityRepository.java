...
/**
 * Repository interface for DeviceIdentity entities Implementation provided by
 * client-firebase module
 *
 * Supports both authenticated devices (stored as subcollection under users) and anonymous
 * devices (stored in root collection)
 */
public interface DeviceIdentityRepository {

	/**
	 * Save authenticated device identity as subcollection under user Stores at:
	...
	/**
	 * Save anonymous device identity at root collection Stores at: /devices/{deviceId}
	 * @param entity The device identity to save
	 * @return The saved device identity
	 */
	...
	/**
	 * Find a device by its visitor ID (fingerprint) across authenticated devices. Used
	 * for device trust verification to identify returning users.
