...
public class StripeConnectStatusResponse {
	...
	// Full constructor
	public StripeConnectStatusResponse(String accountId, String status, boolean detailsSubmitted,
			boolean chargesEnabled, boolean payoutsEnabled, List<String> pendingRequirements,
	...
	public static StripeConnectStatusResponse fromConnectStatus(ConnectAccountStatus status) {
		return new StripeConnectStatusResponse(status.accountId(), status.status(), status.detailsSubmitted(),
				status.chargesEnabled(), status.payoutsEnabled(), status.pendingRequirements(), status.currentlyDue());
	}
