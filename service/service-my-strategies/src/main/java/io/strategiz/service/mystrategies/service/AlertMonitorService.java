...
/**
 * Scheduled service that monitors active alerts and detects trading signals. Uses
 * tier-based scheduling for different subscription levels.
 *
 * Tier-based evaluation frequency: - PRO: Every 1 minute - STARTER: Every 5 minutes -
 * FREE: Every 15 minutes
 *
 * Flow: 1. Fetch active alerts by subscription tier 2. For each alert: a. Check cooldown
 * (skip if in cooldown period) b. Check daily rate limit (skip if exceeded) c. Check
 * signal deduplication (skip if same signal) d. Get current market data e. Execute the
 * strategy code f. Process signals (BUY, SELL) g. Send notifications (for ALERT
 * deployment type) h. Update alert metadata and tracking i. Handle errors with circuit
 * breaker
 */
@Component
...
public class AlertMonitorService extends BaseService {
	...
	@Autowired
	public AlertMonitorService(ReadAlertDeploymentRepository readAlertRepository,
			UpdateAlertDeploymentRepository updateAlertRepository,
			CreateAlertDeploymentHistoryRepository createHistoryRepository,
			StrategyExecutionBusiness strategyExecutionBusiness, MarketDataRepository marketDataRepository,
			@Autowired(required = false) FmpQuoteClient fmpQuoteClient, AlertNotificationService notificationService) {
	...
	/**
	 * PRO tier monitoring - runs every 1 minute (60000ms) Highest frequency for premium
	 * subscribers.
	 */
	@Scheduled(fixedDelay = 60000, initialDelay = 30000) // 1 min interval, 30s initial
															// delay
	...
	/**
	 * STARTER tier monitoring - runs every 5 minutes (300000ms) Mid-tier frequency for
	 * starter subscribers.
	 */
	@Scheduled(fixedDelay = 300000, initialDelay = 60000) // 5 min interval, 1 min initial
															// delay
	...
	/**
	 * FREE tier monitoring - runs every 15 minutes (900000ms) Basic frequency for free
	 * users.
	 */
	@Scheduled(fixedDelay = 900000, initialDelay = 120000) // 15 min interval, 2 min
															// initial delay
	...
	private void monitorAlertsByTier(String tier, int frequencyMinutes) {
		...
		try {
			...
			log.info("Skipped - Cooldown: {} | RateLimit: {} | Dedup: {}", skippedCooldown, skippedRateLimit,
					skippedDedup);
	...
	private enum ProcessOutcome {
		...
		SIGNAL_TRIGGERED, NO_SIGNAL, SKIPPED_COOLDOWN, SKIPPED_DEDUP, ERROR
	...
	private ProcessResult processAlertWithChecks(AlertDeployment alert) {
		...
		for (String symbol : alert.getSymbols()) {
			try {
				...
				ExecutionResult result = strategyExecutionBusiness.executeStrategy(alert.getStrategyId(),
						alert.getProviderId(), alert.getUserId());

				if (!"SUCCESS".equals(result.getStatus())) {
					log.warn("Strategy execution failed for alert {}: {}", alert.getId(), result.getMessage());
					continue;
				}
					...
					for (ExecutionResult.Signal signal : signals) {
						...
						if ("BUY".equals(signal.getType()) || "SELL".equals(signal.getType())) {
							...
							if (isDuplicateSignal(alert, signal.getType(), symbol)) {
								log.debug("Duplicate signal {} for {} on alert {}, skipping", signal.getType(), symbol,
										alert.getId());
							...
							updateAlertRepository.recordSignal(alert.getId(), alert.getUserId(), signal.getType(),
									symbol);

							log.info("Alert {} triggered {} signal for {} at ${}", alert.getId(), signal.getType(),
									symbol, currentPrice);
						}
					}
				}
				...
				log.error("Error processing symbol {} for alert {}: {}", symbol, alert.getId(), e.getMessage());
				return new ProcessResult(ProcessOutcome.ERROR, 0);
			}
		}

		return new ProcessResult(signalCount > 0 ? ProcessOutcome.SIGNAL_TRIGGERED : ProcessOutcome.NO_SIGNAL,
				signalCount);
	}
	...
	/**
	 * Check if alert is in cooldown period. Cooldown is per-symbol and based on
	 * subscription tier.
	 */
	...
	/**
	 * Check if this is a duplicate signal (same signal type for same symbol). This
	 * prevents spamming the same BUY/SELL signal repeatedly.
	 */
	private boolean isDuplicateSignal(AlertDeployment alert, String signalType, String symbol) {
		...
		return signalType.equals(lastSignalType) && symbol.equals(lastSignalSymbol);
	}
	...
	private void resetDailyCountIfNeeded(AlertDeployment alert) {
		...
		if (lastResetDate.isBefore(today)) {
			log.debug("Resetting daily trigger count for alert {} (last reset: {})", alert.getId(), lastResetDate);
			updateAlertRepository.resetDailyTriggerCount(alert.getId(), alert.getUserId());
		}
	}
	...
	private void handleAlertError(AlertDeployment alert, Exception e) {
		try {
			updateAlertRepository.incrementConsecutiveErrors(alert.getId(), alert.getUserId(), e.getMessage());

			// Check if circuit breaker tripped (status changed to ERROR)
			if (alert.shouldTripCircuitBreaker()) {
				log.warn("Circuit breaker tripped for alert {} after {} consecutive errors", alert.getId(),
						alert.getConsecutiveErrors());
			}
		}
	...
	/**
	 * Get latest price from cached market data (populated by batch job). Falls back to
	 * Yahoo Finance if cache miss and fallback is available.
			...
			if (cachedData.isPresent()) {
				...
				if (closePrice != null) {
					...
					if (data.getTimestamp() != null) {
						long ageMinutes = ChronoUnit.MINUTES.between(Instant.ofEpochMilli(data.getTimestamp()),
								Instant.now());
						if (ageMinutes > 30) {
							log.warn("Cached data for {} is {} minutes old (batch may be delayed)", symbol, ageMinutes);
						}
					}

					log.debug("Using cached price for {}: ${} (source: {}, timeframe: {})", symbol, closePrice,
							data.getDataSource(), data.getTimeframe());
	 ...
	 */
	private void createHistoryEntry(AlertDeployment alert, ExecutionResult.Signal signal, String symbol, Double price,
		...
		try {
			...
			log.error("Failed to create history entry for alert {}: {}", alert.getId(), e.getMessage(), e);
		}
	}
	...
	public MonitoringStatus getStatus() {
		return new MonitoringStatus(isRunning.get(), lastRunTime, lastAlertCount, lastSignalCount);
	}
