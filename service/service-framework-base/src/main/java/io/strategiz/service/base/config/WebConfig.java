...
public class WebConfig implements WebMvcConfigurer {
	...
	/**
	 * Creates a RestTemplate bean with appropriate timeout and proxy settings This will
	 * be used for making direct HTTP requests to external APIs
	...
	public RestTemplate restTemplate() {
		try {
			...
			try {
				if (systemProxyHost != null && !systemProxyHost.isEmpty() && systemProxyPort != null
			...
			log.info("RestTemplate initialized with connection timeout: {}ms, read timeout: {}ms", CONNECTION_TIMEOUT,
					READ_TIMEOUT);
			...
			throw new StrategizException(ServiceBaseErrorDetails.RESTTEMPLATE_CREATION_FAILED, "service-framework-base",
					e);
		}
	}

	// Note: The binanceRestTemplate has been removed during consolidation
	// as it represents domain-specific functionality that should not be in the base
	// module.
	...
	public void addInterceptors(InterceptorRegistry registry) {
		...
		registry.addInterceptor(standardHeadersInterceptor)
			...
			.excludePathPatterns("/health", // Health check endpoints
