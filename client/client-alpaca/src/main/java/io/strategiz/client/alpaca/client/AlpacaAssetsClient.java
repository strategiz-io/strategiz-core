 ...
 *
 * Features: - Fetch all assets or filter by status/exchange - Get asset metadata (name,
 * exchange, tradability, etc.) - Filter by asset class (us_equity, crypto)
 */
@Component
public class AlpacaAssetsClient {
	...
	public void init() {
		...
		try {
			...
			log.error(
					"Failed to load Alpaca trading credentials from Vault: {} - AlpacaAssetsClient will be unavailable",
					e.getMessage());
			return;
		}
	...
	public List<AlpacaAsset> getNyseNasdaqStocks() {
		...
		return equities.stream().filter(asset -> {
	...
	public List<AlpacaAsset> getAssets(String status, String assetClass) {
		if (!isAvailable()) {
			throw new StrategizException(AlpacaErrorDetails.CONFIGURATION_ERROR, MODULE_NAME,
					"AlpacaAssetsClient is not available - credentials not configured");
		}
		...
		try {
			...
			ResponseEntity<String> responseEntity = restTemplate.exchange(apiUrl + path.toString(), HttpMethod.GET,
					entity, String.class);
			...
			List<AlpacaAsset> assets = objectMapper.readValue(responseBody, new TypeReference<List<AlpacaAsset>>() {
			});
	...
	public AlpacaAsset getAsset(String symbol) {
		...
		try {
			HttpEntity<String> entity = new HttpEntity<>(createHeaders());
			ResponseEntity<String> responseEntity = restTemplate.exchange(apiUrl + path, HttpMethod.GET, entity,
					String.class);
			...
			throw new StrategizException(AlpacaErrorDetails.INVALID_SYMBOL, MODULE_NAME, e,
