...
/**
 * gRPC client for Python strategy execution service. Handles communication with the
...
public class ExecutionServiceClient {
	...
	@PostConstruct
	public void init() {
		logger.info("Initializing gRPC client for execution service: {}:{}", executionServiceHost,
	 ...
	 */
	public ExecutionResponse executeStrategy(String code, String language, List<MarketDataBar> marketData,
			String userId, String strategyId, int timeoutSeconds) {

		logger.info("Executing strategy via gRPC: strategyId={}, language={}, dataPoints={}", strategyId, language,
				marketData.size());
		...
		try {
			...
			// Call gRPC service
			ExecuteStrategyResponse grpcResponse = blockingStub.withDeadlineAfter(timeoutSeconds + 5, TimeUnit.SECONDS)
				.executeStrategy(request);

			long executionTime = System.currentTimeMillis() - startTime;
			logger.info("Strategy execution completed: success={}, executionTime={}ms", grpcResponse.getSuccess(),
					executionTime);
	...
	public ValidationResponse validateCode(String code, String language) {
		...
		try {
			ValidateCodeRequest request = ValidateCodeRequest.newBuilder().setCode(code).setLanguage(language).build();

			ValidateCodeResponse grpcResponse = blockingStub.withDeadlineAfter(10, TimeUnit.SECONDS)
	...
	public HealthStatus getHealth() {
		try {
			...
			HealthResponse grpcResponse = blockingStub.withDeadlineAfter(5, TimeUnit.SECONDS).getHealth(request);
			...
			return HealthStatus.builder().status("NOT_SERVING").build();
		}
	}
	...
	private ExecutionResponse convertResponse(ExecuteStrategyResponse grpcResponse) {
		...
		if (grpcResponse.getSignalsCount() > 0) {
			List<Signal> signals = grpcResponse.getSignalsList()
				.stream()
		...
		if (grpcResponse.getIndicatorsCount() > 0) {
			List<Indicator> indicators = grpcResponse.getIndicatorsList()
				.stream()
		...
		if (grpcResponse.hasPerformance()) {
			...
			if (grpcPerf.getTradesCount() > 0) {
				List<Trade> trades = grpcPerf.getTradesList()
					.stream()
					.map(t -> Trade.builder()
			...
			if (grpcPerf.getEquityCurveCount() > 0) {
				List<EquityPoint> equityCurve = grpcPerf.getEquityCurveList()
					.stream()
	...
	/**
	 * Execute multiple strategies in batch for live processing. Used by
	 * SymbolSetProcessorJob to evaluate alerts and bots.
	 ...
	 */
	public io.strategiz.client.execution.model.ExecuteListResponse executeList(List<SymbolSetExecutionData> symbolSets,
			String tier, int timeoutSeconds) {

		logger.info("Executing batch via gRPC: tier={}, symbolSets={}, timeout={}s", tier, symbolSets.size(),
				timeoutSeconds);
		...
		try {
			...
			int totalDeployments = symbolSets.stream().mapToInt(s -> s.getDeployments().size()).sum();
			int batchTimeout = Math.max(timeoutSeconds * 2, timeoutSeconds + (totalDeployments / 10));

			// Call gRPC service
			ExecuteListResponse grpcResponse = blockingStub.withDeadlineAfter(batchTimeout, TimeUnit.SECONDS)
				.executeList(request);

			long executionTime = System.currentTimeMillis() - startTime;
			logger.info("Batch execution completed: success={}, total={}, successful={}, failed={}, time={}ms",
					grpcResponse.getSuccess(), grpcResponse.getTotalDeployments(),
					grpcResponse.getSuccessfulDeployments(), grpcResponse.getFailedDeployments(), executionTime);
	...
	private SymbolSetExecution buildSymbolSetExecution(SymbolSetExecutionData data) {
		SymbolSetExecution.Builder builder = SymbolSetExecution.newBuilder().addAllSymbols(data.getSymbols());
	...
	private io.strategiz.client.execution.model.ExecuteListResponse convertListResponse(
		...
		List<io.strategiz.client.execution.model.DeploymentResult> results = grpcResponse.getResultsList()
			.stream()
	...
	private io.strategiz.client.execution.model.DeploymentResult convertDeploymentResult(
		...
		io.strategiz.client.execution.model.DeploymentResult.Builder builder = io.strategiz.client.execution.model.DeploymentResult
			.builder()
	...
	public static class SymbolSetExecutionData {
		...
		public SymbolSetExecutionData(List<String> symbols, Map<String, List<MarketDataBar>> marketData,
	...
	public static class DeploymentExecutionData {
		...
		public DeploymentExecutionData(String deploymentId, String deploymentType, String strategyId,
				String strategyCode, String userId, Map<String, String> parameters) {
