...
/**
 * Client for retrieving OpenAI API costs and usage data. Uses OpenAI's official Costs
 * API.
 *
...
public class OpenAiBillingClient {
	...
	public AiProviderCost getCostReport(LocalDate startDate, LocalDate endDate) {
		...
		try {
			...
			String url = String.format(
					"%s/organization/costs?start_time=%d&end_time=%d&bucket_width=1d&group_by[]=line_item",
	...
	public Map<String, Long> getUsageData(LocalDate startDate, LocalDate endDate) {
		...
		try {
			...
			String url = String.format(
					"%s/organization/usage/completions?start_time=%d&end_time=%d&bucket_width=1d&group_by[]=model",
					API_BASE_URL, startTime, endTime);
	...
	private AiProviderCost parseCostReport(String jsonResponse, LocalDate startDate, LocalDate endDate) {
		try {
				...
				for (JsonNode bucket : data) {
					if (bucket.has("results")) {
						for (JsonNode result : bucket.get("results")) {
							...
							totalCost = totalCost.add(BigDecimal.valueOf(amount / 100.0)); // Convert
																							// cents
																							// to
																							// dollars
			...
			return new AiProviderCost("OpenAI", startDate, endDate, totalCost, costByModel, totalTokens, totalRequests,
					costByService);
