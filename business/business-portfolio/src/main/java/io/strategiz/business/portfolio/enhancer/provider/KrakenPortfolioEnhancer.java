...
/**
 * Kraken-specific portfolio enhancer that transforms raw Kraken data
 * into enhanced portfolio with human-readable names and current prices.
 */
@Component
public class KrakenPortfolioEnhancer {
    ...
    @Autowired
    public KrakenPortfolioEnhancer(SymbolNormalizationBusiness symbolNormalization,
                                   AssetMetadataBusiness assetMetadata,
    ...
    public EnhancedPortfolio enhance(String userId, Map<String, BigDecimal> krakenBalances) {
        ...
        LOGGER.info("Enhanced Kraken portfolio with " + portfolio.getAssets().size() + 
                   " assets, total value: $" + portfolio.getTotalValue());
     ...
     */
    private EnhancedAsset createEnhancedAsset(AssetToEnhance assetInfo, 
        ...
        return new EnhancedAsset.Builder()
            .rawSymbol(assetInfo.originalSymbol)
    ...
    public EnhancedAsset enhanceAsset(String krakenSymbol, BigDecimal quantity) {
        ...
        BigDecimal currentPrice = marketPrice.getCurrentPrice(
            normalized.getStandardSymbol(), 
            normalized.getAssetType()
        );
        
        // Build enhanced asset
        return new EnhancedAsset.Builder()
            .rawSymbol(krakenSymbol)
