 ...
 *
 * Handles email OTP authentication for user login/signin. This service sends OTP codes to
 * verified email addresses and verifies them for authentication.
 *
...
public class EmailOtpAuthenticationService extends BaseService {
	...
	// Made optional to allow service to run even if mail dependency isn't properly
	// resolved
	...
	/**
	 * Generate and send an OTP code to the specified email for authentication Only works
	...
	/**
	 * Generate and send an OTP code to the specified email for authentication. Admin
	 * users can bypass OTP if adminBypassEnabled is true.
	 * @param email Email address to send OTP to (must be verified)
	 * @param purpose Purpose of OTP (e.g., "login", "mfa")
	 * @param userId Optional user ID for admin bypass check
	 * @return true if the OTP was generated and sent (or bypassed for admin), false
	 * otherwise
	 */
	public boolean sendOtp(String email, String purpose, String userId) {
		...
		if (otpCodeRepository != null) {
			...
			OtpCodeEntity entity = new OtpCodeEntity(normalizedEmail, purpose, passwordEncoder.encode(otpCode),
					expiration);
			entity.setMaxAttempts(maxAttempts);
			otpCodeRepository.save(entity, "system");

			log.info("Generated OTP for {}, purpose: {}, expires at: {}", normalizedEmail, purpose, expiration);
		}
		else {
	...
	/**
	 * Verify an OTP code submitted by a user. Admin users can bypass OTP verification if
	 * adminBypassEnabled is true.
	 * @param email Email address
	 * @param purpose Purpose of OTP
	 * @param submittedCode Code submitted by the user
	 * @param userId Optional user ID for admin bypass check
	...
	public boolean verifyOtp(String email, String purpose, String submittedCode, String userId) {
		...
		if (isValid) {
			...
			otpCodeRepository.deleteById(entity.getId()); // Remove after successful
															// verification
		}
		else {
