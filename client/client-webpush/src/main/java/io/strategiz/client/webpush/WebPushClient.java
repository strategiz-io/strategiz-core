 ...
 *
 * <p>
 * Uses VAPID (Voluntary Application Server Identification) keys for authenticating with
 * push services.
 * </p>
 *
 * <p>
 * VAPID keys should be stored in Vault at:
 * </p>
 * <ul>
...
public class WebPushClient {
	...
	public void init() {
		try {
			...
			if (vapidPublicKey == null || vapidPublicKey.isEmpty() || vapidPrivateKey == null
					|| vapidPrivateKey.isEmpty()) {
				log.warn("VAPID keys not configured in Vault at path: {}. Web Push will be unavailable.", secretPath);
				return;
			}
	...
	public WebPushResult sendNotification(PushSubscriptionEntity subscription, String payload) {
		...
		try {
			...
			if (statusCode >= 200 && statusCode < 300) {
				...
				log.warn("Push notification failed for subscription {} with status {}", subscription.getId(),
						statusCode);
			...
			log.error("Error sending push notification to subscription {}: {}", subscription.getId(), e.getMessage());
	...
	/**
	 * Send a push auth notification. Creates a standardized payload for push
	 * authentication.
	 ...
	 * @return WebPushResult with success/failure info
	 */
	public WebPushResult sendPushAuthNotification(PushSubscriptionEntity subscription, String challenge, String purpose,
	 ...
	 */
	private String buildPushAuthPayload(String challenge, String purpose, String ipAddress, String location,
			String userAgent) {
