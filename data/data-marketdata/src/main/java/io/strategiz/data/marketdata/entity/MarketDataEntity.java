 ...
 *
 * Collection structure in Firestore: - Collection: "marketdata" - Document ID:
 * "{symbol}_{timestamp}_{timeframe}" (e.g., "AAPL_2024-01-15T00:00:00Z_1D")
 *
...
public class MarketDataEntity extends BaseEntity implements Serializable {
	...
	public static String createId(String symbol, Long timestampMillis, String timeframe) {
		LocalDateTime dt = LocalDateTime.ofInstant(Instant.ofEpochMilli(timestampMillis), ZoneId.of("UTC"));
		return String.format("%s_%s_%s", symbol.toUpperCase().replace("/", "_").replace("-", "_"), dt.toString(),
				timeframe);
	}
	...
	public void calculateDerivedMetrics(BigDecimal previousClose) {
		if (previousClose != null && previousClose.compareTo(BigDecimal.ZERO) > 0 && close != null) {
			...
			this.changePercent = changeAmount.divide(previousClose, 4, BigDecimal.ROUND_HALF_UP)
	...
	public boolean isValid() {
		return symbol != null && timestamp != null && open != null && high != null && low != null && close != null
	...
	public String toString() {
		return String.format("MarketData[%s %s: O=%.2f H=%.2f L=%.2f C=%.2f V=%.0f]", symbol,
				getTimestampAsLocalDateTime(), open, high, low, close, volume);
	}
