...
/**
 * Webull API Authentication Client Handles API key authentication and request signing for
 * Webull API
 *
 * Webull uses HMAC-SHA1 signature authentication: 1. Construct source parameters from
 * URI, query params, body, and headers 2. Sort parameters alphabetically and concatenate
 * as key-value pairs 3. Generate MD5 hash of request body (if present) 4. Create
 * signature string by concatenating URI, sorted params, and body hash 5. Apply HMAC-SHA1
 * encryption using app secret with appended "&" 6. Encode result in Base64
 */
@Service
public class WebullApiAuthClient {
	 ...
	 */
	public String createSignature(String method, String path, Map<String, String> queryParams, String body,
			String timestamp, String nonce, String appSecret) {
		try {
			if (appSecret == null || appSecret.trim().isEmpty()) {
				log.error("App secret is null or empty");
				throw new StrategizException(ClientErrorDetails.MISSING_CREDENTIALS, "webull-api",
						"App secret is null or empty");
			}
			...
			SecretKeySpec secretKeySpec = new SecretKeySpec(secretWithSuffix.getBytes(StandardCharsets.UTF_8),
					SIGNATURE_ALGORITHM);
	...
	public Mono<Map<String, Object>> getAccountPositions(String appKey, String appSecret, String accountId) {
		...
		return webClient.get()
			.uri(uriBuilder -> uriBuilder.path(path).queryParam("account_id", accountId).build())
			.header("x-app-key", appKey)
			.header("x-signature", signature)
			.header("x-signature-algorithm", SIGNATURE_ALGORITHM)
			.header("x-signature-version", SIGNATURE_VERSION)
	...
	public Mono<Boolean> testConnection(String appKey, String appSecret) {
		return getAccountList(appKey, appSecret).map(response -> {
