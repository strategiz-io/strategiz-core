 ...
 *
 * Records job execution metadata for Market Data backfill and incremental jobs: - Job
 * start/completion timestamps - Success/failure status - Symbols processed and data
 * points stored - Error tracking for failed jobs
 *
 * Provides historical tracking and statistics: - Execution history with pagination -
 * Success rate calculations - Average duration trends - Error frequency analysis
 *
...
public class JobExecutionHistoryBusiness {
	...
	/**
	 * Record the start of a job execution. Creates an execution record with
	 * status=RUNNING. GRACEFUL: If database fails, returns a dummy ID and logs warning
	 * (job continues).
	...
	public String recordJobStart(String jobId, String jobName, String timeframes) {
		...
		try {
			...
			log.debug("Job execution start recorded in {}ms - jobId: {}, jobName: {}, executionId: {}", duration, jobId,
					jobName, executionId);
	...
	/**
	 * Record the completion of a job execution. Updates the execution record with results
	 * and final status. GRACEFUL: If database fails, logs warning and continues.
	 ...
	 */
	public void recordJobCompletion(String executionId, String status, int symbolsProcessed, long dataPointsStored,
			int errorCount, String errorDetails) {
	...
	public Page<JobExecutionEntity> getExecutionHistory(String jobName, int page, int pageSize) {
		if (jobName == null || jobName.trim().isEmpty()) {
			throw new StrategizException(MarketDataErrorDetails.INVALID_INPUT, "business-marketdata",
		...
		Page<JobExecutionEntity> executions = jobExecutionRepository.findByJobNameOrderByStartTimeDesc(jobName,
				pageable);

		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved {} execution records for job {} in {}ms (page {} of {})", executions.getNumberOfElements(),
				jobName, duration, page, executions.getTotalPages());
	...
	public Map<String, Object> getExecutionStats(String jobName, int sinceDays) {
		...
		Map<String, Object> stats = Map.of("successCount", successCount, "failureCount", failureCount, "totalCount",
				totalCount, "successRate", Math.round(successRate * 100.0) / 100.0, // Round
																					// to
																					// 2
																					// decimals
				"avgDurationMs", avgDuration, "periodDays", sinceDays);

		long duration = System.currentTimeMillis() - startTime;
		log.debug("Execution stats for {} calculated in {}ms: {} total, {:.2f}% success", jobName, duration, totalCount,
				successRate);
	...
	public Optional<JobExecutionEntity> getLatestExecution(String jobName) {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Latest execution query for {} completed in {}ms, found: {}", jobName, duration, latest.isPresent());
	...
	public List<JobExecutionEntity> getRecentExecutions(String jobName, int sinceDays) {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved {} recent executions for job {} in {}ms (last {} days)", executions.size(), jobName,
				duration, sinceDays);
	...
	public long countExecutionsByStatus(String jobName, String status, int sinceDays) {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Count for job {} with status {} in {}ms: {} executions", jobName, status, duration, count);

		return count != null ? count : 0L;
	}

	/**
	 * Get paginated execution history across ALL jobs. Used by admin console to view
	 * complete job history. Also cleans up stale RUNNING jobs (older than 12 hours) as a
	...
	/**
	 * Clean up stale RUNNING jobs by marking them as TIMEOUT. Jobs stuck in RUNNING
	 * status for more than 12 hours are considered stale (likely due to HTTP timeout or
	 * server restart).
	 */
	public void cleanupStaleRunningJobs() {
		try {
			...
			if (!staleJobs.isEmpty()) {
				log.info("Found {} stale RUNNING jobs (started before {}), marking as TIMEOUT", staleJobs.size(),
						staleThreshold);
	...
	/**
	 * Get all executions across all jobs since a timestamp. Used for calculating
	 * aggregate statistics in admin console.
	...
	public List<JobExecutionEntity> getAllExecutionsSince(Instant since) {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved {} executions across all jobs since {} in {}ms", executions.size(), since, duration);
