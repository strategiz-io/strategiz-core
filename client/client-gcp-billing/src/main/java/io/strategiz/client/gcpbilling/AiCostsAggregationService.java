...
/**
 * Service for aggregating AI/LLM costs across all providers.
 * Combines costs from Anthropic Claude, OpenAI, and xAI Grok.
 *
...
public class AiCostsAggregationService {
    ...
    public AiCostsAggregationService(
            AnthropicBillingClient anthropicClient,
            OpenAiBillingClient openAiClient,
    ...
    public AiCostsSummary getAiCostsSummary(LocalDate startDate, LocalDate endDate) {
        ...
        long totalTokens = providerDetails.stream()
                .mapToLong(AiProviderCost::tokenUsage)
                .sum();

        long totalRequests = providerDetails.stream()
                .mapToLong(AiProviderCost::requestCount)
                .sum();
        ...
        log.info("Total AI costs: ${} across {} providers ({} total tokens, {} requests)",
                totalCost, providerDetails.size(), totalTokens, totalRequests);
    ...
    /**
     * Get personal Claude development account costs
     * This is a subscription cost, prorated for the period
     *
    ...
    private BigDecimal getPersonalClaudeCosts(LocalDate startDate, LocalDate endDate) {
        ...
        // Prorate the cost
        BigDecimal proratedCost = monthlySubscription
                .multiply(BigDecimal.valueOf(days))
                .divide(BigDecimal.valueOf(daysInMonth), 2, java.math.RoundingMode.HALF_UP);

        log.debug("Personal Claude dev costs: ${} ({} days @ ${}/month)",
                proratedCost, days, monthlySubscription);
