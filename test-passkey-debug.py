...
def main():
    if len(sys.argv) < 2:
        print("Usage: python test-passkey-debug.py <temp_token>")
        sys.exit(1)
    
    temp_token = sys.argv[1]
    user_id = "ed373755-1c85-479a-87d0-b58d010aa6cc"  # From the logs
    
    # Step 1: Begin registration
    print("\n=== Step 1: Begin Registration ===")
    begin_url = f"{BASE_URL}/v1/auth/passkeys/registrations/{user_id}/begin"
    headers = {
        "Authorization": f"Bearer {temp_token}",
        "Content-Type": "application/json"
    }
    
    begin_response = requests.post(begin_url, headers=headers, json={"email": EMAIL})
    ...
    print("\n=== Step 4: Complete Registration ===")
    complete_url = f"{BASE_URL}/v1/auth/passkeys/registrations/{user_id}/complete"
