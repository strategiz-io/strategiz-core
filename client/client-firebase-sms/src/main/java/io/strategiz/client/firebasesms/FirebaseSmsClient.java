 ...
 *
 * This client uses Firebase Phone Authentication to send SMS verification codes. Firebase
 * provides 10,000 free SMS verifications per month.
 */
@Component
public class FirebaseSmsClient {
	...
	public void initialize() {
		...
		try {
			...
			// Remove path prefixes like "src/main/resources/" to get classpath resource
			// name
			String classpathResource = firebaseCredentialsPath.replace("src/main/resources/", "")
				.replace("classpath:", "");
			...
			if (serviceAccountStream == null) {
				log.error("Firebase credentials not found at path: {} or classpath: {}", firebaseCredentialsPath,
						classpathResource);
				return;
			}
	 ...
	 *
	 * Note: Firebase Admin SDK doesn't directly send SMS from server-side. This
	 * implementation stores the OTP and validates it server-side. The actual SMS sending
	 * happens through Firebase client SDK.
	 ...
	 *
	 * This method validates the Firebase ID token sent from the frontend after successful
	...
	public boolean verifyFirebaseIdToken(String idToken, String expectedPhoneNumber) {
		try {
			if (!initialized) {
				log.error(
						"Firebase not initialized. Cannot verify ID token. Check firebase.credentials.path={} and firebase.project.id={}",
			...
			if (!normalizedExpected.equals(normalizedToken)) {
				log.warn("Phone number mismatch. Expected: {}, Got: {}", maskPhoneNumber(expectedPhoneNumber),
