...
/**
 * Console admin controller for per-symbol data freshness monitoring. Provides endpoints
 * for viewing symbol status, searching, and filtering.
 *
 * Endpoints: - GET /v1/console/marketdata/symbols/status - Paginated symbol list with
...
public class ConsoleMarketDataSymbolController extends BaseController {
	 ...
	 */
	@GetMapping("/status")
	@Operation(summary = "Get symbol status list",
			description = "Retrieves paginated symbol status with optional filters")
	public ResponseEntity<SymbolStatusResponse> getSymbolStatus(HttpServletRequest request,
			@RequestParam(required = false) String timeframe, @RequestParam(required = false) String status,
		...
		// Build response
		SymbolStatusResponse response = new SymbolStatusResponse(symbols, page, pageSize, totalCount,
				(int) Math.ceil((double) totalCount / pageSize), summary);
	 ...
	 */
	@GetMapping("/{symbol}")
	@Operation(summary = "Get symbol detail",
			description = "Retrieves detailed status for a single symbol across all timeframes")
	public ResponseEntity<SymbolDetailResponse> getSymbolDetail(HttpServletRequest request,
	...
	public ResponseEntity<List<Map<String, Object>>> getStaleSymbols(HttpServletRequest request,
			@RequestParam(defaultValue = "1D") String timeframe, @RequestParam(defaultValue = "0") int page,
	...
	public ResponseEntity<List<Map<String, Object>>> getFailingSymbols(HttpServletRequest request,
	...
	private SymbolStatusResponse.SymbolStatus convertToSymbolStatus(Map<String, Object> data) {
		...
		// Determine overall status (simplified - would aggregate across timeframes in
		// production)
