...
/**
 * Provider integration entity - root-level collection Collection:
 * provider_integrations/{integrationId}
 *
 * Subcollection: provider_integrations/{integrationId}/provider_data/{dataId}
 *
 * This entity represents a user's connection to an external provider
 * (exchange/brokerage). Provider data (balances, transactions, etc.) is stored as a
 * subcollection under this entity.
 */
...
public class ProviderIntegrationEntity extends BaseEntity {
	...
	@PropertyName("status")
	@JsonProperty("status")
	private String status = "connected"; // Status of the integration - must be lowercase
											// for consistency
	...
	private String environment; // For providers with multiple environments (e.g.,
								// "paper", "live")
