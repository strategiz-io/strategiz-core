...
/**
 * Base controller for authentication-related REST controllers in the Strategiz
 * application. Extends BaseController with authentication-specific functionality.
 *
 * Authentication controllers should: - Extend this class instead of BaseController
 * directly - Use provided user extraction and role validation methods - Override role
 * validation logic as needed for specific auth flows - Throw StrategizException for
...
public abstract class BaseAuthenticationController extends BaseController {
	...
	/**
	 * Validate that the user has the required roles Default implementation - override in
	 * subclasses with actual role checking logic
	...
	protected void validateUserRoles(String userId, String... requiredRoles) {
		...
		// "User " + userId + " lacks required roles: " + String.join(",",
		// requiredRoles));
		// }
	}
	...
	protected void validateAuthenticated(Principal principal) {
		if (principal == null) {
			throw new StrategizException(ErrorCode.AUTHENTICATION_ERROR, "Authentication required for this operation");
		}
	}

	/**
	 * Validate that the user has access to a specific resource Default implementation -
	 * override in subclasses with actual access control logic
	...
	protected void validateResourceAccess(String userId, String resourceType, String resourceId) {
		log.debug("Resource access validation for user: {}, resource: {}:{}", userId, resourceType, resourceId);
		...
		// throw new StrategizException(ErrorCode.AUTHORIZATION_ERROR,
		// "User " + userId + " does not have access to " + resourceType + ":" +
		// resourceId);
		// }
	}
	...
	/**
	 * Check if user has admin privileges Default implementation - override in subclasses
	 * with actual admin checking
	...
	protected void requireAdmin(String userId) {
		if (!isAdmin(userId)) {
			throw new StrategizException(ErrorCode.AUTHORIZATION_ERROR, "Admin privileges required for this operation");
		}
	}
