...
/**
 * Business logic for Webull provider integration Implements dual storage: credentials in
...
public class WebullProviderBusiness extends BaseApiKeyProviderHandler {
	...
	@Autowired
	public WebullProviderBusiness(WebullApiAuthClient webullApiAuthClient,
			WebullCredentialManager webullCredentialManager, WebullDataInitializer dataInitializer) {
	...
	public ProviderIntegrationResult createIntegration(CreateProviderIntegrationRequest request, String userId) {
		...
		try {
			...
			if (providerIntegrationRepository != null) {
				try {
					ProviderIntegrationEntity entity = new ProviderIntegrationEntity(WebullConstants.PROVIDER_ID,
							"api_key", userId);
			...
			throw new StrategizException(WebullProviderErrorDetails.CONNECTION_FAILED, MODULE_NAME, userId,
					e.getMessage());
		}
	}
	...
	public ProviderDataEntity syncProviderData(String userId) {
		...
		try {
			Map<String, String> credentials = getStoredCredentials(userId);
			if (credentials == null || credentials.isEmpty()) {
				throw new StrategizException(WebullProviderErrorDetails.INVALID_CREDENTIALS, MODULE_NAME, userId,
			...
			if (accountId == null || accountId.isEmpty()) {
				throw new StrategizException(WebullProviderErrorDetails.ACCOUNT_ID_REQUIRED, MODULE_NAME, userId,
						"Account ID not found in stored credentials");
			}

			ProviderDataEntity syncedData = dataInitializer.initializeAndStoreData(userId, appKey, appSecret,
					accountId);

			log.info("Successfully synced Webull data for user: {}, total value: {}, holdings: {}", userId,
			...
			throw new StrategizException(WebullProviderErrorDetails.DATA_INITIALIZATION_FAILED, MODULE_NAME, userId,
					e.getMessage());
		}
	}
