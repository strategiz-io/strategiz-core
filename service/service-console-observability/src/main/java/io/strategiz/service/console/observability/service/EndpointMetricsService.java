 ...
 *
 * <p>
 * Provides real-time availability and performance metrics for ALL REST endpoints by
 * querying Spring Boot Actuator's automatic HTTP instrumentation.
 *
 * <p>
...
public class EndpointMetricsService {
	...
	/**
	 * Get metrics for all discovered REST endpoints. Discovers ALL registered endpoints
	 * from Spring, then merges with actual metrics data. This ensures endpoints with zero
	 * traffic are also visible.
	...
	public List<EndpointMetrics> getAllEndpointMetrics() {
		...
		for (Map.Entry<RequestMappingInfo, HandlerMethod> entry : handlerMethods.entrySet()) {
			...
			Set<String> methods = mappingInfo.getMethodsCondition()
				.getMethods()
				.stream()
				.map(Enum::name)
		...
		for (io.micrometer.core.instrument.Timer timer : timers) {
			...
			// Update latency metrics (use the timer with most data - typically the
			// aggregate one)
			if (count > 0) {
		...
		// Sort: endpoints with traffic first (by request count desc), then zero-traffic
		// endpoints (alphabetically)
		return metricsMap.values()
			.stream()
			.sorted(Comparator.comparing(EndpointMetrics::getTotalRequests)
				.reversed()
	...
	public List<EndpointMetrics> getUnhealthyEndpoints() {
		return getAllEndpointMetrics().stream()
			.filter(em -> em.getTotalRequests() > 0) // Only consider endpoints with
														// traffic
