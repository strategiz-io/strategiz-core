 ...
 *
 * Handles phone number registration and verification for user accounts. This service
 * manages the setup and verification of SMS OTP as an authentication method.
 *
...
public class SmsOtpRegistrationService extends BaseService {
	...
	/**
	 * Register a phone number for a user account Sends verification OTP to confirm phone
	 * number ownership
	 */
	public boolean registerPhoneNumber(String userId, String phoneNumber, String ipAddress, String countryCode) {
		...
		if (!featureFlagService.isSmsOtpSignupEnabled()) {
			...
			throw new StrategizException(AuthErrors.AUTH_METHOD_DISABLED, "SMS OTP signup is currently disabled");
		}
		...
		if (existing.isPresent()) {
			...
			Boolean isVerified = (Boolean) existingMethod
		...
		smsOtpMethod.putMetadata(AuthenticationMethodMetadata.SmsOtpMetadata.LAST_OTP_SENT_AT,
				Instant.now().toString());
		smsOtpMethod.putMetadata(AuthenticationMethodMetadata.SmsOtpMetadata.DAILY_SMS_COUNT, 1);
		smsOtpMethod.putMetadata(AuthenticationMethodMetadata.SmsOtpMetadata.DAILY_COUNT_RESET_AT,
				Instant.now().toString());
		...
		if (otpSent) {
			...
			throw new StrategizException(AuthErrors.SMS_SEND_FAILED, "Failed to send verification OTP");
		}
	}

	/**
	 * Verify phone number ownership with OTP code and return tokens Completes the phone
	 * number registration process with authentication
	...
	public java.util.Map<String, Object> verifySmsOtpWithTokenUpdate(String userId, String accessToken,
		...
		java.util.Map<String, Object> authResult = sessionAuthBusiness.addAuthenticationMethod(accessToken, "sms_otp", 2 // ACR
																															// level
																															// 2
																															// for
																															// MFA
	...
	/**
	 * Verify phone number ownership with OTP code Completes the phone number registration
	 * process
	...
	public boolean verifyPhoneNumberRegistration(String userId, String phoneNumber, String otpCode) {
		...
		if (methodOpt.isEmpty()) {
			throw new StrategizException(AuthErrors.OTP_NOT_FOUND, "No phone number registration found for this user");
		}
	...
	public boolean resendVerificationOtp(String userId, String phoneNumber, String ipAddress, String countryCode) {
		...
		if (methodOpt.isEmpty()) {
			throw new StrategizException(AuthErrors.OTP_NOT_FOUND, "No phone number registration found");
		}
		...
		String resetAtStr = method
			.getMetadataAsString(AuthenticationMethodMetadata.SmsOtpMetadata.DAILY_COUNT_RESET_AT);
		Long resetAt = resetAtStr != null ? Long.parseLong(resetAtStr) : null;
		int maxDaily = smsOtpConfig.isDevMockSmsEnabled() ? 100 : 10;

		// Check if we need to reset the daily count
		if (resetAt != null
				&& Instant.now().isAfter(Instant.ofEpochMilli(resetAt).plus(java.time.Duration.ofDays(1)))) {
			dailyCount = 0;
			method.putMetadata(AuthenticationMethodMetadata.SmsOtpMetadata.DAILY_SMS_COUNT, 0);
			method.putMetadata(AuthenticationMethodMetadata.SmsOtpMetadata.DAILY_COUNT_RESET_AT,
					Instant.now().toString());
		}
		...
		if (otpSent) {
			...
			int currentCount = (Integer) method
				.getMetadata(AuthenticationMethodMetadata.SmsOtpMetadata.DAILY_SMS_COUNT);
			...
			throw new StrategizException(AuthErrors.SMS_SEND_FAILED, "Failed to resend verification OTP");
		}
	}
	...
	public List<AuthenticationMethodEntity> getVerifiedUserSmsOtpMethods(String userId) {
		return authMethodRepository.findByUserIdAndTypeAndIsEnabled(userId, AuthenticationMethodType.SMS_OTP, true)
			.stream()
			.filter(method -> Boolean.TRUE
				.equals(method.getMetadata(AuthenticationMethodMetadata.SmsOtpMetadata.IS_VERIFIED)))
			.toList();
	}
	 ...
	 */
	public Map<String, Object> verifyFirebaseTokenAndComplete(String firebaseIdToken, String userId, String phoneNumber,
		...
		try {
			...
			if (!tokenValid) {
				...
				throw new StrategizException(AuthErrors.INVALID_CREDENTIALS, "Invalid Firebase authentication token");
			}
			...
			throw new StrategizException(AuthErrors.SMS_SEND_FAILED, "Phone verification failed: " + e.getMessage());
		}
	}
	...
	private Map<String, Object> completePhoneRegistration(String userId, String phoneNumber) {
		...
		SessionAuthBusiness.TokenPair tokens = sessionAuthBusiness.createAuthenticationTokenPair(userId,
				List.of("SMS_OTP"), "sms", null, // deviceId
				null // ipAddress
		);
	...
	/**
	 * Complete SMS authentication after Firebase verification Finds the user by phone
	 * number and creates a session
	 */
	...
	/**
	 * Find user ID by phone number from authentication methods. Searches all SMS_OTP
	 * authentication methods using collection group query.
	 */
		...
		for (AuthenticationMethodEntity method : allSmsMethods) {
			...
			if (normalizedSearchPhone.equals(normalizedMethodPhone)) {
				Boolean isVerified = (Boolean) method
					.getMetadata(AuthenticationMethodMetadata.SmsOtpMetadata.IS_VERIFIED);
				if (Boolean.TRUE.equals(isVerified)) {
					// userId is stored in metadata by the repository during collection
					// group query
	...
	private Optional<AuthenticationMethodEntity> findSmsOtpByPhoneNumber(String userId, String phoneNumber) {
		...
		return methods.stream()
			.filter(method -> phoneNumber
				.equals(method.getMetadataAsString(AuthenticationMethodMetadata.SmsOtpMetadata.PHONE_NUMBER)))
			.findFirst();
	}
