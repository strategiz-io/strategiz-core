...
/**
 * Base repository for WatchlistItem entities using Firestore Stores watchlist items as
 * subcollection under users/{userId}/watchlist
 */
@Repository
public class WatchlistBaseRepository extends BaseRepository<WatchlistItemEntity> {
	...
	/**
	 * Get user-scoped collection for watchlist items Returns users/{userId}/watchlist
	 */
	@Override
	...
	public WatchlistItemEntity save(WatchlistItemEntity entity, String userId) {
		try {
			...
			log.info("WATCHLIST_SAVE_DEBUG: userId={}, entityId={}, symbol={}", userId, entity.getId(),
					entity.getSymbol());
			...
			if (userId != null && entity.getId() != null && userId.equals(entity.getId())) {
				log.error("WATCHLIST_SAVE_BUG: userId equals entity.getId()! This would create wrong path. userId={}",
						userId);
				throw new DataRepositoryException(DataRepositoryErrorDetails.INVALID_ARGUMENT, "WatchlistItemEntity",
						"userId cannot equal entity.getId() - potential bug detected");
			}
			...
			if (isCreate) {
				...
				if (!entity._hasAudit()) {
					throw new DataRepositoryException(DataRepositoryErrorDetails.AUDIT_FIELDS_MISSING,
							"WatchlistItemEntity");
				}
	...
	public boolean existsBySymbol(String symbol, String userId) {
		try {
			Query query = getUserScopedCollection(userId).whereEqualTo("symbol", symbol.toUpperCase());
	...
	public boolean delete(String id, String userId) {
		try {
			...
			throw new DataRepositoryException(DataRepositoryErrorDetails.ENTITY_DELETE_FAILED, e, "WatchlistItemEntity",
					id);
		}
	}
	...
	public List<WatchlistItemEntity> findByType(String type, String userId) {
		try {
			Query query = getUserScopedCollection(userId).whereEqualTo("type", type.toUpperCase());
