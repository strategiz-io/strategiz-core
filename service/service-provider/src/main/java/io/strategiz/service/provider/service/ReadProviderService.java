...
/**
 * Service for reading provider connections and data. Handles business logic for
 * retrieving provider status, balances, and transaction data.
 *
	...
	public ProvidersListResponse getProvidersList(String userId) {
		...
		if (portfolioProviderRepository != null) {
			try {
				...
				log.info("Found {} active providers for user: {}", providers != null ? providers.size() : "null",
						userId);
				...
				for (PortfolioProviderEntity provider : providers) {
					...
					// Set proper provider name based on ID (or use stored name if
					// available)
					String providerName = provider.getProviderName() != null ? provider.getProviderName()
					...
					log.info("Added provider {} (status={}, providerName={}) to list for user: {}",
							provider.getProviderId(), providerResponse.getStatus(), providerResponse.getProviderName(),
							userId);
				}
			}
			catch (Exception e) {
	...
	public ReadProviderResponse getProviders(String userId) {
		...
		try {
			...
			if ("test-user-123".equals(userId)) {
				...
				response.setAccountInfo(Map.of("provider", "Kraken", "type", "Exchange", "features",
						List.of("trading", "portfolio", "market_data")));
