...
public class DeviceIdentity extends BaseEntity {
    ...
    public int getTrustScore() {
        ...
        if (Boolean.TRUE.equals(incognitoMode)) score -= 10;
        if (Boolean.TRUE.equals(vpnDetected)) score -= 20;
        if (Boolean.TRUE.equals(proxyDetected)) score -= 20;
        if (Boolean.TRUE.equals(botDetected)) score -= 40;
        if (Boolean.TRUE.equals(tamperingDetected)) score -= 30;
        if (Boolean.TRUE.equals(hasLiedLanguages)) score -= 15;
        if (Boolean.TRUE.equals(hasLiedResolution)) score -= 15;
        if (Boolean.TRUE.equals(hasLiedOs)) score -= 15;
        if (Boolean.TRUE.equals(hasLiedBrowser)) score -= 15;
        
        // Add points for positive indicators
        if (fingerprintConfidence != null && fingerprintConfidence > 0.95) score += 10;
        if (publicKey != null && !publicKey.isEmpty()) score += 5;
