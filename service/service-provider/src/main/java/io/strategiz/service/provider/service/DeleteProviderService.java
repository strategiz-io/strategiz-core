...
/**
 * Service for deleting provider connections and data. Handles business logic for
...
public class DeleteProviderService extends ProviderBaseService {
	...
	public DeleteProviderResponse deleteProvider(DeleteProviderRequest request) {
		providerLog.info("Deleting provider: {} for user: {}", request.getProviderId(), request.getUserId());
		...
		try {
			...
			providerLog.info("Deleted provider: {} for user: {}", request.getProviderId(), request.getUserId());

		}
		catch (Exception e) {
			providerLog.error("Error deleting provider: {} for user: {}", request.getProviderId(), request.getUserId(),
					e);
	...
	private void processProviderDeletion(DeleteProviderRequest request, DeleteProviderResponse response) {
		...
		try {
			// 1. Soft delete provider from Firestore (marks as deleted with
			// isActive=false)
			// Path: users/{userId}/portfolio/data/providers/{providerId}
			providerLog.info("Soft deleting provider from Firestore for user: {}, provider: {}", request.getUserId(),
					request.getProviderId());
			boolean providerDeleted = portfolioProviderRepository.delete(request.getUserId(), request.getProviderId());
			data.put("providerDeleted", providerDeleted);

			// 2. Delete holdings subcollection (hard delete - removes cached portfolio
			// data)
			...
			boolean holdingsDeleted = providerHoldingsRepository.delete(request.getUserId(), request.getProviderId());
			data.put("holdingsDeleted", holdingsDeleted);

			// 3. Delete OAuth tokens from Vault
			String vaultPath = "secret/strategiz/users/" + request.getUserId() + "/providers/"
					+ request.getProviderId();
			...
			try {
				...
				providerLog.debug("Invalidated AI insights cache for user {} after provider deletion",
						request.getUserId());
			}
			catch (Exception e) {
				providerLog.warn("Failed to invalidate insights cache for user {}: {}", request.getUserId(),
						e.getMessage());
			...
			throw new StrategizException(ServiceProviderErrorDetails.PROVIDER_DELETE_FAILED, "service-provider", e,
					request.getProviderId());
		}
	}
	...
	private void processCleanupOptions(DeleteProviderRequest request, Map<String, Object> data) {
		...
		if (Boolean.TRUE.equals(cleanupOptions.get("revokeTokens"))) {
			...
			providerLog.info("Revoked tokens for provider: {} user: {}", request.getProviderId(), request.getUserId());
		}
		...
		if (Boolean.TRUE.equals(cleanupOptions.get("clearCachedData"))) {
			...
			providerLog.info("Cleared cached data for provider: {} user: {}", request.getProviderId(),
					request.getUserId());
		}
		...
		if (Boolean.TRUE.equals(cleanupOptions.get("removeWebhooks"))) {
			...
			providerLog.info("Removed webhooks for provider: {} user: {}", request.getProviderId(),
					request.getUserId());
		}
