...
/**
 * Represents the allocation of portfolio assets within a specific category. Used for
 * aggregated portfolio views and visualization (e.g., pie charts).
 */
public class CategoryAllocation {
	...
	public static class Builder {
		...
		public Builder calculatePercentage(BigDecimal totalPortfolioValue) {
			if (totalPortfolioValue != null && totalPortfolioValue.compareTo(BigDecimal.ZERO) > 0) {
				this.percentage = this.value.divide(totalPortfolioValue, 4, RoundingMode.HALF_UP)
		...
		public Builder calculateProfitLossPercent() {
			if (this.costBasis != null && this.costBasis.compareTo(BigDecimal.ZERO) > 0) {
				this.profitLossPercent = this.profitLoss.divide(this.costBasis, 4, RoundingMode.HALF_UP)
					.multiply(BigDecimal.valueOf(100))
					.setScale(2, RoundingMode.HALF_UP);
			}
			return this;
		...
		public CategoryAllocation build() {
			if (category == null) {
				throw new StrategizException(PortfolioErrorDetails.INVALID_ARGUMENT, "CategoryAllocation",
	...
	public String toString() {
		return String.format("%s: $%.2f (%.2f%%) - %d assets", category.getDisplayName(), value, percentage,
				assetCount);
	}
