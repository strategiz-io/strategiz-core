 ...
 *
 * These tests make actual HTTP calls to the production API to verify:
 * 1. SMS OTP send functionality (backend-only flow)
 * 2. SMS OTP verification
 * 3. Firebase phone authentication token verification
 * 4. Error handling (rate limiting, invalid codes, expired sessions)
 * 5. Phone number lookup for sign-in
 *
 * Environment Variables Required:
 *   - STRATEGIZ_API_URL: Override API URL (defaults to production)
 *   - STRATEGIZ_TEST_PHONE: Test phone number for SMS tests (optional, uses mock if not set)
 *   - STRATEGIZ_PROD_TOKEN: Authentication token for protected endpoints
 *   - STRATEGIZ_CLIENT_ID + STRATEGIZ_CLIENT_SECRET: Service account credentials
 *
 * Run:
 *   # With service account
 *   STRATEGIZ_CLIENT_ID=sa_xxx STRATEGIZ_CLIENT_SECRET=xxx mvn test -Dtest=SmsOtpProductionE2ETest -pl service/service-auth
 *
 *   # With manual token
 *   STRATEGIZ_PROD_TOKEN=xxx mvn test -Dtest=SmsOtpProductionE2ETest -pl service/service-auth
 *
...
public class SmsOtpProductionE2ETest {
    ...
    void testSendSmsOtp_ValidPhone() throws Exception {
        ...
        // Could be 200 (success) or 400/404 (phone not registered) - both are valid API responses
        if (response.statusCode() == 200) {
            JsonNode json = objectMapper.readTree(response.body());
    ...
    void testVerifySmsOtp_InvalidCode() throws Exception {
        ...
        assertTrue(response.statusCode() >= 400 || !objectMapper.readTree(response.body()).path("success").asBoolean(true),
    ...
    void testVerifySmsOtp_InvalidSessionId() throws Exception {
        ...
        HttpResponse<String> response = executePut("/v1/auth/sms-otp/authentications/invalid-session-id", requestBody, false);
    ...
    void testRateLimiting() throws Exception {
        ...
        String testPhone = "+15559999999"; // Use a different number to avoid affecting other tests
        String requestBody = String.format("""
                {
                    "phoneNumber": "%s",
                    "countryCode": "US"
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (response.statusCode() == 429 ||
                (response.body() != null && response.body().contains("rate"))) {
                rateLimitHits++;
                break;
            }
    ...
    void testSmsOtpEndpointsAvailable() throws Exception {
        ...
        // Should return 200 (success) or 404 (not found for specific userId) - both indicate endpoint exists
    ...
    void testApiResponseFormat() throws Exception {
        ...
        assertDoesNotThrow(() -> objectMapper.readTree(response.body()),
