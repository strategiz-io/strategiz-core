 ...
 *
 * Owner Subscription Model: - Strategies have both creatorId (original author) and
 * ownerId (current rights holder) - creatorId: Immutable, for attribution and provenance
 * tracking - ownerId: Mutable, receives subscription revenue, can transfer ownership -
 * When ownership transfers, ownerId changes but creatorId remains the same
 *
 * Strategy responses include both fields so frontend can display: - "Created
 * by @username" (creatorId) - "Owned by @username" (ownerId, only shown if different from
 * creator)
 */
@RestController
...
public class StrategyMarketplaceController extends BaseController {
	 ...
	 *
	 * Each strategy includes: - creatorId: Original author (for attribution) - ownerId:
	 * Current owner (who receives subscription revenue) - These may differ if ownership
	...
	public ResponseEntity<Object> listPublicStrategies(@RequestParam(required = false) String category,
	 ...
	 *
	 * Query params: - include: Comma-separated list (trades, equityCurve, comments)
	 *
	 * Example: GET /v1/marketplace/strategies/{id}?include=trades,equityCurve
	 * @param id Strategy ID
	 * @param include Optional include parameters
	 * @param user Authenticated user (optional - allows public access to public
	 * strategies)
	...
	public ResponseEntity<Object> getStrategy(@PathVariable String id, @RequestParam(required = false) String include,
	...
	/**
	 * Get strategy share preview for Open Graph metadata. Used by social media platforms
	 * when sharing strategy links.
	 * @param id Strategy ID
	...
	public ResponseEntity<Object> getSharePreview(@PathVariable String id,
			@AuthUser(required = false) AuthenticatedUser user) {
		try {
			String userId = user != null ? user.getUserId() : null;
			...
			if (e.getErrorDetails() == MarketplaceErrorDetails.STRATEGY_NOT_FOUND) {
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("error", "Strategy not found"));
			}
	 ...
	 *
	 * Sets both creatorId and ownerId to the creating user: - creatorId: Set once at
	 * creation, never changes (attribution) - ownerId: Initially same as creator, can
	 ...
	 *
	 * Only the current OWNER can update the strategy (not the original creator). If
	 * ownership has been transferred, the original creator loses edit access.
	 */
	@PutMapping("/{id}")
	@CrossOrigin(origins = { "http://localhost:3000", "http://localhost:3001", "https://strategiz.io" },
			allowedHeaders = "*")
	public ResponseEntity<Object> updateStrategy(@PathVariable String id, @RequestBody Map<String, Object> strategyData,
			@AuthUser AuthenticatedUser user) {
		try {
			String userId = user.getUserId();
	...
	@CrossOrigin(origins = { "http://localhost:3000", "http://localhost:3001", "https://strategiz.io" },
			allowedHeaders = "*")
	public ResponseEntity<Object> deleteStrategy(@PathVariable String id, @AuthUser AuthenticatedUser user) {
		try {
			String userId = user.getUserId();
	...
	@CrossOrigin(origins = { "http://localhost:3000", "http://localhost:3001", "https://strategiz.io" },
			allowedHeaders = "*")
	public ResponseEntity<Object> purchaseStrategy(@PathVariable String id, @AuthUser AuthenticatedUser user) {
		try {
			String userId = user.getUserId();
	...
	@CrossOrigin(origins = { "http://localhost:3000", "http://localhost:3001", "https://strategiz.io" },
			allowedHeaders = "*")
	public ResponseEntity<Object> createCheckoutSession(@PathVariable String id, @AuthUser AuthenticatedUser user) {
		try {
			String userId = user.getUserId();
	...
	public ResponseEntity<Object> applyStrategy(@PathVariable String id,
			@RequestBody Map<String, Object> applicationData, @AuthUser AuthenticatedUser user) {
		try {
			String userId = user.getUserId();
