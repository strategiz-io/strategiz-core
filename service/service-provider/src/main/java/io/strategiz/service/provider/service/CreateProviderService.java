...
/**
 * Service that orchestrates provider creation by delegating to appropriate business
 * modules. This is a thin orchestration layer following the delegation pattern:
 *
 * Flow for API Key Providers (Kraken, Binance US): 1. Service receives request and
 * validates 2. Service delegates to business module's createIntegration() - which stores
 * credentials in Vault 3. Service delegates to business module's testConnection() - which
 * uses stored credentials 4. Service returns response or handles errors
 *
 * Flow for OAuth Providers (Coinbase, Alpaca, Schwab): 1. Service receives request and
 * validates 2. Service delegates to business module's createIntegration() - which
 * generates OAuth URL 3. Service returns OAuth URL to frontend (no connection test until
 * OAuth callback)
 *
 * Note: This service does NOT store credentials - it only orchestrates. All actual
 * storage happens in the business modules.
 *
...
public class CreateProviderService extends BaseService {
	...
	public CreateProviderService(CoinbaseProviderBusiness coinbaseProviderBusiness,
			KrakenProviderBusiness krakenProviderBusiness, BinanceUSProviderBusiness binanceUSProviderBusiness,
			AlpacaProviderBusiness alpacaProviderBusiness, SchwabProviderBusiness schwabProviderBusiness,
			RobinhoodProviderBusiness robinhoodProviderBusiness, WebullProviderBusiness webullProviderBusiness,
			EtradeProviderBusiness etradeProviderBusiness, ProfileService profileService,
	...
	public CreateProviderResponse createProvider(CreateProviderRequest request) {
		log.info("Delegating provider creation for user: {}, provider: {}", request.getUserId(),
		...
		try {
			...
			// OAuth providers need user authorization first before we can test the
			// connection
			if ("oauth".equalsIgnoreCase(request.getConnectionType())) {
				...
				// Skip connection test for OAuth - go directly to creating the
				// integration
				...
				ProviderIntegrationResult storeResult = handler.createIntegration(integrationRequest,
						request.getUserId());

				if (!storeResult.isSuccess()) {
					throw new StrategizException(ServiceProviderErrorDetails.PROVIDER_CONNECTION_FAILED,
							"service-provider", request.getProviderId(), "Failed to store credentials");
				}

				// Step 2: Delegate connection testing to provider (uses stored
			...
			// For API key providers, set trading mode to "live" when successfully
			// connected
			if (!"oauth".equalsIgnoreCase(request.getConnectionType()) && result.isSuccess()) {
				try {
					profileService.updateDemoMode(request.getUserId(), false); // false =
																				// live
																				// mode
				...
				try {
					...
					log.debug("Invalidated AI insights cache for user {} after connecting {}", request.getUserId(),
							request.getProviderId());
				}
				catch (Exception e) {
					log.warn("Failed to invalidate insights cache for user {}: {}", request.getUserId(),
							e.getMessage());
			...
			log.error("Error creating provider for user: {}, provider: {}", request.getUserId(),
					request.getProviderId(), e);
	 ...
	 */
	private CreateProviderResponse convertToServiceResponse(CreateProviderRequest request,
		...
		Map<String, Object> connectionData = result.getMetadata() != null ? new HashMap<>(result.getMetadata())
				: new HashMap<>();
		...
		if (result.getMetadata() != null && result.getMetadata().containsKey("oauthUrl")) {
			...
			log.info("OAuth flow initiated for provider: {}, URL: {}", request.getProviderId(),
	...
	public Map<String, Object> getConnectedProviders(String userId) {
		...
		try {
			...
			List<Map<String, Object>> providers = integrations.stream().map(entity -> {
				Map<String, Object> provider = new HashMap<>();
				provider.put("providerId", entity.getProviderId());
				provider.put("providerName", entity.getProviderId()); // Use providerId as
																		// name for now
				provider.put("status",
						ProviderStatus.CONNECTED.getValue().equals(entity.getStatus()) ? "active" : "inactive");
				provider.put("connectedAt", entity.getCreatedDate());
				provider.put("lastSyncedAt", entity.getModifiedDate()); // Use modified
																		// date as last
																		// sync
				return provider;
