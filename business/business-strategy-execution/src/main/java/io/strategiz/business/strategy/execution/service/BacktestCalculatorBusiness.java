...
/**
 * Business layer service for calculating backtest performance metrics from strategy
...
public class BacktestCalculatorBusiness {
	...
	public BacktestPerformance calculatePerformance(List<SignalData> signals, List<Map<String, Object>> marketData,
		...
		List<BacktestTrade> trades = pairSignalsToTrades(buySignals, sellSignals, priceMap, latestPrice,
				latestTimestamp);
	...
	private Map<String, Double> buildPriceMap(List<Map<String, Object>> marketData) {
		...
		return marketData.stream()
			...
			.collect(Collectors.toMap(bar -> formatDateKey(bar.get("timestamp")),
					bar -> ((Number) bar.get("close")).doubleValue(), (existing, replacement) -> replacement // Handle
																												// duplicates
			));
	}
	...
	/**
	 * Pair buy signals with subsequent sell signals to form trades. Any remaining buy
	 * signals without a matching sell are treated as unrealized positions using the
	 * latest market price.
	 */
	private List<BacktestTrade> pairSignalsToTrades(List<SignalData> buySignals, List<SignalData> sellSignals,
		...
		while (buyIndex < buySignals.size() && sellIndex < sellSignals.size()) {
			...
			while (sellIndex < sellSignals.size()
					&& parseTimestamp(sellSignals.get(sellIndex).getTimestamp()) <= buyTime) {
				sellIndex++;
			}
			...
			double buyPrice = buySignal.getPrice() > 0 ? buySignal.getPrice()
					: getPriceFromMap(priceMap, buySignal.getTimestamp());
			double sellPrice = sellSignal.getPrice() > 0 ? sellSignal.getPrice()
					: getPriceFromMap(priceMap, sellSignal.getTimestamp());
		...
		if (latestPrice > 0) {
			while (buyIndex < buySignals.size()) {
				...
				if (buyPrice > 0) {
					...
					logger.debug("Added unrealized trade: bought at {} on {}, current value {} ({}% gain)", buyPrice,
							buySignal.getTimestamp(), latestPrice, String.format("%.2f", pnlPercent));
				}
	...
	/**
	 * Parse timestamp string to LocalDateTime. Handles ISO format, Unix seconds, and Unix
	 * milliseconds.
	 */
	...
	/**
	 * Format period as human-readable string. - >= 1 year: "X.XY" (e.g., "5.3Y") - >= 2
	 * months: "XM" (e.g., "6M") - >= 2 weeks: "XW" (e.g., "3W") - < 2 weeks: "XD" (e.g.,
	 * "10D") - > 5 years: "Since MMM YYYY"
	...
	/**
	 * Calculate buy-and-hold comparison metrics from market data. Returns the return if
	 * user just bought at first price and sold at last price.
	 * @param marketData List of OHLCV data points with timestamp and close price
	...
	private BuyAndHoldMetrics calculateBuyAndHold(List<Map<String, Object>> marketData, double initialCapital) {
		...
		try {
			...
			logger.debug("Buy-and-hold calculation: firstPrice={}, lastPrice={}, return={}%", firstPrice, lastPrice,
