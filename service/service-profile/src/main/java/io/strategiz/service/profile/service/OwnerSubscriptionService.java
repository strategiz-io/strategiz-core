...
/**
 * Service for managing owner subscription settings. Handles enabling/disabling
 * subscriptions, updating settings, and retrieving subscription info.
 */
@Service
public class OwnerSubscriptionService extends BaseService {
	...
	public OwnerSubscriptionService(OwnerSubscriptionSettingsRepository subscriptionSettingsRepository,
			UserRepository userRepository, StripeConnectService stripeConnectService) {
	...
	public OwnerSubscriptionSettingsResponse enableSubscriptions(String userId, EnableSubscriptionsRequest request) {
		...
		OwnerSubscriptionSettings settings = subscriptionSettingsRepository.enableSubscriptions(userId,
	...
	/**
	 * Update Stripe Connect status. Called by Stripe webhook handlers.
	 * @param userId The owner's user ID
	...
	public OwnerSubscriptionSettingsResponse updateStripeConnectStatus(String userId, String accountId, String status) {
		...
		OwnerSubscriptionSettings settings = subscriptionSettingsRepository.updateStripeConnectStatus(userId, accountId,
				status);
	...
	/**
	 * Create a Stripe Connect account for the user. This is the first step in enabling
	 * subscriptions.
	 * @param userId The owner's user ID
	...
	/**
	 * Get the Stripe Connect onboarding link for a user. Use this when the user needs to
	 * continue or refresh onboarding.
	 * @param userId The owner's user ID
	...
	public StripeConnectStatusResponse getStripeConnectStatus(String userId) {
		...
		if (settingsOpt.isEmpty() || settingsOpt.get().getStripeConnectAccountId() == null) {
			...
			return new StripeConnectStatusResponse(null, "not_started", false, false, false, null, null);
		}
	...
	/**
	 * Refresh Stripe Connect status from Stripe API. Called after user completes
	 * onboarding or from webhook.
	 * @param userId The owner's user ID
	 * @return Updated subscription settings
	 */
	...
	/**
	 * Handle Stripe Connect account webhook update. Called when account.updated event is
	...
	public void handleStripeConnectWebhook(String accountId, String userId, boolean chargesEnabled,
		...
		log.info(
				"Handling Stripe Connect webhook for user {}: chargesEnabled={}, payoutsEnabled={}, detailsSubmitted={}",
		...
		try {
			...
			if (!accountId.equals(settings.getStripeConnectAccountId())) {
				log.warn("Account ID mismatch for user {}: expected {}, got {}", userId,
						settings.getStripeConnectAccountId(), accountId);
				return;
			}
