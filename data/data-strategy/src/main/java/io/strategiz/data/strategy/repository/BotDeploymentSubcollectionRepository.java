 ...
 *
 * This is a subcollection repository - bots are owned by users and scoped under their
 * document.
 *
 * Benefits of subcollection approach: - Ownership clarity: Bots clearly belong to users -
 * Automatic cleanup: Delete user → automatically deletes all bots - Security: Simpler
 * Firebase rules (users can only access their own bots) - Performance: Faster user
 * queries (no need to filter top-level collection) - Cost: Cheaper queries (don't scan
 * entire database)
 *
 * For cross-user queries (e.g., "find all bots for strategy X"), use collection group
 * queries.
 */
@Repository
public class BotDeploymentSubcollectionRepository extends SubcollectionRepository<BotDeployment> {
	...
	public long countRunningBots(String userId) {
		...
		return findAllInSubcollection(userId).stream().filter(bot -> "RUNNING".equals(bot.getStatus())).count();
	}
	...
	public List<BotDeployment> getAllBotsForStrategy(String strategyId) {
		try {
			...
			throw new DataStrategyException(DataStrategyErrorDetails.BOT_QUERY_FAILED, getModuleName(), e, strategyId,
					e.getMessage());
		}
	}
