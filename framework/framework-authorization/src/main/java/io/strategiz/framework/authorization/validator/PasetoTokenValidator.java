 ...
 *
 * <p>
 * This class does NOT perform database lookups. Token validation is purely
 * cryptographic. Session revocation checking is a separate concern handled by the
 * business layer if needed.
 *
 * <p>
 * Uses paseto4j library for PASETO v4.local token validation.
 */
@Component
public class PasetoTokenValidator {
	...
	public void init() {
		...
		if (identityKey == null || sessionKey == null) {
			log.error(
					"CRITICAL: Token keys not found in Vault. Token validation will fail until keys are" + " loaded.");
			log.error("Check Vault connectivity and ensure keys exist at: tokens.{}.identity-key and"
					+ " tokens.{}.session-key", env, env);
			keysLoaded = false;
		}
		else {
			keysLoaded = true;
