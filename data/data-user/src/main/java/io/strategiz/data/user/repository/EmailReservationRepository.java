 ...
 *
 * This repository leverages Firestore's document ID uniqueness to guarantee
 * email uniqueness across the platform. The document ID is the normalized
 * lowercase email address.
 *
 * Key behaviors:
 * - reserve() will fail if the email document already exists (email taken)
...
public interface EmailReservationRepository {
    ...
    /**
     * Create a new email reservation.
     * Uses Firestore create semantics - fails if document already exists.
     *
     * @param reservation The reservation to create
     * @return The created reservation
     * @throws io.strategiz.data.base.exception.DataRepositoryException if email already reserved
     */
     ...
     * An email is available if:
     * 1. No reservation exists, OR
     * 2. A PENDING reservation exists but has expired
     *
     * Note: This is a point-in-time check. For atomic operations,
     * use reserve() within a transaction.
     *
    ...
    /**
     * Confirm a pending reservation, transitioning it to CONFIRMED status.
     * Should be called within a transaction along with user creation.
     *
     * @param email The email address to confirm
     * @return The confirmed reservation
     * @throws io.strategiz.data.base.exception.DataRepositoryException if reservation not found
     */
    ...
    /**
     * Delete all expired PENDING reservations.
