...
/**
 * Base repository for StrategySubscription entities using Firestore. Used internally by
...
public class StrategySubscriptionBaseRepository extends BaseRepository<StrategySubscriptionEntity> {
	...
	public Optional<StrategySubscriptionEntity> findByUserAndStrategy(String userId, String strategyId) {
		...
		return subscriptions.stream().filter(s -> s.getStrategyId().equals(strategyId)).findFirst();
	}
	...
	public boolean hasActiveSubscription(String userId, String strategyId) {
		return findByUserAndStrategy(userId, strategyId).map(StrategySubscriptionEntity::isValid).orElse(false);
	}
	...
	public List<StrategySubscriptionEntity> findActiveByUserId(String userId, int limit) {
		try {
			List<QueryDocumentSnapshot> docs = getCollection().whereEqualTo("userId", userId)
				.whereEqualTo("isActive", true)
				.whereIn("status",
						Arrays.asList(SubscriptionStatus.ACTIVE.name(), SubscriptionStatus.TRIAL.name(),
	...
	public List<StrategySubscriptionEntity> findActiveByStrategyId(String strategyId, int limit) {
		try {
			List<QueryDocumentSnapshot> docs = getCollection().whereEqualTo("strategyId", strategyId)
				.whereEqualTo("isActive", true)
				.whereIn("status", Arrays.asList(SubscriptionStatus.ACTIVE.name(), SubscriptionStatus.TRIAL.name()))
				.orderBy("startedAt", Query.Direction.DESCENDING)
				.limit(limit)
				.get()
				.get()
	...
	public int countActiveSubscribers(String strategyId) {
		return (int) findByStrategyId(strategyId).stream().filter(StrategySubscriptionEntity::isValid).count();
	}
