...
/**
 * Client for fetching data from Alpaca using OAuth access tokens. This class handles all
 * account and position data retrieval operations.
 */
@Component
public class AlpacaDataClient {
	...
	public Map<String, Object> getPortfolioHistory(String accessToken, String period, String timeframe) {
		Map<String, String> params = Map.of("period", period != null ? period : "1M", "timeframe",
	 ...
	 */
	private <T> T makeAuthenticatedRequest(HttpMethod method, String endpoint, String accessToken,
			Map<String, String> params) {
		try {
			// Validate access token
			if (accessToken == null || accessToken.trim().isEmpty()) {
				throw new StrategizException(AlpacaErrors.ALPACA_AUTH_FAILED, "Access token is required");
			}
			...
			if (response.getBody() == null) {
				throw new StrategizException(AlpacaErrors.ALPACA_INVALID_RESPONSE, "Empty response from Alpaca API");
			}
			...
			throw new StrategizException(AlpacaErrors.ALPACA_API_ERROR, "Alpaca API error: " + responseBody);
