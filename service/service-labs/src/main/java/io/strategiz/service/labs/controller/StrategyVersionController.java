...
/**
 * REST controller for strategy versioning operations. Handles creating new versions of
 * deployed strategies and viewing version history.
 */
@RestController
...
public class StrategyVersionController extends BaseController {
	...
	/**
	 * POST /v1/strategies/{id}/version - Create a new version of a deployed strategy This
	 * creates a draft copy for editing while the original stays deployed.
	 */
	@PostMapping("/{id}/version")
	@Operation(summary = "Create strategy version",
			description = "Create a new draft version of a deployed strategy for editing")
	public ResponseEntity<StrategyResponse> createVersion(@PathVariable String id, Authentication authentication) {
	...
	/**
	 * GET /v1/strategies/{id}/versions - Get version history Returns all versions of a
	 * strategy, sorted newest first.
	 */
	@GetMapping("/{id}/versions")
	@Operation(summary = "Get version history", description = "Get all versions of a strategy")
	public ResponseEntity<List<StrategyResponse>> getVersionHistory(@PathVariable String id,
