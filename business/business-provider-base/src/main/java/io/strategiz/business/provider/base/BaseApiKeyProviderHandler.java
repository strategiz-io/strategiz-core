...
/**
 * Base implementation for API key-based provider integrations. This class provides common
 * functionality for providers that use API keys (like Kraken, Binance US) as opposed to
 * OAuth providers.
 *
 * The key pattern is: 1. Store credentials in Vault first (createIntegration) 2. Test
 * connection using stored credentials from Vault (testConnection)
 *
...
public abstract class BaseApiKeyProviderHandler implements ProviderIntegrationHandler {
	...
	/**
	 * Test connection using credentials stored in Vault. This method should always
	 * retrieve credentials from Vault to ensure it's testing with the same credentials
	 * that were stored.
	...
	public boolean testConnection(CreateProviderIntegrationRequest request, String userId) {
		...
		try {
			...
			log.error("Error testing {} connection for user: {} - Error: {}", getProviderId(), userId, e.getMessage());
			return false;
		}
	}

	/**
	 * Get stored credentials from Vault for a user. Must be implemented by concrete
	 * providers.
	 * @param userId The user ID
	...
	/**
	 * Test connection using credentials retrieved from Vault. Must be implemented by
	 * concrete providers.
	 * @param storedCredentials Credentials retrieved from Vault
	 * @param userId The user ID
	 * @return true if connection successful, false otherwise
	 */
	protected abstract boolean testConnectionWithStoredCredentials(Map<String, String> storedCredentials,
			String userId);
