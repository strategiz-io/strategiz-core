 ...
 *
 * Flow:
 * 1. User registers browser for push notifications (creates PushSubscriptionEntity)
 * 2. On sign-in attempt, system creates PushAuthRequestEntity and sends push notification
 * 3. User approves/denies from the push notification
 * 4. System validates the response and completes authentication
 *
 * Use Cases:
 * - Primary authentication (for returning users with registered devices)
 * - MFA step (as second factor)
 * - Account recovery (to verify device ownership)
 */
@Component
public class PushAuthBusiness {
     ...
     * If the endpoint already exists for this user, update it.
     */
    public PushSubscriptionEntity registerSubscription(String userId,
                                                        String endpoint,
                                                        String p256dh,
                                                        String auth,
                                                        String deviceName) {
    ...
    /**
     * Initiate a push auth request for a user.
     * Creates a pending request and returns it. The caller is responsible for sending the push notification.
     *
     ...
     */
    public InitiatePushAuthResult initiatePushAuth(String userId,
                                                    String purpose,
                                                    String ipAddress,
                                                    String location,
                                                    String userAgent) {
        ...
        if (existingPending.size() >= maxPendingRequests) {
            log.warn("User {} already has {} pending push auth requests, cancelling oldest",
                    userId, existingPending.size());
        ...
        log.info("Created push auth request {} for user {} with challenge {}",
                savedRequest.getId(), userId, maskChallenge(challenge));
    ...
    /**
     * Approve a push auth request.
     * Called when user taps "Approve" on the push notification.
     *
    ...
    public PushAuthResponse approvePushAuth(String challenge, String userId, String subscriptionId) {
        ...
        if (!request.getUserId().equals(userId)) {
            log.warn("User ID mismatch for push auth request {} - expected {}, got {}",
                    request.getId(), request.getUserId(), userId);
    ...
    /**
     * Deny a push auth request.
     * Called when user taps "Deny" on the push notification.
     */
    public PushAuthResponse denyPushAuth(String challenge, String userId) {
        ...
        // Verify user ID matches
        if (!request.getUserId().equals(userId)) {
            log.warn("User ID mismatch for push auth denial {} - expected {}, got {}",
                    request.getId(), request.getUserId(), userId);
            return new PushAuthResponse(false, "Unauthorized", null);
        }
    ...
    /**
     * Poll for push auth completion.
     * Used by the sign-in page to check if the user has approved/denied.
     *
    ...
    /**
     * Mark expired push auth requests.
    ...
    /**
     * Increment failed attempts for a subscription.
     * After too many failures, the subscription may be deactivated.
     */
    public void recordSubscriptionFailure(String subscriptionId) {
        ...
        // Check if subscription should be deactivated (after 5 failures, isValid returns false)
        if (!subscription.isValid()) {
            subscription.setPushAuthEnabled(false);
            log.warn("Deactivated push subscription {} due to {} failed attempts",
                    subscriptionId, subscription.getFailedAttempts());
        }
     ...
     */
    public record InitiatePushAuthResult(
        boolean success,
        String error,
        PushAuthRequestEntity request,
     ...
     */
    public record PushAuthResponse(
        boolean success,
        String message,
        PushAuthRequestEntity request
    ) {}
