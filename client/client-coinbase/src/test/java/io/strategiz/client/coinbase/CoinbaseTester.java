...
/**
 * Simple tester class for Coinbase API integration This class demonstrates how to use the
 * CoinbaseClient to make API calls
 */
public class CoinbaseTester {
	...
	public static void main(String[] args) {
		...
		try {
			...
			CoinbaseResponse<TickerPrice> response = client.publicRequest(HttpMethod.GET, "/prices/BTC-USD/spot",
					params, new ParameterizedTypeReference<CoinbaseResponse<TickerPrice>>() {
					});

			System.out.println("Bitcoin price: " + response.getData().get(0).getAmount() + " "
					+ response.getData().get(0).getCurrency());
			...
			if (!API_KEY.equals("your-api-key")) {
				...
				CoinbaseResponse<Map<String, Object>> accountsResponse = client.signedRequest(HttpMethod.GET,
						"/accounts", null, API_KEY, PRIVATE_KEY,
