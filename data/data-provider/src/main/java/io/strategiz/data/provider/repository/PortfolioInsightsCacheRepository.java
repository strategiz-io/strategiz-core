...
/**
 * Repository for caching AI portfolio insights. Path:
 * users/{userId}/cache/portfolioInsights
 *
 * Cache Strategy: - Return cached insights immediately if valid (< 24 hours old, not
 * invalidated) - Invalidate cache when provider data changes (sync, connect, disconnect)
...
public class PortfolioInsightsCacheRepository {
	...
	public Optional<PortfolioInsightsCacheEntity> getCachedInsights(String userId) {
		try {
			...
			log.info("Returning valid cached insights for user: {}, expires: {}", userId, cache.getExpiresAt());
	...
	public void saveCache(String userId, PortfolioInsightsCacheEntity cache) {
		try {
			...
			cache.setExpiresAt(
					Timestamp.ofTimeSecondsAndNanos(now.getSeconds() + TimeUnit.HOURS.toSeconds(CACHE_TTL_HOURS), 0));
