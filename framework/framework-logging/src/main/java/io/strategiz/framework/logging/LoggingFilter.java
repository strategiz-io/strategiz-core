...
/**
 * Enhanced servlet filter that sets up request context information for logging. This
 * filter runs at the highest precedence to ensure context is set as early as possible in
 * the request lifecycle.
 *
 * Features: - Request correlation ID generation - Request timing and performance logging
 * - User context extraction from headers/sessions - Structured logging context setup
 */
@Component
...
public class LoggingFilter implements Filter {
	...
	private void logRequestEnd(HttpServletRequest request, HttpServletResponse response, long durationMs) {
		...
		if (durationMs > SLOW_REQUEST_THRESHOLD) {
			log.warn("Slow request completed: {} {} - {} - {}ms", method, path, statusCode, durationMs);
		}
		else if (statusCode >= 400) {
			log.warn("Request completed with error: {} {} - {} - {}ms", method, path, statusCode, durationMs);
		}
		else {
			log.debug("Request completed: {} {} - {} - {}ms", method, path, statusCode, durationMs);
		}
