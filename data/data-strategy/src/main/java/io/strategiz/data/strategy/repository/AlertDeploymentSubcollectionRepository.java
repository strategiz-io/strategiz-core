 ...
 *
 * This is a subcollection repository - alerts are owned by users and scoped under their
 * document.
 *
 * Benefits of subcollection approach: - Ownership clarity: Alerts clearly belong to users
 * - Automatic cleanup: Delete user → automatically deletes all alerts - Security: Simpler
 * Firebase rules (users can only access their own alerts) - Performance: Faster user
 * queries (no need to filter top-level collection) - Cost: Cheaper queries (don't scan
 * entire database)
 *
 * For cross-user queries (e.g., "find all alerts for strategy X"), use collection group
 * queries.
 */
@Repository
public class AlertDeploymentSubcollectionRepository extends SubcollectionRepository<AlertDeployment> {
	...
	public long countActiveAlerts(String userId) {
		...
		return findAllInSubcollection(userId).stream().filter(alert -> "ACTIVE".equals(alert.getStatus())).count();
	}

	/**
	 * Get all alerts across ALL users for a specific strategy (collection group query).
	 *
	 * This is useful for calculating strategy.deploymentCount. Note: This is a collection
	 * group query - more expensive than subcollection queries.
	 * @param strategyId The strategy ID
	...
	public List<AlertDeployment> getAllAlertsForStrategy(String strategyId) {
		try {
			...
			throw new DataStrategyException(DataStrategyErrorDetails.ALERT_QUERY_FAILED, getModuleName(), e, strategyId,
					e.getMessage());
		}
	}
