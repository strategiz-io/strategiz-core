...
/**
 * Model for ClickHouse Cloud API cost response. Represents costs in ClickHouse Credits
 * (CHC) with breakdown by category.
 *
 ...
 */
public record ClickHouseCloudCost(LocalDate startDate, LocalDate endDate, BigDecimal totalCostChc,
		BigDecimal computeCostChc, BigDecimal storageCostChc, BigDecimal backupCostChc, BigDecimal dataTransferCostChc,
		String currency) {
	...
	public static ClickHouseCloudCost empty(LocalDate startDate, LocalDate endDate) {
		return new ClickHouseCloudCost(startDate, endDate, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO,
				BigDecimal.ZERO, BigDecimal.ZERO, "USD");
	}

	/**
	 * Convert ClickHouse Credits (CHC) to USD. Note: 1 CHC = $1 USD as of 2025
	 */
