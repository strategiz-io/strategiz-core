...
/**
 * Client for retrieving GCP infrastructure inventory using Cloud Asset API.
 * Provides comprehensive resource listing and categorization.
 *
 * Cloud Asset Inventory API is FREE to use.
...
public class GcpAssetInventoryClient {
        ...
        try (AssetServiceClient client = AssetServiceClient.create()) {
            ...
            for (com.google.cloud.asset.v1.Asset asset : response.getAssetsList()) {
                ...
                GcpResource resource = new GcpResource(
                        ...
                        region,
                        "ACTIVE",
                        Instant.now(), // Would need additional API call for actual creation time
                        estimatedCost,
                        labels
                );
    ...
    private String extractResourceName(String fullName) {
        ...
        // Format: //service.googleapis.com/projects/{project}/locations/{location}/resources/{name}
    ...
    private BigDecimal estimateResourceCost(String resourceType, String resourceName) {
        ...
        // This is a simplified estimation - actual costs come from BigQuery billing export

        try {
            // Query last month's cost for this specific service
            BigDecimal serviceCost = billingClient.getServiceCost(
                    resourceType,
                    java.time.LocalDate.now().minusMonths(1).withDayOfMonth(1),
                    java.time.LocalDate.now().minusMonths(1).withDayOfMonth(
                            java.time.LocalDate.now().minusMonths(1).lengthOfMonth()
                    )
            );
