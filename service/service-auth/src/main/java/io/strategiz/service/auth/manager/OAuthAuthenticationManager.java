...
public class OAuthAuthenticationManager {
	...
	public void ensureOAuthMethod(UserEntity user, String provider, String providerId, String email) {
		...
		List<AuthenticationMethodEntity> existingMethods = findExistingOAuthMethods(user.getUserId(), oauthType,
				providerId);
	...
	private AuthenticationMethodType getOAuthTypeForProvider(String provider) {
		return switch (provider.toLowerCase()) {
			...
			default -> throw new StrategizException(ServiceAuthErrorDetails.OAUTH_CONFIGURATION_ERROR, "service-auth",
					"Unsupported OAuth provider: " + provider);
		};
	}

	private List<AuthenticationMethodEntity> findExistingOAuthMethods(String userId, AuthenticationMethodType oauthType,
			String providerId) {
		// Find existing OAuth methods of the same type for this user
		List<AuthenticationMethodEntity> methods = authenticationMethodRepository.findByUserIdAndType(userId,
				oauthType);
	...
	private void createNewOAuthMethod(UserEntity user, String provider, String providerId, String email,
			AuthenticationMethodType oauthType) {
	...
	private void activateExistingMethodIfNeeded(AuthenticationMethodEntity entity, UserEntity user) {
		...
		if (!entity.getIsActive()) {
			logger.info("Reactivating {} OAuth method for user: {}", entity.getAuthenticationMethod(),
					user.getUserId());
	...
	private AuthenticationMethodEntity buildOAuthMethodEntity(UserEntity user, String provider, String providerId,
			String email, AuthenticationMethodType oauthType) {
