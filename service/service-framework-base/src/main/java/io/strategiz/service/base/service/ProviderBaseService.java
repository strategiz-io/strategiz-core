...
/**
 * Base service for all provider-related services. Extends BaseService and adds
 * provider-specific common functionality.
 *
...
public abstract class ProviderBaseService extends BaseService {
	...
	protected String generateState(String userId, String context) {
		...
		providerLog.debug("Generated state parameter for user: {}, provider: {}, context: {}", userId, getProviderId(),
				context);
		return state;
	}
	...
	protected void logApiCall(String userId, String endpoint, boolean success, long responseTimeMs) {
		if (success) {
			providerLog.info("API_CALL_SUCCESS - User: {}, Provider: {}, Endpoint: {}, Time: {}ms", userId,
					getProviderId(), endpoint, responseTimeMs);
		}
		else {
			providerLog.warn("API_CALL_FAILURE - User: {}, Provider: {}, Endpoint: {}, Time: {}ms", userId,
					getProviderId(), endpoint, responseTimeMs);
		}
	}
	...
	protected boolean areCredentialsConfigured(String userId) {
		...
		providerLog.debug("Checking credentials configuration for user: {}, provider: {}", userId, getProviderId());
		return false;
	}
	...
	/**
	 * Get the provider ID for this service Each provider service must implement this
	 */
	protected abstract String getProviderId();

	/**
	 * Get the provider display name Each provider service must implement this
	 */
