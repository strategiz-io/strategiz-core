...
/**
 * Business logic for Coinbase provider integration. Handles OAuth flows, API
 * interactions, and business rules specific to Coinbase.
 */
@Component
public class CoinbaseProviderBusiness implements ProviderIntegrationHandler {
	...
	@Autowired
	public CoinbaseProviderBusiness(CoinbaseClient coinbaseClient, CoinbaseDataClient coinbaseDataClient,
			CoinbaseOAuthClient coinbaseOAuthClient, PortfolioProviderRepository portfolioProviderRepository,
	...
	public String generateAuthorizationUrl(String userId, String state) {
		...
		String authorizationUrl = String.format("%s?client_id=%s&redirect_uri=%s&state=%s&scope=%s&response_type=code",
				authUrl, clientId, redirectUri, state, scope);
	...
	public Map<String, Object> getAccountBalance(String userId, String accessToken) {
		...
		try {
			...
			Map<String, Object> balanceData = makeAuthenticatedRequest("/accounts", accessToken, null);
	 ...
	 */
	public Map<String, Object> getTransactionHistory(String userId, String accessToken, String accountId,
			Integer limit) {
		validateRequired("userId", userId);
		validateRequired("accessToken", accessToken);

		try {
			String endpoint = accountId != null ? String.format("/accounts/%s/transactions", accountId)
					: "/transactions";
			...
			Map<String, Object> transactionData = makeAuthenticatedRequest(endpoint, accessToken, params);
	...
	private void validateOAuthConfiguration() {
		if (clientId == null || clientId.trim().isEmpty()) {
			throw new StrategizException(ErrorCode.VALIDATION_ERROR, "Coinbase OAuth client ID is not configured");
		}
		if (clientSecret == null || clientSecret.trim().isEmpty()) {
			throw new StrategizException(ErrorCode.VALIDATION_ERROR, "Coinbase OAuth client secret is not configured");
		}
		if (redirectUri == null || redirectUri.trim().isEmpty()) {
			throw new StrategizException(ErrorCode.VALIDATION_ERROR, "Coinbase OAuth redirect URI is not configured");
		}
	}
	...
	private Map<String, Object> makeAuthenticatedRequest(String endpoint, String accessToken,
		...
		try {
			...
			if (endpoint.equals("/user")) {
				...
				return coinbaseClient.oauthRequest(HttpMethod.GET, endpoint, accessToken, params,
	...
	public ProviderIntegrationResult createIntegration(CreateProviderIntegrationRequest request, String userId) {
		...
		log.info("OAuth configuration - clientId: {}, redirectUri: {}", clientId != null ? "configured" : "null",
		...
		try {
			...
			throw new StrategizException(CoinbaseProviderErrorDetails.INTEGRATION_CREATION_FAILED,
					"business-provider-coinbase", e, userId);
		}
	}
	...
	public void completeOAuthFlow(String userId, String authorizationCode, String state) {
		...
		try {
			...
			throw new StrategizException(CoinbaseProviderErrorDetails.OAUTH_FLOW_FAILED, "business-provider-coinbase",
					e, userId);
		}
	}
	 ...
	 * fetches latest portfolio data from Coinbase
	...
	public ProviderHoldingsEntity syncProviderData(String userId) {
		...
		try {
			...
			if (secretData == null || secretData.isEmpty()) {
				throw new StrategizException(CoinbaseProviderErrorDetails.TOKENS_NOT_FOUND,
						"business-provider-coinbase", userId);
			}

			String accessToken = (String) secretData.get("accessToken");
			if (accessToken == null || accessToken.isEmpty()) {
				throw new StrategizException(CoinbaseProviderErrorDetails.ACCESS_TOKEN_NOT_FOUND,
						"business-provider-coinbase", userId);
			}
			...
			if (holdings.isPresent()) {
				...
				throw new StrategizException(CoinbaseProviderErrorDetails.NO_DATA_AFTER_SYNC,
						"business-provider-coinbase", userId);
			}
			...
			throw new StrategizException(CoinbaseProviderErrorDetails.SYNC_FAILED, "business-provider-coinbase", e,
					userId);
		}
	}
	...
	private void storeTokensInVault(String userId, String accessToken, String refreshToken, Instant expiresAt) {
		try {
			...
			throw new StrategizException(CoinbaseProviderErrorDetails.TOKEN_STORAGE_FAILED,
					"business-provider-coinbase", e, userId);
		}
	}
	...
	/**
	 * Fetch portfolio data from Coinbase and store in new portfolio structure. Path:
	...
	private void fetchAndStorePortfolioData(String userId, String accessToken) {
		...
		try {
			...
			ProviderHoldingsEntity holdingsEntity = transformToProviderHoldingsEntity(accountsResponse, userId,
					accessToken);
			...
			log.info("Successfully stored Coinbase portfolio data for user: {} with {} holdings", userId,
			...
			throw new StrategizException(CoinbaseProviderErrorDetails.PORTFOLIO_FETCH_FAILED,
					"business-provider-coinbase", e, userId);
		}
	}
	 ...
	 */
	private ProviderHoldingsEntity transformToProviderHoldingsEntity(Map<String, Object> accountsResponse,
	...
	private boolean isFiatCurrency(String symbol) {
		return symbol != null && (symbol.equals("USD") || symbol.equals("EUR") || symbol.equals("GBP")
	...
	private CostBasisData calculateCostBasis(String accessToken, String accountId, String currency) {
		try {
			...
			// Fetch transactions for this account (limit to 100 most recent for
			// performance)
			...
			for (Map<String, Object> transaction : transactions) {
				try {
					...
					// Parse values
					BigDecimal nativeAmountValue = new BigDecimal(nativeAmountStr).abs(); // Use
																							// abs()
																							// in
																							// case
																							// it's
			...
			if (totalQuantityBought.compareTo(BigDecimal.ZERO) > 0) {
				...
				log.debug("Calculated cost basis for {}: totalCost=${}, totalQty={}, avgPrice=${}", currency,
						totalCostBasis, totalQuantityBought, averageBuyPrice);
			}
			else {
				log.debug("No buy transactions found for {}", currency);
			}

			return new CostBasisData(totalCostBasis.compareTo(BigDecimal.ZERO) > 0 ? totalCostBasis : null,
					averageBuyPrice);
