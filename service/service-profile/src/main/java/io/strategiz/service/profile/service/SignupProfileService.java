...
public class SignupProfileService extends BaseService {
	...
	/**
	 * Generate an identity token for signup flow This token uses the identity-key (not
	 * session-key) and has limited scope It's used to verify identity during the
	 * multi-step signup process
	 *
	 * Two-Phase Token Flow: Phase 1 (Signup): identity token with scope="profile:create",
	 * acr="0" Phase 2 (After Auth): session token with full scopes, acr="1"+"
	 */
	private String generatePartialAuthToken(String userId, String email) {
		...
		boolean isValidUUID = userId != null
