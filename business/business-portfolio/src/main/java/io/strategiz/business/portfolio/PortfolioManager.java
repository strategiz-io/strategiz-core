...
/**
 * Core business logic for portfolio operations. Aggregates portfolio data from multiple
 * providers stored in Firestore. Implements clean architecture pattern - reads from data
 * layer, performs business logic.
 */
@Component
public class PortfolioManager {
	...
	/**
	 * Aggregates portfolio data from ALL connected providers. Reads provider data from
	 * Firestore (already synced by provider sync processes).
	...
	public PortfolioData getAggregatedPortfolioData(String userId) {
		...
		try {
			...
			for (PortfolioProviderEntity provider : providers) {
				log.info("=====> Provider found: providerId={}, name={}, totalValue={}", provider.getProviderId(),
						provider.getProviderName(), provider.getTotalValue());
			}
			...
			for (PortfolioProviderEntity provider : providers) {
				try {
					...
					// Build exchange data for this provider (including holdings from
					// subcollection)
					...
					log.debug("Processed provider {}: totalValue={}, dayChange={}", provider.getProviderId(),
							provider.getTotalValue(), provider.getDayChange());

				}
				catch (Exception e) {
					log.error("Error processing provider {} for user {}: {}", provider.getProviderId(), userId,
							e.getMessage(), e);
			...
			log.info("Aggregated portfolio for user {}: totalValue={}, dayChange={} ({}%), {} exchanges", userId,
					totalValue, totalDayChange, dayChangePercent, exchanges.size());
			...
			throw new StrategizException(PortfolioErrorDetails.PORTFOLIO_DATA_RETRIEVAL_FAILED, "business-portfolio", e,
					userId);
		}
	}

	/**
	 * Builds ExchangeData from PortfolioProviderEntity, including holdings from
	 * subcollection.
	...
	private PortfolioData.ExchangeData buildExchangeData(String userId, PortfolioProviderEntity provider) {
		...
		exchangeData
			.setName(provider.getProviderName() != null ? provider.getProviderName() : provider.getProviderId());
		exchangeData.setValue(provider.getTotalValue() != null ? provider.getTotalValue() : BigDecimal.ZERO);

		// Fetch holdings from subcollection:
		// users/{userId}/portfolio/{providerId}/holdings/current
		Map<String, PortfolioData.AssetData> assets = new HashMap<>();
		try {
			Optional<ProviderHoldingsEntity> holdingsOpt = providerHoldingsRepository.findByUserIdAndProviderId(userId,
			...
			if (holdingsOpt.isPresent()) {
				...
				if (holdingsList != null && !holdingsList.isEmpty()) {
					log.info("=====> Found {} holdings for provider {} (user: {})", holdingsList.size(),
					...
					log.warn("=====> Holdings list is empty for provider {} (user: {})", provider.getProviderId(),
							userId);
				}
			}
			else {
				log.warn("=====> No holdings document found for provider {} (user: {})", provider.getProviderId(),
						userId);
			}
		}
		catch (Exception e) {
			log.error("Error fetching holdings for provider {} (user: {}): {}", provider.getProviderId(), userId,
					e.getMessage(), e);
			// Continue without holdings - at least we have the summary data
		}

		exchangeData.setAssets(assets);
		log.info("=====> Built exchange data for {} with {} assets", provider.getProviderId(), assets.size());
	...
	/**
	 * Overloaded method to support the legacy Map<String, Object> interface This is a
	 * temporary bridge method that will be removed once all services are migrated to
	 * Synapse
