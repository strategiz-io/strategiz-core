...
/**
 * Kraken API Authentication Client Handles API key authentication and request signing for
 * Kraken API
 */
@Service
public class KrakenApiAuthClient {
	...
	public String createSignature(String path, String nonce, String postData, String apiSecret) {
		try {
			...
			if (apiSecret == null || apiSecret.trim().isEmpty()) {
				...
				throw new StrategizException(ClientErrorDetails.MISSING_CREDENTIALS, "kraken-api",
						"API secret is null or empty");
			}
			...
			try {
				...
				throw new StrategizException(ClientErrorDetails.INVALID_CREDENTIALS, "kraken-api", e,
	 ...
	 */
	public Mono<Map<String, Object>> makeAuthenticatedRequest(String endpoint, String apiKey, String apiSecret,
			Map<String, String> params, String otp) {
		...
		if (params != null) {
			params.forEach((key, value) -> postData.append("&").append(key).append("=").append(value));
		}
		...
		// Make request
		WebClient webClient = webClientBuilder.baseUrl(KRAKEN_API_BASE_URL).build();
