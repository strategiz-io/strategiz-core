...
/**
 * Comprehensive integration tests for MarketTickerController Tests market ticker data
...
class MarketTickerControllerTest {
	...
	class GetMarketTickersTests {
		...
		void shouldReturnDemoDataStructureCorrectly() throws Exception {
			...
			// Note: CoinbaseClient doesn't have getTickerPrice method implemented yet
			when(alphaVantageClient.getStockQuote(anyString())).thenThrow(new RuntimeException("API unavailable"));
		...
		void shouldHandlePartialApiFailuresGracefully() throws Exception {
			...
			// Note: CoinbaseClient doesn't have getTickerPrice method implemented yet
			when(alphaVantageClient.getStockQuote(anyString())).thenThrow(new RuntimeException("Stock API down"));
		...
		void shouldIncludeProperCorsHeaders() throws Exception {
			...
			// Note: CoinbaseClient doesn't have getTickerPrice method implemented yet
			when(alphaVantageClient.getStockQuote(anyString())).thenThrow(new RuntimeException("Use demo data"));

			// When & Then
			mockMvc
				.perform(
						get(BASE_URL).contentType(MediaType.APPLICATION_JSON).header("Origin", "http://localhost:3000"))
				.andDo(print())
				.andExpect(status().isOk())
	...
	class ErrorHandlingTests {
		...
		void shouldHandleCompleteApiServiceOutageGracefully() throws Exception {
			...
			// Note: CoinbaseClient and CoinGeckoClient don't have the expected methods
			// implemented yet
			when(alphaVantageClient.getStockQuote(anyString()))
		...
		void shouldNotExposeInternalApiErrorsToClient() throws Exception {
			...
			// When & Then - Should return generic response without exposing internal
			// details
			mockMvc.perform(get(BASE_URL))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.items").exists())
				...
				.andExpect(content()
					.string(org.hamcrest.Matchers.not(org.hamcrest.Matchers.containsString("sk-1234567890"))))
				.andExpect(
						content().string(org.hamcrest.Matchers.not(org.hamcrest.Matchers.containsString("API key"))));
		}
