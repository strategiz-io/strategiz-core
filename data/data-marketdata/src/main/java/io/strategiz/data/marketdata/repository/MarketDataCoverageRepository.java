...
/**
 * Repository interface for market data coverage statistics. Stores periodic snapshots of
 * data completeness metrics.
 *
 * Coverage snapshots are calculated daily (or on-demand) and stored in Firestore for
 * historical tracking and trend analysis.
 */
public interface MarketDataCoverageRepository {

	/**
	 * Save a coverage snapshot. Creates or updates the snapshot with audit fields.
	 * @param entity The coverage entity to save
	 * @param userId The user ID triggering the save (typically "system" for scheduled
	 * jobs)
	...
	/**
	 * Get the most recent coverage snapshot. Sorted by calculatedAt descending.
	 * @return Optional containing the latest snapshot, or empty if none exist
	 */
	Optional<MarketDataCoverageEntity> findLatest();

	/**
	 * Find coverage snapshots within a date range. Useful for viewing historical trends.
	...
	/**
	 * Find the N most recent snapshots. Useful for displaying recent history.
	...
	/**
	 * Delete old coverage snapshots based on retention policy. Typically deletes
