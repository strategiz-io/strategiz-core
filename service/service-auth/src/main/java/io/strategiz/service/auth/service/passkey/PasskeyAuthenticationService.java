...
public class PasskeyAuthenticationService extends BaseService {
	...
	public PasskeyAuthenticationService(PasskeyChallengeService challengeService,
	 ...
	 */
	public record AuthenticationResult(boolean success, String accessToken, String refreshToken, String userId,
			String errorMessage) {
	 ...
	 */
	public record AuthenticationCompletion(String credentialId, String authenticatorData, String clientDataJSON,
			String signature, String userHandle, String ipAddress, String deviceId) {
	}
	...
	private Object createAuthenticationOptions(String challenge) {
		...
		return Map.of("challenge", challenge, "timeout", challengeTimeoutMs, "rpId", rpId, "allowCredentials",
				List.of(), "userVerification", "preferred");
	}
	...
	public AuthenticationResult completeAuthentication(AuthenticationCompletion completion) {
		...
		Optional<AuthenticationMethodEntity> authMethodOpt = authMethodRepository
			.findByPasskeyCredentialId(credentialId);
		...
		String publicKeyBase64 = authMethod
			.getMetadataAsString(AuthenticationMethodMetadata.PasskeyMetadata.PUBLIC_KEY_BASE64);

		// Extract userId from the authentication method's path
		// The authentication method repository should have set this, but we can also
		// extract from path
		String userId = extractUserIdFromAuthMethod(authMethod);

		try {
			// Verify signature using the utility
			boolean signatureValid = PasskeySignatureVerifier.verifySignature(publicKeyBase64, authenticatorData,
			...
			SessionAuthBusiness.AuthRequest authRequest = new SessionAuthBusiness.AuthRequest(userId, null, // userEmail
					...
					deviceId, deviceId, // Use deviceId as fingerprint for now
					ipAddress, "Passkey Client", // userAgent placeholder
					demoMode);
	...
	public Optional<Passkey> getCredentialByCredentialId(String credentialId) {
		return authMethodRepository.findByPasskeyCredentialId(credentialId).map(this::convertAuthMethodToPasskey);
	}
	...
	public boolean credentialExistsForUser(String credentialId, String userId) {
		...
		return authMethodRepository.findByPasskeyCredentialId(credentialId).map(authMethod -> {
	 ...
	 */
	/**
	 * Extract userId from authentication method The method should contain userId info or
	 * we extract from document path
	 */
	...
	private Passkey convertAuthMethodToPasskey(AuthenticationMethodEntity authMethod) {
		...
		model.setCredentialId(
				authMethod.getMetadataAsString(AuthenticationMethodMetadata.PasskeyMetadata.CREDENTIAL_ID));
		model.setUserId(extractUserIdFromAuthMethod(authMethod));
		model.setAuthenticatorName(
				authMethod.getMetadataAsString(AuthenticationMethodMetadata.PasskeyMetadata.AUTHENTICATOR_NAME));
		// Use createdDate from BaseEntity instead of separate REGISTRATION_TIME metadata
		if (authMethod.getCreatedDate() != null) {
			model.setRegistrationTime(Instant.ofEpochSecond(authMethod.getCreatedDate().getSeconds(),
		...
		model.setTrusted(Boolean
			.parseBoolean(authMethod.getMetadataAsString(AuthenticationMethodMetadata.PasskeyMetadata.TRUSTED)));
