...
import io.strategiz.trading.agent.gemini.model.GeminiTradingSignal;
import io.strategiz.trading.agent.gemini.model.TradingAgentPrompt;
...
public class GeminiTradingAgent {
    ...
    private String detectMarketTrends(List<HistoricalPriceData> data) {
        ...
        if (mediumTermChange > 15.0) {
            trendBuilder.append("Strong medium-term bullish trend. ");
        } else if (mediumTermChange > 5.0) {
            trendBuilder.append("Moderate medium-term bullish trend. ");
        } else if (mediumTermChange < -15.0) {
            trendBuilder.append("Strong medium-term bearish trend. ");
        } else if (mediumTermChange < -5.0) {
            trendBuilder.append("Moderate medium-term bearish trend. ");
        } else {
            trendBuilder.append("Neutral medium-term trend. ");
        }
        
        // Volatility
        if (recentVolatility > 0.03) {
            trendBuilder.append("High market volatility.");
        } else if (recentVolatility > 0.015) {
            trendBuilder.append("Moderate market volatility.");
        } else {
            trendBuilder.append("Low market volatility.");
        }
    ...
    private double calculatePriceChange(List<HistoricalPriceData> data, int period) {
        ...
        double currentPrice = data.get(0).getClose();
        double pastPrice = data.get(Math.min(period, data.size() - 1)).getClose();
     ...
     */
    private double calculateAverageVolume(List<HistoricalPriceData> data, int days) {
        int periods = Math.min(days, data.size());
        
        double totalVolume = 0;
        for (int i = 0; i < periods; i++) {
            totalVolume += data.get(i).getVolume();
        }
    ...
    private double calculateVolumeChange(List<HistoricalPriceData> data, int period) {
        ...
        double pastVolume = data.get(Math.min(period, data.size() - 1)).getVolume();
        
        if (pastVolume == 0) {
            return 0.0;
        }
        
        return ((currentVolume - pastVolume) / pastVolume) * 100.0;
    }
    ...
    private double calculateVolatility(List<HistoricalPriceData> data, int period) {
        int periods = Math.min(period, data.size() - 1);
        if (periods < 2) {
            return 0.0;
        }
        ...
        double variance = sumSquaredDev / returns.size();
