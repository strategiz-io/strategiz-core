...
/**
 * Client for interacting with the AlphaVantage API. This class handles all low-level API
 * communication with AlphaVantage following Synapse patterns.
 */
@Component
public class AlphaVantageClient {
	...
	public StockData getStockQuote(String symbol) {
		...
		try {
			...
			if (!response.getBody().containsKey("Global Quote")) {
				log.error("Invalid response format from AlphaVantage API for symbol: {}. Response: {}", symbol,
						response.getBody());
	...
	public Map<String, StockData> getBatchStockQuotes(List<String> symbols) {
		...
		// AlphaVantage doesn't have a true batch API, so we need to make multiple
		// requests
