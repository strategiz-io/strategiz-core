...
/**
 * Service for Signal Scout agent - market signal analysis. Enhanced with Financial
 * Modeling Prep (FMP) API for real-time market data including quotes, technical
 * indicators (RSI, MACD, SMA), sector performance, and market news.
 */
@Service
...
public class SignalScoutService extends BaseService {
			...
			try {
				...
				if (!indexQuotes.isEmpty()) {
					...
					if (topIndex.getChangePercent() != null) {
						String direction = topIndex.getChangePercent().compareTo(BigDecimal.ZERO) >= 0 ? "bullish"
								: "bearish";
						String sign = topIndex.getChangePercent().compareTo(BigDecimal.ZERO) >= 0 ? "+" : "";
						signals.add(new MarketSignalDto("momentum", direction, getIndexName(topIndex.getSymbol()),
								String.format("%s %s%.2f%% ($%.2f)", topIndex.getSymbol(), sign,
										topIndex.getChangePercent(), topIndex.getPrice()),
								"High", topIndex.getChangePercent()));
					}
				}
			}
			catch (Exception e) {
			...
			try {
				...
				if (!sectorQuotes.isEmpty()) {
					...
					if (topSector.getChangePercent() != null) {
						String direction = topSector.getChangePercent().compareTo(BigDecimal.ZERO) >= 0 ? "bullish"
								: "bearish";
						String sign = topSector.getChangePercent().compareTo(BigDecimal.ZERO) >= 0 ? "+" : "";
						signals.add(new MarketSignalDto("momentum", direction, getSectorName(topSector.getSymbol()),
								String.format("Leading sector %s%.2f%%", sign, topSector.getChangePercent()), "Medium",
					...
					if (bottomSector.getChangePercent() != null
							&& !bottomSector.getSymbol().equals(topSector.getSymbol())) {
						String direction = bottomSector.getChangePercent().compareTo(BigDecimal.ZERO) >= 0 ? "bullish"
								: "bearish";
						String sign = bottomSector.getChangePercent().compareTo(BigDecimal.ZERO) >= 0 ? "+" : "";
						signals.add(new MarketSignalDto("oversold", direction, getSectorName(bottomSector.getSymbol()),
								String.format("Lagging sector %s%.2f%%", sign, bottomSector.getChangePercent()), "Low",
		...
		if (fmpQuoteClient != null && fmpQuoteClient.isConfigured()) {
			try {
				...
				if (vix != null && vix.getPrice() != null) {
					...
					signals.add(new MarketSignalDto("sentiment", direction, "VIX",
							String.format("VIX at %.2f - %s", vixValue, sentiment), confidence, vix.getPrice()));
				}
			}
			catch (Exception e) {
	...
	private String buildMarketContext(AgentChatRequest request) {
		...
		else if (messageLower.contains("market")
				&& (messageLower.contains("condition") || messageLower.contains("overview"))) {
