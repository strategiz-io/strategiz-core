...
/**
 * Repository interface for company fundamentals operations. Abstracts the underlying
 * storage implementation (database-agnostic).
 *
 * <p>
...
public interface FundamentalsRepository {
	...
	/**
	 * Find latest fundamentals for a symbol (any period type). Returns the most recent
	 * record based on fiscal_period.
	 ...
	 */
	List<FundamentalsEntity> findBySymbolAndPeriodTypeOrderByFiscalPeriodDesc(String symbol, String periodType);
	...
	/**
	 * Find fundamentals for multiple symbols (latest for each). Useful for batch queries
	 * in strategy execution or portfolio analysis.
	...
	/**
	 * Stock screener: Find symbols with P/E ratio in a specific range. Only considers
	 * latest fundamentals (TTM or most recent period).
	...
	/**
	 * Delete all fundamentals records for a symbol. Useful for cleanup or re-importing
	 * data.
	 * @param symbol the stock symbol
	 */
