...
/**
 * Client for retrieving Anthropic Claude API costs and usage data.
 * Uses Anthropic's official Cost Report API.
 *
...
public class AnthropicBillingClient {
    ...
    public AiProviderCost getCostReport(LocalDate startDate, LocalDate endDate) {
        ...
        try {
            String url = String.format("%s/organizations/cost_report?start_date=%s&end_date=%s",
                    API_BASE_URL,
    ...
    public Map<String, Long> getUsageReport(LocalDate startDate, LocalDate endDate) {
        ...
        try {
            String url = String.format("%s/organizations/usage_report/messages?start_date=%s&end_date=%s",
                    API_BASE_URL,
                    startDate.format(DATE_FORMAT),
                    endDate.format(DATE_FORMAT));
    ...
    private AiProviderCost parseCostReport(String jsonResponse, LocalDate startDate, LocalDate endDate) {
        try {
            ...
            log.info("Parsed Anthropic cost report: ${} total, {} tokens, {} requests",
