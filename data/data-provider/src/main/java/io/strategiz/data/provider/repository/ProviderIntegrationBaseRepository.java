...
/**
 * Base repository for ProviderIntegration entities using Firestore Stores provider
 * integrations as subcollection under users/{userId}/provider_integrations
 */
@Repository
public class ProviderIntegrationBaseRepository extends BaseRepository<ProviderIntegrationEntity> {
	...
	/**
	 * Get user-scoped collection for provider integrations Returns
	 * users/{userId}/provider_integrations
	 */
	...
	public ProviderIntegrationEntity save(ProviderIntegrationEntity entity, String userId) {
		try {
			...
			if (isCreate) {
				...
				if (!entity._hasAudit()) {
					throw new ProviderIntegrationException(DataProviderErrorDetails.AUDIT_FIELDS_MISSING,
							"ProviderIntegrationEntity");
				}
	...
	public List<ProviderIntegrationEntity> findAllByUserId(String userId) {
		try {
			...
			Query query = getUserScopedCollection(userId).whereIn("status", Arrays.asList("connected", "CONNECTED"))
				.whereEqualTo("isActive", true);

			log.debug(
					"ProviderIntegrationBaseRepository: Query path: provider_integrations/users/{}/integrations, filter: status={}",
			...
			List<ProviderIntegrationEntity> results = docs.stream().map(doc -> {
				ProviderIntegrationEntity entity = doc.toObject(ProviderIntegrationEntity.class);
				entity.setId(doc.getId());
			...
			Thread.currentThread().interrupt();
			log.error("ProviderIntegrationBaseRepository: Failed to find entities for userId {}: {}", userId,
					e.getMessage());
			throw new ProviderIntegrationException(DataProviderErrorDetails.REPOSITORY_FIND_FAILED, e,
					"ProviderIntegrationEntity", userId);
		}
		catch (ExecutionException e) {
			log.error("ProviderIntegrationBaseRepository: Failed to find entities for userId {}: {}", userId,
					e.getMessage());
	...
	public Optional<ProviderIntegrationEntity> findByUserIdAndProviderId(String userId, String providerId) {
		return findAllByUserId(userId).stream().filter(entity -> providerId.equals(entity.getProviderId())).findFirst();
	}
