...
/**
 * Base repository for PortfolioSummary entities using Firestore Stores portfolio summary
 * at: users/{userId}/portfolio/summary
 *
 * The summary document lives alongside provider documents in the portfolio collection.
 * Example structure: users/{userId}/portfolio/summary ← This document
 * users/{userId}/portfolio/coinbase ← Provider document users/{userId}/portfolio/alpaca ←
 * Provider document
 */
@Repository
public class PortfolioSummaryBaseRepository extends BaseRepository<PortfolioSummaryEntity> {
	...
	/**
	 * Get user-scoped portfolio collection Returns users/{userId}/portfolio
	 */
	...
	public List<PortfolioSummaryEntity> findByDateRange(String userId, Instant from, Instant to) {
		try {
			Query query = getPortfolioCollection(userId).whereGreaterThanOrEqualTo("last_synced_at", from)
	...
	private void validateInputs(PortfolioSummaryEntity entity, String userId) {
		if (entity == null) {
			throw new ProviderIntegrationException(DataProviderErrorDetails.INVALID_ARGUMENT, "PortfolioSummaryEntity",
