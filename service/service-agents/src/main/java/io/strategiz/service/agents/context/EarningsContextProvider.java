 ...
 * Combines Finnhub earnings calendar with historical fundamentals data
 */
@Component
public class EarningsContextProvider {
    ...
    private void appendUpcomingEarningsCalendar(StringBuilder context, List<String> filterSymbols) {
        if (!earningsClient.isConfigured()) {
            context.append("UPCOMING EARNINGS: [Earnings API not configured - provide API key for real-time calendar]\n\n");
            return;
        }

        try {
            List<EarningsCalendarEvent> upcoming;
    ...
    private void appendSymbolUpcomingEarnings(StringBuilder context, String symbol) {
        ...
        try {
            List<EarningsCalendarEvent> upcoming = earningsClient.getEarningsForSymbols(
                    List.of(symbol), 60); // Look 60 days ahead for specific symbol
    ...
    private void appendSymbolFundamentals(StringBuilder context, String symbol) {
        ...
        try {
            ...
            if (fundamentals != null) {
                ...
                context.append("  • EPS Growth YoY: ").append(formatPercent(fundamentals.getEpsGrowthYoy())).append("\n");
                ...
                if (fundamentals.getRevenueGrowthYoy() != null) {
                    context.append("  • Revenue Growth YoY: ").append(formatPercent(fundamentals.getRevenueGrowthYoy())).append("\n");
                }
                if (fundamentals.getProfitMargin() != null) {
                    context.append("  • Profit Margin: ").append(formatPercent(fundamentals.getProfitMargin())).append("\n");
                }
    ...
    private void appendSymbolRecentEarnings(StringBuilder context, String symbol) {
        ...
        try {
            ...
            List<EarningsCalendarEvent> recent = earningsClient.getRecentEarnings(90).stream()
            ...
            if (!recent.isEmpty()) {
                ...
                context.append(String.format("Beat Rate: %d/%d (%.0f%%)\n\n",
                        beats, recent.size(), (beats * 100.0 / recent.size())));
            }
        } catch (Exception e) {
