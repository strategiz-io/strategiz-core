...
/**
 * Base service for all authentication-related services. Provides common auth patterns
 * following clean architecture.
 *
 * Services should return data objects for success cases and throw StrategizException for
 * errors.
 */
public abstract class AuthBaseService {
	...
	protected boolean isCodeExpired(Date timestamp, int expiryMinutes) {
		...
		if (expired) {
			authLog.debug("Code expired for method: {}, timestamp: {}, expiry: {} minutes", getAuthMethodId(),
					timestamp, expiryMinutes);
		}
	...
	protected void logAuthAttempt(String userId, String action, boolean success) {
		if (success) {
			authLog.info("AUTH_SUCCESS - User: {}, Method: {}, Action: {}", userId, getAuthMethodId(), action);
		}
		else {
			authLog.warn("AUTH_FAILURE - User: {}, Method: {}, Action: {}", userId, getAuthMethodId(), action);
		}
	}
	...
	protected void logSecurityEvent(String userId, String event, String details) {
		authLog.warn("SECURITY_EVENT - User: {}, Method: {}, Event: {}, Details: {}", userId, getAuthMethodId(), event,
				details);
	}
	...
	/**
	 * Get the authentication method ID for this service Each auth service must implement
	 * this
	 */
	protected abstract String getAuthMethodId();

	/**
	 * Get the authentication method display name Each auth service must implement this
	 */
