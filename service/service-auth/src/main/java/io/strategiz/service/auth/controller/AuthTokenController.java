...
/**
 * Controller for one-time authentication tokens. Used for cross-app SSO (token relay
 * pattern).
 *
 * Endpoints: - POST /v1/auth/token/generate - Generate a one-time token (requires auth) -
...
public class AuthTokenController extends BaseController {
	...
	/**
	 * Generate a one-time auth token for cross-app SSO. Manually extracts and validates
	 * authentication from cookies/headers to work around cookie domain restrictions
	 * across subdomains.
	...
	public ResponseEntity<Map<String, Object>> generateToken(@RequestParam("redirect") String redirectUrl,
			HttpServletRequest request) {

		String clientIp = getClientIp(request);

		// Manually extract user from security context (populated by
		// PasetoAuthenticationFilter)
		io.strategiz.framework.authorization.context.AuthenticatedUser user = io.strategiz.framework.authorization.context.SecurityContextHolder
		...
		try {
			...
			return ResponseEntity.ok(Map.of("success", true, "token", token, "redirectUrl", fullRedirectUrl));
		}
		catch (Exception e) {
	...
	/**
	 * Exchange a one-time token for a session. Does not require authentication (token is
	 * the auth).
	...
	public ResponseEntity<Map<String, Object>> exchangeToken(@RequestBody Map<String, String> body,
		...
		try {
			...
			return ResponseEntity.ok(Map.of("success", true, "userId", result.userId(), "email",
					result.email() != null ? result.email() : "", "name", result.name() != null ? result.name() : "",
					"role", result.role() != null ? result.role() : "", "accessToken", result.accessToken(),
