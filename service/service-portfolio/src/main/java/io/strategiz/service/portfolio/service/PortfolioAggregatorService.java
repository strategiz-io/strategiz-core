...
/**
 * Service for aggregating portfolio data across all connected providers. Single
 * Responsibility: Aggregates data from multiple providers into unified views.
 *
 * Data structure: users/{userId}/portfolio/summary ← Aggregated totals
...
public class PortfolioAggregatorService extends BaseService {
	...
	@Autowired
	public PortfolioAggregatorService(PortfolioProviderRepository portfolioProviderRepository,
			ProviderHoldingsRepository providerHoldingsRepository, PortfolioProviderService portfolioProviderService,
	...
	public PortfolioSummaryResponse getPortfolioSummary(String userId) {
		...
		try {
			...
			log.info("Portfolio summary for user {}: total value: {}, providers: {}", userId, totalValue,
					connectedProviders);
	...
	public PortfolioOverviewResponse getPortfolioOverview(String userId) {
		...
		try {
			...
			if (readPortfolioSummaryRepository != null) {
				try {
					...
					if (summary != null) {
						...
						totalCashBalance = summary.getCashAvailable() != null ? summary.getCashAvailable()
								: BigDecimal.ZERO;
						usedPrecomputedTotals = true;
						log.debug("Using pre-computed totals from portfolio/summary for user {}: totalValue={}", userId,
								totalValue);

						// Read pre-computed category allocations for pie chart
						if (summary.getCategoryAllocations() != null) {
							for (PortfolioSummaryEntity.CategoryAllocationData data : summary
							...
							log.debug("Loaded {} category allocations from portfolio/summary",
									categoryAllocations.size());
						}
					}
				}
				catch (Exception e) {
					log.warn("Could not read portfolio/summary for user {}, will compute totals: {}", userId,
							e.getMessage());
				}
			}
			...
			for (PortfolioProviderEntity provider : providers) {
				...
				providerSummary.setProviderName(provider.getProviderName() != null ? provider.getProviderName()
						: getProviderDisplayName(provider.getProviderId()));

				String resolvedProviderType = provider.getProviderType() != null ? provider.getProviderType()
						: getProviderType(provider.getProviderId());
				...
				log.debug("Provider {}: type={}, category={}", provider.getProviderId(), resolvedProviderType,
			...
			PortfolioOverviewResponse.AssetAllocation allocation = calculateAssetAllocation(allPositions, totalValue,
					totalCashBalance);
			...
			log.info("Portfolio overview for user {}: {} providers, {} positions, total value: {}", userId,
					providerSummaries.size(), allPositions.size(), totalValue);
	...
	private BigDecimal calculatePercentChange(BigDecimal change, BigDecimal total) {
		...
		return change.divide(previousValue, 4, RoundingMode.HALF_UP).multiply(new BigDecimal("100"));
	}
	...
	private BigDecimal calculateProfitLossPercent(BigDecimal profitLoss, BigDecimal totalValue) {
		...
		return profitLoss.divide(costBasis, 4, RoundingMode.HALF_UP).multiply(new BigDecimal("100"));
	}
	...
	private String getProviderType(String providerId) {
		Map<String, String> providerTypes = Map.of(ServicePortfolioConstants.PROVIDER_KRAKEN, "crypto",
				ServicePortfolioConstants.PROVIDER_COINBASE, "crypto", ServicePortfolioConstants.PROVIDER_BINANCE,
				"crypto", ServicePortfolioConstants.PROVIDER_ALPACA, "equity",
				ServicePortfolioConstants.PROVIDER_SCHWAB, "equity", ServicePortfolioConstants.PROVIDER_ROBINHOOD,
				"equity");
	...
	private String getProviderCategory(String providerId) {
		Map<String, String> providerCategories = Map.of(ServicePortfolioConstants.PROVIDER_KRAKEN, "exchange",
				ServicePortfolioConstants.PROVIDER_COINBASE, "exchange", ServicePortfolioConstants.PROVIDER_BINANCE,
				"exchange", ServicePortfolioConstants.PROVIDER_ALPACA, "brokerage",
				ServicePortfolioConstants.PROVIDER_SCHWAB, "brokerage", ServicePortfolioConstants.PROVIDER_ROBINHOOD,
				"brokerage");
		return providerCategories.getOrDefault(providerId, "exchange");
	}

	/**
	 * Calculate category-based asset allocation for pie chart visualization. Groups
	 * positions by asset category (crypto, stocks, bonds, cash, etc.)
	 ...
	 */
	public List<CategoryAllocation> calculateCategoryAllocation(List<PortfolioPositionResponse> positions,
			BigDecimal totalValue, BigDecimal cashBalance) {

		log.debug("Calculating category allocation for {} positions, total value: {}", positions.size(), totalValue);
		...
		for (CategoryAllocation allocation : allocations) {
			log.debug("  {}: ${} ({}%)", allocation.getCategoryName(), allocation.getValue(),
