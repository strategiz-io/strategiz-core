...
/**
 * Component responsible for initializing and storing Kraken portfolio data. Handles
 * fetching data from Kraken API and persisting to Firestore.
 *
...
public class KrakenDataInitializer {
	...
	@Autowired
	public KrakenDataInitializer(KrakenApiAuthClient krakenClient, KrakenDataTransformer dataTransformer,
	...
	/**
	 * Initialize and store provider data for a user. Fetches account balance, trade
	 * history, and current prices, then transforms and stores the data.
	...
	public ProviderHoldingsEntity initializeAndStoreData(String userId, String apiKey, String apiSecret, String otp) {
		...
		try {
			...
			log.debug("Fetched balance data for user: {}, assets count: {}", userId,
					balanceResponse.get("result") != null ? ((Map) balanceResponse.get("result")).size() : 0);
			...
			ProviderHoldingsEntity holdingsEntity = dataTransformer.transformEnrichedData(userId, enrichedData,
					tradesResponse);
			...
			log.info("Successfully initialized and stored Kraken data for user: {}, total value: {}", userId,
					holdingsEntity.getTotalValue());
			...
			throw new StrategizException(KrakenProviderErrorDetails.DATA_INITIALIZATION_FAILED, MODULE_NAME, userId,
					e.getMessage());
		}
	}
	...
	private void updateProviderSummary(String userId, ProviderHoldingsEntity holdings) {
		try {
			java.util.Optional<PortfolioProviderEntity> providerOpt = portfolioProviderRepository
				.findByUserIdAndProviderId(userId, KrakenConstants.PROVIDER_ID);
	...
	private Map<String, Object> fetchAccountBalance(String apiKey, String apiSecret, String otp) {
		...
		try {
			...
			if (response == null) {
				throw new StrategizException(KrakenProviderErrorDetails.BALANCE_FETCH_FAILED, MODULE_NAME,
						"null response");
			}
			...
			if (response.containsKey("error") && response.get("error") instanceof List) {
				...
				if (!errors.isEmpty()) {
					...
					if (errorMsg.contains(KrakenConstants.ERROR_INVALID_KEY)) {
						throw new StrategizException(KrakenProviderErrorDetails.INVALID_CREDENTIALS, MODULE_NAME,
								errorMsg);
					}
					else if (errorMsg.contains(KrakenConstants.ERROR_RATE_LIMIT)) {
						throw new StrategizException(KrakenProviderErrorDetails.RATE_LIMIT_EXCEEDED, MODULE_NAME,
								errorMsg);
					}
					else {
						throw new StrategizException(KrakenProviderErrorDetails.BALANCE_FETCH_FAILED, MODULE_NAME,
			...
			throw new StrategizException(KrakenProviderErrorDetails.BALANCE_FETCH_FAILED, MODULE_NAME, e.getMessage());
		}
	}
			...
			if (balanceResponse.containsKey("result")) {
						...
						try {
							...
							if (balance.compareTo(BigDecimal.ZERO) > 0) {
								...
								// Handle regular assets (ATOM, ADA, DOT, etc.)
									...
									// For regular crypto assets, Kraken uses simple
									// format: ATOMUSD, ADAUSD, etc.
								...
								log.debug("Asset {} will query pairs: {}", asset, pairsToQuery
									.subList(Math.max(0, pairsToQuery.size() - 3), pairsToQuery.size()));
							}
						}
				...
				if (!pairsToQuery.isEmpty() && fmpPriceService != null) {
					...
					for (String asset : assetToPairMap.keySet()) {
						...
						String normalizedSymbol = asset.replace("XXBT", "BTC")
							.replace("XETH", "ETH")
							.replace("XXRP", "XRP")
							.replace("XLTC", "LTC")
							.replace("XXLM", "XLM")
						...
						if (fmpPrices != null && !fmpPrices.isEmpty()) {
							...
							for (String asset : assetToPairMap.keySet()) {
								...
								if (fmpPrice != null && fmpPrice > 0) {
									...
									log.info("Got price for {} (normalized: {}) from FMP: ${}", asset, normalizedSymbol,
											price);
								}
								else {
									log.warn("Could not find price for asset {} (normalized: {}) in FMP", asset,
											normalizedSymbol);
								}
							}
						}
							...
							log.error(
									"Failed to get prices from FMP - response was empty, using fallback static prices");
	...
	/**
	 * Get fallback static prices when external APIs fail. These are approximate market
	 * prices to ensure portfolio displays values.
	 */
