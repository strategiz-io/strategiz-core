 ...
 *
 * Endpoints: - POST /v1/portfolio/refresh/on-signin - Refresh after user authentication -
 * POST /v1/portfolio/refresh - Manual refresh (button click or auto-refresh) - GET
 * /v1/portfolio/refresh/status - Get last refresh times
 *
 * Also maintains backward compatibility with old endpoints: - POST
 * /v1/dashboard/sync/on-signin - POST /v1/dashboard/sync/refresh - GET
 * /v1/dashboard/sync/status
 */
@RestController
@CrossOrigin(origins = "*")
public class PortfolioRefreshController extends BaseController {
	...
	/**
	 * Refresh portfolio on user sign-in. Called immediately after authentication to load
	 * initial data.
	 *
	 * New endpoint: POST /v1/portfolio/refresh/on-signin Legacy endpoint: POST
	 * /v1/dashboard/sync/on-signin (backward compatible)
	 */
		...
		try {
			...
			if (authHeader == null || !authHeader.startsWith("Bearer ")) {
				...
				return ResponseEntity.status(401)
					.body(Map.of("error", "No valid authentication token", "timestamp", Instant.now().toString()));
			}
			...
			return ResponseEntity
				.ok(Map.of("status", "success", "userId", userId, "demoMode", demoMode != null ? demoMode : false,
						"refreshResult", result, "timestamp", Instant.now().toString()));
			...
			throw new StrategizException(ServiceDashboardErrorDetails.SYNC_INITIALIZATION_FAILED, MODULE_NAME,
					userId != null ? userId : "unknown", e.getMessage());
		}
	}

	/**
	 * Manual portfolio refresh. Called when user clicks refresh button or auto-refresh
	 * interval triggers.
	 *
	 * New endpoint: POST /v1/portfolio/refresh Legacy endpoint: POST
	 * /v1/dashboard/sync/refresh (backward compatible)
	 */
	...
	public ResponseEntity<Map<String, Object>> refreshPortfolio(HttpServletRequest request) {
		...
		try {
			String authHeader = request.getHeader("Authorization");
			if (authHeader == null || !authHeader.startsWith("Bearer ")) {
				return ResponseEntity.status(401).body(Map.of("error", "No valid authentication token"));
			}
			...
			if (Boolean.TRUE.equals(demoMode)) {
				return ResponseEntity.ok(Map.of("status", "skipped", "reason", "Demo mode active", "timestamp",
			...
			return ResponseEntity
				.ok(Map.of("status", "success", "refreshResult", result, "timestamp", Instant.now().toString()));
			...
			throw new StrategizException(ServiceDashboardErrorDetails.DASHBOARD_ERROR, MODULE_NAME, "refresh",
					e.getMessage());
		}
	}
	 ...
	 *
	 * New endpoint: GET /v1/portfolio/refresh/status Legacy endpoint: GET
	 * /v1/dashboard/sync/status (backward compatible)
	 */
