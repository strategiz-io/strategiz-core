 ...
 *
 * ULTRA SIMPLE USAGE: 1. Extend this class 2. Use save(entity, userId) for create/update
 * 3. Use findById(id) for queries 4. Use delete(id, userId) for soft deletes
 *
...
public abstract class BaseRepository<T extends BaseEntity> {
	...
	/**
	 * Get the module name for this repository. Each repository must implement this to
	 * identify its module.
	 *
	...
	public T save(T entity, String userId) {
		try {
			...
			throw new DataRepositoryException(DataRepositoryErrorDetails.ENTITY_SAVE_FAILED, getModuleName(), e,
	...
	/**
	 * Force create an entity, even if it already has an ID set. Use this when the entity
	 * ID is pre-assigned (e.g., UserEntity where getId() returns userId). This bypasses
	 * the auto-detection logic in save().
	...
	public T getById(String id) {
		return findById(id).orElseThrow(() -> new DataRepositoryException(DataRepositoryErrorDetails.ENTITY_NOT_FOUND,
	...
	public List<T> saveAll(List<T> entities, String userId) {
		try {
			...
			throw new DataRepositoryException(DataRepositoryErrorDetails.BULK_OPERATION_FAILED, e,
					entityClass.getSimpleName());
		}
	}
	...
	protected List<T> findByField(String fieldName, Object value) {
		try {
			Query query = getCollection().whereEqualTo(fieldName, value).whereEqualTo("isActive", true);
	...
	private T performCreate(T entity, String userId) throws InterruptedException, ExecutionException {
		...
		if (transaction != null) {
			...
			log.debug("Created {} with ID {} in transaction by user {}", entityClass.getSimpleName(), entity.getId(),
					userId);
		}
		else {
	...
	private T performUpdate(T entity, String userId) throws InterruptedException, ExecutionException {
		...
		if (transaction != null) {
			// Use transaction for atomic operation
			transaction.set(docRef, entity);
			log.debug("Updated {} with ID {} in transaction by user {}", entityClass.getSimpleName(), entity.getId(),
					userId);
		}
		else {
	...
	private void validateInputs(T entity, String userId) {
		if (entity == null) {
			throw new DataRepositoryException(DataRepositoryErrorDetails.INVALID_ARGUMENT, entityClass.getSimpleName(),
	...
	protected CollectionReference getCollection() {
		...
		if (annotation == null) {
			throw new DataRepositoryException(DataRepositoryErrorDetails.COLLECTION_ANNOTATION_MISSING,
					entityClass.getSimpleName());
		}
