 ...
 *
 * Provides mocked AI strategy service to enable BDD tests to run
 * without requiring real AI service credentials or external API calls.
 *
 ...
 *
 * NOTE: These are UNIT tests with mocked AI - they verify test structure
 * and constant extraction logic. For INTEGRATION tests with real AI,
 * use a separate test suite with proper credentials.
 */
...
public class BDDTestConfiguration {
     ...
     *
     * This mock parses the prompt and generates Python code demonstrating the expected format.
     */
    @Bean
    @Primary
    public AIStrategyService mockAIStrategyService() {
        return new AIStrategyService(null) {
            @Override
            ...
            private String extractSymbol(String prompt) {
                ...
                // Check in order of appearance in prompt (first mention wins for multi-symbol prompts)
                if (lower.contains("qqq") && lower.indexOf("qqq") < lower.indexOf("spy")) return "QQQ";
                if (lower.contains("aapl") || lower.contains("apple")) return "AAPL";
                if (lower.contains("msft") || lower.contains("microsoft")) return "MSFT";
                if (lower.contains("googl") || lower.contains("google")) return "GOOGL";
                if (lower.contains("tsla") || lower.contains("tesla")) return "TSLA";
                if (lower.contains("nvda")) return "NVDA";
                if (lower.contains("amzn") || lower.contains("amazon")) return "AMZN";
                if (lower.contains("spy")) return "SPY";
                if (lower.contains("qqq")) return "QQQ";
