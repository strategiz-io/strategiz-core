...
/**
 * Repository for managing account recovery requests.
 * Recovery requests are stored in the recovery_requests top-level collection.
 *
 * <p>Key operations:</p>
 * <ul>
...
public interface RecoveryRequestRepository {
    ...
    /**
     * Find active recovery requests for a user.
     * Active = not expired, not completed, not cancelled.
     *
    ...
    /**
     * Count recovery requests for an email in the last N hours.
    ...
    /**
     * Count recovery requests from an IP in the last N hours.
     * Used for rate limiting.
     *
     ...
     * Called periodically to clean up old requests.
     *
     ...
     * Called when user successfully logs in or completes recovery.
     *
     * @param userId the user ID
