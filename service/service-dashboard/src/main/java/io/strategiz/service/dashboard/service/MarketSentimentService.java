...
/**
 * Service for market sentiment analysis within the Dashboard module. This service
 * provides fear & greed index, trending assets, and sentiment indicators.
 */
@Service
public class MarketSentimentService extends BaseService {
	...
	@Autowired
	public MarketSentimentService(PortfolioManager portfolioManager
	...
	public MarketSentimentResponse getMarketSentiment(String userId) {
		...
		try {
			...
			throw new StrategizException(ServiceDashboardErrorDetails.SENTIMENT_CALCULATION_FAILED, "service-dashboard",
					e, userId);
		}
	}
	...
	private MarketSentimentResponse.SentimentIndicator calculateOverallSentiment() {
		...
		try {
			// In a real implementation, we would fetch sentiment data from an external
			// API
			// For now, we'll use the CoinGecko client to get market data

			// TODO: Replace with real sentiment API call
			// This is a placeholder implementation that generates a score between 0 and
			// 100
	...
	private MarketSentimentResponse.MarketTrends calculateMarketTrends() {
		...
		try {
			...
			BigDecimal neutralTrendPercentage = new BigDecimal("100").subtract(uptrendPercentage)
	...
	private BigDecimal fetchOverallMarketSentiment() {
		...
		try {
			...
			/*
			 * Map<String, Object> marketData = coinGeckoClient.getGlobalMarketData(); if
			 * (marketData != null &&
			 * marketData.containsKey("market_cap_change_percentage_24h_usd")) { Double
			 * marketCapChange = (Double)
			 * marketData.get("market_cap_change_percentage_24h_usd");
			 *
			 * // Convert market cap change to a sentiment score (0-100) // If market cap
			 * is up by 5% or more, sentiment is very bullish (80-100) // If market cap is
			 * down by 5% or more, sentiment is very bearish (0-20) if (marketCapChange !=
			 * null) { double normalizedChange = (marketCapChange + 5.0) / 10.0; //
			 * Normalize to 0-1 range normalizedChange = Math.max(0.0, Math.min(1.0,
			 * normalizedChange)); // Clamp to 0-1 double sentimentValue =
			 * normalizedChange * 100.0; return new
			 * BigDecimal(String.valueOf(sentimentValue)).setScale(2,
	...
	private BigDecimal fetchAssetSentiment(String symbol) {
		...
		try {
			...
			sentimentValue = Math.max(0.0, Math.min(100.0, sentimentValue)); // Clamp to
																				// 0-100
