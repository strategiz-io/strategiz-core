...
/**
 * Service for News Sentinel agent - real-time news and sentiment analysis. Uses FMP API
 * for stock news, market news, and press releases. Uses FMP API for SEC filings and
...
public class NewsSentinelService extends BaseService {
		...
		try {
			...
			for (FmpNewsArticle article : articles) {
				...
				dto.setSummary(article.getText() != null && article.getText().length() > 200
						? article.getText().substring(0, 200) + "..." : article.getText());
				...
				if (article.getTitle() != null) {
					...
					else if (titleLower.contains("drop") || titleLower.contains("fall")
							|| titleLower.contains("decline") || titleLower.contains("crash")
							|| titleLower.contains("bearish")) {
	...
	/**
	 * Extract stock symbol from user message Simple extraction - could be enhanced with
	 * NLP
	 */
	...
	private boolean isCommonWord(String word) {
		...
		return switch (word) {
			case "A", "I", "THE", "FOR", "AND", "OR", "IS", "IT", "TO", "OF", "IN", "ON", "AT", "NEWS", "SEC", "ANY",
					"ALL", "NEW", "NOW", "UP", "DOWN", "GO", "BE", "DO", "HAS", "HAD", "WAS", "ARE", "CAN", "MAY",
					"GET", "PUT", "CALL", "BUY", "SELL" ->
				true;
			default -> false;
		};
	}
