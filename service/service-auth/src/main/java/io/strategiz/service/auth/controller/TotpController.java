...
public class TotpController {
    ...
    public ResponseEntity<ApiResponse<TotpSetupResponse>> setupTotp(@RequestBody TotpSetupRequest request) {
        ...
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success("TOTP setup initiated successfully", response));
    ...
    public ResponseEntity<ApiResponse<Void>> verifyTotp(@RequestBody TotpVerifyRequest request) {
        ...
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success("TOTP setup completed successfully"));
    ...
    public ResponseEntity<ApiResponse<TotpAuthResponse>> authenticateWithTotp(
        ...
        try {
            ...
            // Optional<TokenService.TokenPair> tokenPairOpt = totpService.authenticateWithTotp(request.userId(), request.code());
            
            // Simulation for migration purposes - replace with actual implementation later
            boolean isAuthenticated = totpService.verifyUserTotp(request.userId(), request.code());
            ...
            TotpAuthResponse response = TotpAuthResponse.success(accessToken, refreshToken);
