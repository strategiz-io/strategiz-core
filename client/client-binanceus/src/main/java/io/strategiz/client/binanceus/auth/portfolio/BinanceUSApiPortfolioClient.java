package io.strategiz.client.binanceus.auth.portfolio;
...
/**
 * Client for fetching portfolio data from Binance US API
 */
@Service
public class BinanceUSApiPortfolioClient {

	private static final Logger log = LoggerFactory.getLogger(BinanceUSApiPortfolioClient.class);

	private final BinanceUSApiAuthClient apiAuthClient;

	private final BinanceUSCredentialManager credentialManager;

	public BinanceUSApiPortfolioClient(BinanceUSApiAuthClient apiAuthClient,
	...
	public Map<String, Object> getPortfolio(String userId) {
		try {
			...
			BinanceUSApiCredentials credentials = credentialManager.getCredentials(userId);
			if (credentials == null) {
			...
			// Fetch account data
			Map<String, Object> accountData = apiAuthClient
				.getAccountInfo(credentials.getApiKey(), credentials.getApiSecret())
				.block();
	...
	public List<Map<String, Object>> getBalances(String userId) {
		try {
			...
			Map<String, Object> balanceData = apiAuthClient
				.getAccountBalances(credentials.getApiKey(), credentials.getApiSecret())
				.block();
	...
	public List<Map<String, Object>> getOpenOrders(String userId, String symbol) {
		try {
			...
			// Fetch open orders
			Map<String, Object> orderData = apiAuthClient
				.getOpenOrders(credentials.getApiKey(), credentials.getApiSecret(), symbol)
				.block();
	...
	public List<Map<String, Object>> getTradeHistory(String userId, String symbol, Integer limit) {
		try {
			...
			// Fetch trade history
			Map<String, Object> tradeData = apiAuthClient
				.getTradeHistory(credentials.getApiKey(), credentials.getApiSecret(), symbol, limit)
				.block();
	...
	/**
	 * Calculate total portfolio value in USD Note: This is a simplified calculation - in
	 * production you'd need current prices from market data
