...
public class ProviderStatusService extends BaseService {
	...
	private static final Map<String, ProviderConfig> KNOWN_PROVIDERS = Map.of("alpaca",
			new ProviderConfig("alpaca", "Alpaca Markets", "STOCK"), "schwab",
			new ProviderConfig("schwab", "Charles Schwab", "STOCK"), "robinhood",
			new ProviderConfig("robinhood", "Robinhood", "STOCK"), "coinbase",
			new ProviderConfig("coinbase", "Coinbase", "CRYPTO"), "kraken",
			new ProviderConfig("kraken", "Kraken", "CRYPTO"), "binanceus",
			new ProviderConfig("binanceus", "Binance US", "CRYPTO"), "google",
			new ProviderConfig("google", "Google OAuth", "OAUTH"), "facebook",
			new ProviderConfig("facebook", "Facebook OAuth", "OAUTH"));
	...
	public List<ProviderStatusResponse> listProviders() {
		...
		for (Map.Entry<String, ProviderConfig> entry : KNOWN_PROVIDERS.entrySet()) {
			...
			ProviderStatusResponse response = new ProviderStatusResponse(config.name, config.displayName, config.type);
			response.setActiveIntegrations(0); // TODO: Add cross-user integration
												// counting
