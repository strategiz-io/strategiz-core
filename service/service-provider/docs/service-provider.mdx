...
**Documentation**:
- [📄 Provider Connection API](provider-connection-api.mdx) ✅ Complete
...
- **`strategiz-ui/SignUpForm.tsx`** → User clicks "Connect Provider" (Step 3)

### 2. Service Layer (API Entry)
- **`service-provider/CreateProviderController`** → REST endpoint `/v1/providers/connect`
- Orchestrates the entire provider connection flow

### 3. Business Orchestration
- **`business-provider-kraken/ProviderConnectionService`** → Provider-specific business logic
- Coordinates connection workflow and error handling

### 4. External API Integration (Raw Data)
- **`client-kraken/KrakenApiPortfolioClient`** → Calls Kraken REST API
...
- **`business-provider-kraken/KrakenPortfolioEnhancerService`** → Takes raw data from step 4
- Calls **`business-portfolio/MarketPriceBusiness`** for price enrichment
- **`client-yahoofinance/YahooFinanceClient`** → Fetches real-time market prices
...
- **`service-portfolio/KrakenPortfolioService`** → Calculates profit/loss metrics
- Integrates enriched data and prepares final response

### 7. Data Persistence
- **`data-provider/ProviderDataBaseRepository`** → Stores enriched data to Firestore
- Collection: `users/{userId}/provider_data/{providerId}`
- Document ID = providerId (e.g., "kraken")

### 8. Credentials Storage
- **`client-vault`** → Stores OAuth tokens and API credentials securely in HashiCorp Vault
