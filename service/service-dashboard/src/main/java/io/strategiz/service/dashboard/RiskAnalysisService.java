...
import io.strategiz.service.dashboard.model.RiskAnalysisData;
...
public class RiskAnalysisService extends BaseService {
    ...
    public RiskAnalysisData getRiskAnalysis(String userId) {
        ...
        try {
            ...
            // Get historical data for volatility calculations - in a real implementation this would come from a database
     ...
     * @return Risk analysis data
     */
    private RiskAnalysisData calculateRiskMetrics(
    ...
    private RiskAnalysisData.VolatilityMetric calculateVolatilityMetrics(List<PortfolioData> historicalData) {
        ...
        BigDecimal standardDeviation = new BigDecimal(String.valueOf(randomizedVolatility))
        ...
        BigDecimal maxDrawdown = new BigDecimal(String.valueOf(drawdown))
    ...
    private RiskAnalysisData.DiversificationMetric calculateDiversificationMetrics(PortfolioData portfolioData) {
        ...
        try {
            ...
            if (largestValue.compareTo(BigDecimal.ZERO) > 0) {
                BigDecimal largestPercentage = largestValue
    ...
    private RiskAnalysisData.CorrelationMetric calculateCorrelationMetrics(
        ...
        if (portfolioData != null && portfolioData.getExchanges() != null && !portfolioData.getExchanges().isEmpty()) {
            ...
            for (PortfolioData.ExchangeData exchangeData : portfolioData.getExchanges().values()) {
                ...
                if (exchangeName.contains("coinbase") || 
                    exchangeName.contains("binance") || 
                    exchangeName.contains("kraken")) {
                    hasCrypto = true;
                }
                
                // Check if this is a stock exchange
                if (exchangeName.contains("robinhood") || 
                    exchangeName.contains("etrade") || 
                    exchangeName.contains("schwab") ||
                    exchangeName.contains("fidelity")) {
                    hasStocks = true;
                }
            }
