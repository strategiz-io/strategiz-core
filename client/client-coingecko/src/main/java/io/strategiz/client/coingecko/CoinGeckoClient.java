...
/**
 * Client for interacting with the CoinGecko API. This class handles all low-level API
 * communication with CoinGecko following Synapse patterns.
 */
@Component
public class CoinGeckoClient {
	...
	public List<CryptoCurrency> getCryptocurrencyMarketData(List<String> coinIds, String currency) {
		...
		try {
			...
			ResponseEntity<List<CryptoCurrency>> response = restTemplate.exchange(builder.toUriString(), HttpMethod.GET,
					entity, new ParameterizedTypeReference<List<CryptoCurrency>>() {
					});
	 ...
	 * @return Map containing global market data like total market cap, trading volume,
	 * etc.
	 */
	...
	public Map<String, Object> getGlobalMarketData() {
		...
		try {
			...
			ResponseEntity<Map<String, Object>> response = restTemplate.exchange(builder.toUriString(), HttpMethod.GET,
