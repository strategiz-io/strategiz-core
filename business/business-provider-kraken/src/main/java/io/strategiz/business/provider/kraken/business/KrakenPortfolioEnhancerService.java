...
/**
 * Service responsible for enhancing Kraken portfolio data retrieved from storage.
 * Applies symbol normalization and data transformations to ensure consistent display.
 * 
...
public class KrakenPortfolioEnhancerService {
    ...
    /**
     * Enhance portfolio data retrieved from Firebase storage.
     * This method ensures that raw Kraken symbols are normalized to standard crypto symbols.
     * 
    ...
    public ProviderDataEntity enhancePortfolioData(ProviderDataEntity data) {
        ...
        if (data.getHoldings() != null && !data.getHoldings().isEmpty()) {
            List<ProviderDataEntity.Holding> enhancedHoldings = data.getHoldings().stream()
