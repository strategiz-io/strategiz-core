...
/**
 * Controller for seeding and managing symbols in Firestore. Provides endpoints to migrate
 * static symbol mappings to Firestore.
 */
@RestController
...
public class SymbolSeedController {
	...
	/**
	 * Seed all symbols (crypto + stocks + ETFs) into Firestore. This is a one-time
	 * migration endpoint.
	 */
	@PostMapping("/seed")
	...
	private List<SymbolEntity> createCryptoSymbols() {
		...
		symbols.add(createCrypto("BTC", "Bitcoin", "payment", "The first and largest cryptocurrency", 8, Map.of("YAHOO",
				"BTC-USD", "ALPACA", "BTC/USD", "COINBASE", "BTC-USD", "BINANCE", "BTCUSDT", "KRAKEN", "XXBTZUSD")));
		symbols.add(createCrypto("ETH", "Ethereum", "platform", "Smart contract platform", 18, Map.of("YAHOO",
	...
	private List<SymbolEntity> createEtfSymbols() {
		...
		symbols
			.add(createEtf("ITOT", "iShares Core S&P Total U.S. Stock Market ETF", "index", "Total US stock market"));
		...
		symbols.add(createEtf("XLY", "Consumer Discretionary Select Sector SPDR", "sector",
		...
		symbols
			.add(createEtf("VEA", "Vanguard FTSE Developed Markets ETF", "international", "Developed markets ex-US"));
	...
	private SymbolEntity createCrypto(String symbol, String name, String category, String description, int decimals,
		...
		entity.setPrimaryDataSource("ALPACA"); // Use Alpaca for crypto (supports
												// /v1beta3/crypto/us/bars)
		entity.setTimeframes(List.of("1m", "30m", "1h", "4h", "1D")); // Crypto supports
																		// same timeframes
																		// as stocks
