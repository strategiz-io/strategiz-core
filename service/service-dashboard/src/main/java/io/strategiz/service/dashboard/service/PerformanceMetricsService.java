...
/**
 * Service for portfolio performance metrics operations. This service provides historical
 * performance data and portfolio metrics.
 */
@Service
public class PerformanceMetricsService extends BaseService {
	...
	public PerformanceMetricsData getPerformanceMetrics(String userId) {
		...
		try {
			...
			// Get historical portfolio values - in a real implementation this would come
			// from a database
			...
			throw new StrategizException(ServiceDashboardErrorDetails.METRICS_CALCULATION_FAILED, "service-dashboard",
					e, userId);
		}
	}
	 ...
	 * @return Performance metrics response
	 */
	private PerformanceMetricsData calculatePerformanceMetrics(PortfolioData currentData,
		...
		List<PerformanceMetricsData.PortfolioValueDataPoint> dataPoints = new ArrayList<>();

		// Current time for the latest data point
		LocalDateTime now = LocalDateTime.now(ZoneOffset.UTC);

		// Add current data point
		PerformanceMetricsData.PortfolioValueDataPoint currentPoint = new PerformanceMetricsData.PortfolioValueDataPoint();
		currentPoint.setTimestamp(now);
		currentPoint.setValue(currentData.getTotalValue() != null ? currentData.getTotalValue() : BigDecimal.ZERO);
		...
		if (historicalData != null) {
			for (PortfolioData historicalPoint : historicalData) {
				...
				if (historicalValue != null) {
					PerformanceMetricsData.PortfolioValueDataPoint dataPoint = new PerformanceMetricsData.PortfolioValueDataPoint();
	 ...
	 */
	private PerformanceMetricsData.PerformanceSummary calculateSummary(PortfolioData currentData,
			List<PortfolioData> historicalData) {

		PerformanceMetricsData.PerformanceSummary summary = new PerformanceMetricsData.PerformanceSummary();

		// Get current total value
		BigDecimal currentValue = currentData.getTotalValue() != null ? currentData.getTotalValue() : BigDecimal.ZERO;

		// Initialize with daily change from current data
		summary.setDailyChange(currentData.getDailyChange() != null ? currentData.getDailyChange() : BigDecimal.ZERO);
		summary.setDailyChangePercentage(
				currentData.getDailyChangePercent() != null ? currentData.getDailyChangePercent() : BigDecimal.ZERO);
	...
	/**
	 * Finds the historical data point closest to the target date In a real
	 * implementation, PortfolioData would have timestamp information
	...
	/**
	 * Finds the oldest historical data point In a real implementation, PortfolioData
	 * would have timestamp information
	 * @param historicalData List of historical data points
