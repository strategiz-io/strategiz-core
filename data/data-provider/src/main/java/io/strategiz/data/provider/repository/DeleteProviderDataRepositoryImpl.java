 ...
 * Performs hard delete of provider data since it's cached/synced data that can be re-fetched.
 */
@Repository
public class DeleteProviderDataRepositoryImpl implements DeleteProviderDataRepository {
    ...
    public boolean deleteProviderData(String userId, String providerId) {
        ...
        try {
            ...
            log.error("Failed to delete provider data for userId={}, providerId={}: {}", userId, providerId, e.getMessage(), e);
            return false;
        }
    }
    ...
    public int deleteProviderDataByType(String userId, String accountType) {
        ...
        try {
            ...
            List<QueryDocumentSnapshot> docs = collection
                .whereEqualTo("provider_type", accountType)
            ...
            log.info("Successfully deleted {} provider data documents of type {} for userId={}",
                    deletedCount, accountType, userId);
            return deletedCount;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            log.error("Interrupted while deleting provider data by type for userId={}, accountType={}",
                    userId, accountType, e);
            return 0;
        } catch (ExecutionException e) {
            log.error("Failed to delete provider data by type for userId={}, accountType={}: {}",
                    userId, accountType, e.getMessage(), e);
            return 0;
        }
    }
}
