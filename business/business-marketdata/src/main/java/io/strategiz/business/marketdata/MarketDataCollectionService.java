 ...
 *
 * Features: - Multi-threaded backfill with configurable concurrency (default 2 threads) -
 * Batch saves (500 entities per batch) - Comprehensive field mapping to MarketDataEntity
 * - Symbol metadata enrichment from Assets API - All timeframe support (1m, 30m, 1h, 4h,
 * 1D, 1W, 1M) - Automatic pagination handling - Rate limiting and error recovery
 *
 * Target: All symbols from Firestore SymbolService Storage: TimescaleDB (when enabled) or
 * Firestore
 */
@Service
public class MarketDataCollectionService {
	...
	@Autowired
	public MarketDataCollectionService(AlpacaHistoricalClient historicalClient, AlpacaAssetsClient assetsClient,
			MarketDataRepository marketDataRepository,
			@Autowired(required = false) MarketDataClickHouseRepository clickHouseRepository,
			SymbolService symbolService, @Value("${marketdata.batch.thread-pool-size:2}") int threadPoolSize,
	...
	/**
	 * Get symbols to collect from Firestore SymbolService. Returns provider-formatted
	 * symbols (e.g., "AAPL", "BTC/USD").
	 */
	...
	/**
	 * Backfill historical data for all symbols from Firestore. Uses multi-threading for
	...
	public CollectionResult backfillIntradayData(String timeframe) {
		...
		log.info("Starting market data backfill for {} symbols, timeframe: {}, lookback: {} months", symbols.size(),
				timeframe, backfillMonths);
	 ...
	 * @return Collection result with statistics
	 */
	public CollectionResult backfillIntradayData(List<String> symbols, LocalDateTime startDate, LocalDateTime endDate,
			String timeframe) {
		log.info("Backfilling {} symbols from {} to {} ({})", symbols.size(), startDate, endDate, timeframe);
		...
		try {
			...
			List<CompletableFuture<SymbolResult>> futures = symbols.stream()
				.map(symbol -> CompletableFuture.supplyAsync(() -> {
					try {
						...
						SymbolResult result = processSymbol(symbol, startDate, endDate, timeframe,
			...
			try {
				...
				// Wait for all futures to complete (they already updated counters in the
				// async tasks)
				...
				if (currentJobStatus.isCancelRequested()) {
					...
					log.info("Backfill completed: {} symbols, {} bars stored, {} errors", symbolsProcessed.get(),
							dataPointsStored.get(), errorCount.get());
				}
	...
	/**
	 * Start backfill asynchronously for multiple timeframes (non-blocking). Returns
	...
	public void backfillIntradayDataAsync(List<String> timeframes, LocalDateTime startDate, LocalDateTime endDate) {
		...
		for (String timeframe : timeframes) {
			try {
				...
				log.info("--- Timeframe {} completed: {} symbols, {} bars, {} errors ---", timeframe,
	...
	/**
	 * Smart gap-filling backfill that only fetches missing historical data. For each
	 * symbol, checks existing data range and only fetches: 1. Data before the earliest
	 * existing timestamp (historical gaps)
	 *
	...
	public CollectionResult backfillGaps(LocalDateTime targetStartDate, String timeframe) {
		...
		List<String> symbols = getSymbolsToCollect();
		log.info("Starting gap-fill backfill for {} symbols, target start: {}, timeframe: {}", symbols.size(),
				targetStartDate, timeframe);
		...
		try {
			...
			List<CompletableFuture<SymbolResult>> futures = symbols.stream()
				.map(symbol -> CompletableFuture.supplyAsync(() -> {
					try {
						...
						Optional<Map<String, Instant>> existingRange = clickHouseRepository
							.getDateRangeForSymbolAndTimeframe(symbol, timeframe);
						...
						if (existingRange.isPresent()) {
							...
							// Fetch from target start to earliest existing (fill
							// historical gap)
							fetchStart = targetStartDate;
							fetchEnd = earliestDate.minusDays(1);
							log.info("Filling gap for {}: {} to {} (existing starts at {})", symbol, fetchStart,
									fetchEnd, earliestDate);
						}
						else {
							// No existing data - fetch full range
							fetchStart = targetStartDate;
							fetchEnd = now;
							log.info("No existing data for {} - fetching full range: {} to {}", symbol, fetchStart,
									fetchEnd);
						}
			...
			try {
				...
				currentJobStatus.setCompleted();
				log.info(
						"Gap-fill backfill completed: {} symbols processed, {} skipped (already complete), {} bars stored, {} errors",
	...
	/**
	 * Convert our short timeframe format to Alpaca API format. Alpaca expects: 1Min,
	...
	private String toAlpacaTimeframe(String timeframe) {
		...
		switch (timeframe) {
			...
			case "1Month":
				return "1Month";
			default:
				return timeframe; // Minute-based timeframes pass through
		}
	}

	/**
	 * Determine if a symbol is a cryptocurrency. Crypto symbols in Alpaca format contain
	 * "/USD" (e.g., "BTC/USD"). Also checks SymbolService for asset type.
	 */
	...
	private String toStorageTimeframe(String timeframe) {
		...
		switch (timeframe) {
			...
			case "30m":
				return "30m";
			default:
				return timeframe; // Unknown timeframes pass through
		}
	}

	/**
	 * Process a single symbol - fetch bars and store in batches. Automatically detects
	 * crypto symbols and uses the appropriate API endpoint.
	 */
	private SymbolResult processSymbol(String symbol, LocalDateTime startDate, LocalDateTime endDate, String timeframe,
		...
		try {
			...
			if (isCrypto) {
				...
				log.info(">>> Fetching crypto bars for {} via historicalClient (alpaca timeframe: {})...", cryptoSymbol,
						alpacaTimeframe);
				bars = historicalClient.getCryptoBars(cryptoSymbol, startDate, endDate, alpacaTimeframe);
			}
			else {
				log.info(">>> Fetching stock bars for {} via historicalClient (alpaca timeframe: {})...", symbol,
						alpacaTimeframe);
			...
			log.info(">>> Converting {} bars to entities for {} (storage timeframe: {})", bars.size(), symbol,
					storageTimeframe);
			List<MarketDataEntity> entities = bars.stream()
				.map(bar -> convertBar(symbol, bar, storageTimeframe, assetMetadata))
				.filter(Objects::nonNull)
				.collect(Collectors.toList());
			log.info(">>> Converted to {} entities for {} (filtered out {})", entities.size(), symbol,
					bars.size() - entities.size());
			...
			log.info(">>> Symbol {}: fetched {} bars, converted {}, stored {}", symbol, bars.size(), entities.size(),
					stored);
			...
			throw new StrategizException(MarketDataErrorDetails.SYMBOL_PROCESSING_FAILED, "business-marketdata", e,
					symbol);
		}
	}
	 ...
	 */
	private MarketDataEntity convertBar(String symbol, AlpacaBar bar, String timeframe, AlpacaAsset assetMetadata) {
		try {
	...
	/**
	 * Save entities in batches to storage Batch size configured via
	 * application.properties (default 500)
	 */
	private int saveBatch(List<MarketDataEntity> entities) {
		...
		for (int i = 0; i < entities.size(); i += batchSize) {
			...
			try {
				...
				log.debug("Saved batch of {} entities ({}/{})", batch.size(), totalSaved, entities.size());
	...
	/**
	 * Cancel the currently running backfill job. The job will stop processing new symbols
	 * but will finish the current symbol.
	 * @return true if a job was running and cancellation was requested, false if no job
	 * was running
	 */
	...
	public static class CollectionResult {
		...
		public String toString() {
			return String.format("CollectionResult[symbols=%d, dataPoints=%d, errors=%d]", totalSymbolsProcessed,
					totalDataPointsStored, errorCount);
		}
