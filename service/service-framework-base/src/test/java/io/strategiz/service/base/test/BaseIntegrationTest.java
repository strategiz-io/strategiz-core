 ...
 *
 * <p>
 * These tests use Spring Boot's test framework with MockMvc to invoke actual controller
 * endpoints without starting a full HTTP server.
 * </p>
 *
...
public abstract class BaseIntegrationTest {
	...
	/**
	 * Setup method called before each test. Override in subclasses for test-specific
	 * setup.
	 */
	@BeforeEach
	public void setUp() {
	...
	protected ResultActions performGet(String endpoint) throws Exception {
		...
		return mockMvc.perform(get(endpoint).contentType(MediaType.APPLICATION_JSON)).andDo(print());
	}
	...
	protected ResultActions performGet(String endpoint, Map<String, String> params) throws Exception {
		...
		MockHttpServletRequestBuilder request = get(endpoint).contentType(MediaType.APPLICATION_JSON);
	...
	protected ResultActions performPost(String endpoint, Object requestBody) throws Exception {
		...
		return mockMvc.perform(post(endpoint).contentType(MediaType.APPLICATION_JSON).content(json)).andDo(print());
	}
	...
	protected ResultActions performPost(String endpoint) throws Exception {
		...
		return mockMvc.perform(post(endpoint).contentType(MediaType.APPLICATION_JSON)).andDo(print());
	}
	...
	protected ResultActions performPut(String endpoint, Object requestBody) throws Exception {
		...
		return mockMvc.perform(put(endpoint).contentType(MediaType.APPLICATION_JSON).content(json)).andDo(print());
	}
	...
	protected ResultActions performDelete(String endpoint) throws Exception {
		...
		return mockMvc.perform(delete(endpoint).contentType(MediaType.APPLICATION_JSON)).andDo(print());
	}
	...
	protected ResultActions performPatch(String endpoint, Object requestBody) throws Exception {
		...
		return mockMvc.perform(patch(endpoint).contentType(MediaType.APPLICATION_JSON).content(json)).andDo(print());
	}
	 ...
	 * @throws Exception if request fails
	 */
	protected ResultActions performAuthenticatedPost(String endpoint, String accessToken, Object requestBody)
			throws Exception {
		String json = objectMapper.writeValueAsString(requestBody);
	 ...
	 * @throws Exception if request fails
	 */
	protected ResultActions performAuthenticatedPut(String endpoint, String accessToken, Object requestBody)
			throws Exception {
		String json = objectMapper.writeValueAsString(requestBody);
		log.debug("PUT {} (authenticated) with body: {}", endpoint, json);

		return mockMvc
			.perform(put(endpoint).contentType(MediaType.APPLICATION_JSON)
				.header("Authorization", "Bearer " + accessToken)
				.content(json))
			.andDo(print());
	}
	...
	protected ResultActions performAuthenticatedDelete(String endpoint, String accessToken) throws Exception {
		...
		return mockMvc
			.perform(delete(endpoint).contentType(MediaType.APPLICATION_JSON)
				.header("Authorization", "Bearer " + accessToken))
			.andDo(print());
	}
	 ...
	 * @throws Exception if assertion fails
	 */
	protected ResultActions andExpectField(ResultActions actions, String jsonPath, Object expectedValue)
			throws Exception {
