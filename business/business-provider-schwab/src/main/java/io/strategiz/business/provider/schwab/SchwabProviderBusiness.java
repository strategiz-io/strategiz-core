...
/**
 * Business logic for Charles Schwab provider integration. Handles OAuth flows, API
 * interactions, and business rules specific to Charles Schwab.
 *
 * Key features: - OAuth 2.0 flow with automatic token refresh - Portfolio data fetching
 * with positions - Transaction history for cost basis calculation - Real-time quote
 * fetching via Schwab Market Data API - P&L calculations
 */
@Component
public class SchwabProviderBusiness implements ProviderIntegrationHandler {
	...
	@Autowired
	public SchwabProviderBusiness(SchwabClient schwabClient, PortfolioProviderRepository portfolioProviderRepository,
			ProviderHoldingsRepository providerHoldingsRepository,
			@Qualifier("vaultSecretService") SecretManager secretManager, UniversalSymbolMapper symbolMapper) {
	...
	public SchwabConnectionResult handleOAuthCallback(String userId, String authorizationCode, String state) {
		...
		try {
			...
			result.setExpiresAt(Instant.now().plusSeconds(expiresIn != null ? expiresIn : 1800)); // 30
																									// min
																									// default
			result.setConnectedAt(Instant.now());
			result.setStatus("connected");

			// Try to get user account information from Schwab (optional - don't fail
			// OAuth if this fails)
			try {
	 ...
	 * fetches latest portfolio data from Schwab.
	...
	/**
	 * Get a valid access token, refreshing if necessary. Schwab access tokens expire
	 * after 30 minutes.
	 */
	...
	/**
	 * Fetch portfolio data from Schwab and store in new portfolio structure. Path:
	 * users/{userId}/portfolio/providers/{providerId}/holdings/current This includes
	 * positions, balances, and transaction history for cost basis calculation.
	 */
	private void fetchAndStorePortfolioData(String userId, String accessToken) {
		...
		try {
			...
			ProviderHoldingsEntity holdingsEntity = transformToProviderHoldingsEntity(accountsWithPositions, userId,
					accessToken);
			...
			log.info("Successfully stored Schwab portfolio data for user: {} with {} holdings", userId,
	...
	/**
	 * Transform Schwab API accounts response to ProviderHoldingsEntity. Schwab returns
	 * account data with securitiesAccount nested structure.
	 *
	 * Note: Schwab provides averagePrice directly in position data, so we don't need to
	 * fetch transactions separately for cost basis calculation.
	 */
	private ProviderHoldingsEntity transformToProviderHoldingsEntity(List<Map<String, Object>> accounts, String userId,
		...
		for (Map<String, Object> account : accounts) {
			try {
				...
				if (currentBalances != null) {
					...
					// liquidationValue is the true net account value (assets - margin
					// debt)
					...
					log.debug(
							"Schwab balance data - cash: {}, buyingPower: {}, marginBalance: {}, liquidationValue: {}",
		...
		if (liquidationValue.compareTo(BigDecimal.ZERO) > 0) {
			...
			entity.setTotalValue(totalValue); // totalValue already includes cash from
		...
		log.info(
				"Schwab portfolio summary - totalValue: {}, cashBalance: {}, buyingPower: {}, marginBalance: {}, liquidationValue: {}",
	...
	/**
	 * Transform a single Schwab position to a Holding. Schwab provides averagePrice
	 * directly in position data for cost basis calculation.
	 */
	...
	/**
	 * Calculate cost basis from transaction history. Uses TRADE transactions (buy/sell)
		...
		try {
			...
			for (Map.Entry<String, List<Map<String, Object>>> entry : txBySymbol.entrySet()) {
				...
				if (totalQuantity.compareTo(BigDecimal.ZERO) > 0) {
					...
					log.debug("Calculated cost basis for {}: totalCost={}, avgPrice={}", symbol, totalCost, avgPrice);
				}
			}
		...
		for (ProviderHoldingsEntity.Holding holding : holdings) {
			...
			try {
				...
				if (lastPrice != null && holding.getQuantity() != null) {
					...
					BigDecimal newValue = lastPrice.multiply(holding.getQuantity()).setScale(2, RoundingMode.HALF_UP);
