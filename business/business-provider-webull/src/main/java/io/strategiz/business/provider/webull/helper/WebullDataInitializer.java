...
/**
 * Component responsible for initializing and storing Webull portfolio data. Handles
 * fetching data from Webull API and persisting to Firestore.
 *
...
public class WebullDataInitializer {
	...
	/**
	 * Initialize and store provider data for a user. Fetches account positions and
	 * balance from Webull API.
	...
	public ProviderDataEntity initializeAndStoreData(String userId, String appKey, String appSecret, String accountId) {
		...
		try {
			...
			try {
				Map<String, Object> balanceResponse = webullClient.getAccountBalance(appKey, appSecret, accountId)
					.block();
			...
			ProviderDataEntity savedData = createProviderDataRepo.createOrReplaceProviderData(userId,
					WebullConstants.PROVIDER_ID, data);

			log.info("Successfully initialized and stored Webull data for user: {}, total value: {}", userId,
					savedData.getTotalValue());
	...
	private Map<String, Object> fetchAccountPositions(String appKey, String appSecret, String accountId) {
		...
		try {
			...
			if (response == null) {
				throw new StrategizException(WebullProviderErrorDetails.POSITIONS_FETCH_FAILED, MODULE_NAME,
						"null response");
			}
