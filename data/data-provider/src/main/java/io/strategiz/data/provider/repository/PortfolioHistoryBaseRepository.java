...
/**
 * Repository for PortfolioHistory entities using Firestore. Extends BaseRepository for
 * standard CRUD operations with audit support.
 *
 * Stores portfolio history as subcollection under users/{userId}/portfolio_history. Each
 * snapshot is stored with document ID as date in YYYY-MM-DD format.
 */
@Repository
public class PortfolioHistoryBaseRepository extends BaseRepository<PortfolioHistoryEntity> {
	...
	/**
	 * Override to use user-scoped collection for portfolio history. Returns
	 * users/{userId}/portfolio_history
	 */
	@Override
	...
	/**
	 * Save portfolio history using date-based document ID. Overrides to set document ID
	 * from snapshotDate before saving.
	 */
	@Override
	public PortfolioHistoryEntity save(PortfolioHistoryEntity entity, String userId) {
		...
		if (entity.getId() == null || entity.getId().isEmpty()) {
			throw new ProviderIntegrationException(DataProviderErrorDetails.INVALID_ARGUMENT, "PortfolioHistoryEntity",
					"PortfolioHistoryEntity must have snapshotDate or id set");
		}
	...
	public List<PortfolioHistoryEntity> findByUserId(String userId) {
		try {
			Query query = getUserScopedCollection(userId).whereEqualTo("isActive", true)
