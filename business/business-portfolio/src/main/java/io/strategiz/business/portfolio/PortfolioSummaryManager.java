...
/**
 * Business component for managing portfolio summary calculations and updates. Aggregates
 * data from all portfolio providers and maintains portfolio summary.
 *
...
public class PortfolioSummaryManager {
	...
	public void refreshPortfolioSummary(String userId) {
		...
		try {
			...
			// Don't throw - portfolio summary update is not critical, provider operations
			// should continue
		}
	}
	 ...
	 */
	private PortfolioSummaryEntity calculatePortfolioSummary(List<PortfolioProviderEntity> providers,
		...
		List<PortfolioSummaryEntity.CategoryAllocationData> categoryAllocations = calculateCategoryAllocations(
				valueByType, assetCountByType, totalValue, totalCash);
	 ...
	 */
	private PortfolioSummaryEntity.AssetAllocation calculateAssetAllocation(Map<String, BigDecimal> valueByType,
		...
		if (totalValue.compareTo(BigDecimal.ZERO) > 0) {
			allocation.setCryptoPercent(
					cryptoValue.divide(totalValue, 4, RoundingMode.HALF_UP).multiply(new BigDecimal("100")));
			allocation.setStocksPercent(
					stocksValue.divide(totalValue, 4, RoundingMode.HALF_UP).multiply(new BigDecimal("100")));
			allocation.setForexPercent(
					forexValue.divide(totalValue, 4, RoundingMode.HALF_UP).multiply(new BigDecimal("100")));
		}
		else {
	...
	/**
	 * Calculate category allocations for pie chart display. Maps provider types to
	 * user-friendly categories with colors.
	 */
	private List<PortfolioSummaryEntity.CategoryAllocationData> calculateCategoryAllocations(
			Map<String, BigDecimal> valueByType, Map<String, Integer> assetCountByType, BigDecimal totalValue,
		...
		for (Map.Entry<String, CategoryInfo> entry : categoryMappings.entrySet()) {
			...
			if (value.compareTo(BigDecimal.ZERO) > 0) {
				...
				allocations.add(new PortfolioSummaryEntity.CategoryAllocationData(info.category, info.categoryName,
						info.color, value, percentage, assetCount));
			}
		}
		...
		if (totalCash.compareTo(BigDecimal.ZERO) > 0) {
			...
			allocations.add(new PortfolioSummaryEntity.CategoryAllocationData("CASH", "Cash & Money Market", "#FFFFFF",
					totalCash, cashPercentage, 1 // Cash is counted as 1 "asset"
			));
		}
