 ...
 *
 * <p>Uses generic messages to avoid leaking security-sensitive information. Detailed information is
...
public enum AuthorizationErrorDetails implements ErrorDetails {
  ...
  /**
   * User lacks required scope(s) for this action. Note: Do NOT expose which scopes are required in
   * the error message.
   */
  INSUFFICIENT_PERMISSIONS(HttpStatus.FORBIDDEN, "insufficient-permissions"),

  /**
   * User lacks relationship-based access to the resource. Note: Do NOT expose which resource or
   * relation in the error message.
   */
