...
/**
 * Service for asset allocation operations. This service provides data for asset
 * allocation pie charts and related visualizations.
 */
@Service
public class AssetAllocationService extends BaseService {
	...
	private static final List<String> COLORS = Arrays.asList("#3498db", "#2ecc71", "#e74c3c", "#f39c12", "#9b59b6",
			"#1abc9c", "#d35400", "#34495e", "#16a085", "#c0392b", "#27ae60", "#8e44ad", "#f1c40f", "#e67e22",
			"#95a5a6");
	...
	public AssetAllocationData getAssetAllocation(String userId) {
		...
		try {
			...
			throw new StrategizException(ServiceDashboardErrorDetails.ASSET_ALLOCATION_FAILED, "service-dashboard", e,
					userId);
		}
	}
			...
			if (exchangeData.getAssets() != null) {
				for (Map.Entry<String, PortfolioData.AssetData> assetEntry : exchangeData.getAssets().entrySet()) {
					...
					if (assetMap.containsKey(assetKey)) {
						...
						BigDecimal existingValue = allocation.getValue() != null ? allocation.getValue()
								: BigDecimal.ZERO;
