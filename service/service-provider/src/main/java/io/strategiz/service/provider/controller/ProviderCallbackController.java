...
/**
 * Controller for handling OAuth callbacks from provider integrations. Processes
 * authorization codes and completes OAuth flows for various providers.
 *
...
public class ProviderCallbackController extends BaseController {
	...
	@Autowired
	public ProviderCallbackController(ProviderCallbackService providerCallbackService, CookieUtil cookieUtil) {
		this.providerCallbackService = providerCallbackService;
		this.cookieUtil = cookieUtil;
	}

	/**
	 * Handle OAuth callback from provider (GET request from OAuth provider). Sets
	 * HTTP-only session cookie for authentication before redirecting to frontend. Creates
	 * a database-backed session with full audit trail (IP, User-Agent).
	 ...
	 */
	@GetMapping("/{provider}")
	public RedirectView handleOAuthCallback(@PathVariable String provider, @RequestParam(required = false) String code,
		...
		log.info("Provider: {}, State: {}, Has code: {}, Error: {}, ClientIP: {}", provider, state,
				code != null ? "yes" : "no", error, clientIp);
		...
		try {
			...
			if (code == null || code.isEmpty()) {
				...
				return new RedirectView(providerCallbackService.getErrorRedirectUrl(provider, "missing_code",
						"Authorization code is required"));
			}

			if (state == null || state.isEmpty()) {
				log.error("Missing state parameter from {} callback", provider);
				return new RedirectView(providerCallbackService.getErrorRedirectUrl(provider, "missing_state",
			...
			return new RedirectView(
					providerCallbackService.getErrorRedirectUrl(provider, "processing_error", e.getMessage()));
		}
	}

	/**
	 * Handle OAuth callback from provider (POST request from frontend). Used when
	 * frontend captures the callback and sends it to backend.
	...
	public ResponseEntity<ProviderCallbackResponse> handleOAuthCallbackPost(@PathVariable String provider,
			@RequestBody Map<String, String> callbackData, Principal principal) {
		...
		log.info("Received OAuth callback POST from provider: {}, state: {}, user: {}", provider, state,
		...
		try {
			...
			if (error != null) {
				log.error("OAuth error from {}: {} - {}", provider, error, errorDescription);
				throw new StrategizException(ServiceProviderErrorDetails.OAUTH_TOKEN_EXCHANGE_FAILED,
						"service-provider", principal != null ? principal.getName() : "unknown", provider, error);
			}
			...
			// Process the OAuth callback (POST doesn't have request context, use
			// defaults)
			ProviderCallbackResponse response = providerCallbackService.processOAuthCallback(provider, code, state,
					"0.0.0.0", "Frontend POST Callback");
	...
	/**
	 * Extract the real client IP address from the request. Handles proxies, load
	 * balancers, and CDNs by checking standard headers.
