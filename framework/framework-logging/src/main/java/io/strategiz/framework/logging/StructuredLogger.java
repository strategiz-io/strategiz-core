...
/**
 * Structured logger utility that provides a fluent API for creating structured log
 * entries with key-value pairs.
 *
 * Usage examples:
 *
 * // Simple structured logging StructuredLogger.info() .operation("user_signup")
 * .userId("usr_123") .email("user@example.com") .log("User signup completed");
 *
 * // Performance logging StructuredLogger.performance() .operation("database_query")
 * .duration(150) .table("users") .log("Database query executed");
 *
 * // Error logging with context StructuredLogger.error() .operation("payment_processing")
 * .userId("usr_123") .amount(99.99) .errorCode("PAYMENT_FAILED") .log("Payment processing
 * failed", exception);
 */
public class StructuredLogger {
	...
	public static LogEntry performance() {
		return new LogEntry(LoggerFactory.getLogger(getCallerClass()), LogLevel.INFO).category("performance");
	}
	...
	public static LogEntry business() {
		return new LogEntry(LoggerFactory.getLogger(getCallerClass()), LogLevel.INFO).category("business");
	}
	...
	public static LogEntry security() {
		return new LogEntry(LoggerFactory.getLogger(getCallerClass()), LogLevel.WARN).category("security");
	}
	...
	public static LogEntry audit() {
		return new LogEntry(LoggerFactory.getLogger(getCallerClass()), LogLevel.INFO).category("audit");
	}
	...
	public static String kvs(Object... keyValues) {
		if (keyValues.length % 2 != 0) {
			throw new io.strategiz.framework.exception.StrategizException(
					io.strategiz.framework.logging.exception.LoggingErrorDetails.INVALID_ARGUMENT, "LogUtils",
					"keyValues must have an even number of arguments");
		}
