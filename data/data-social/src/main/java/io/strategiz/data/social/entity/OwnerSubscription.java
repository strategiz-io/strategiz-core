...
/**
 * Entity representing a user-to-user (owner) subscription.
 * Users subscribe to OWNERS (not individual strategies) to deploy all of the owner's PUBLIC strategies.
 *
 * <p>Collection path: users/{ownerId}/subscribers/{subscriptionId}</p>
 *
 * <p>Business Model (NO TIERS - Owner sets ONE price):</p>
 * <ul>
 ...
 * </ul>
 *
 * <p>Subscriptions persist across strategy ownership transfers:</p>
 * <ul>
 *   <li>If Bob subscribes to Alice, he can deploy Alice's owned strategies</li>
 *   <li>If Alice sells a strategy to Charlie, Bob still subscribes to Alice (not that strategy)</li>
 *   <li>Bob can now deploy all of Alice's remaining strategies + follow strategy ownership changes</li>
 * </ul>
 *
 * <p>Note: This is different from PlatformSubscription (data-preferences) which tracks
 * user subscriptions to the Strategiz PLATFORM (Explorer, Strategist, Quant tiers).</p>
 *
...
public class OwnerSubscription extends BaseEntity {
    ...
    public String getTier() {
        // Return tier based on whether subscription is free or paid (for backward compatibility)
