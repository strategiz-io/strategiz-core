...
/**
 * Base repository for ProviderData entities using Firestore Stores provider data as
 * subcollection under users/{userId}/provider_data
 */
@Repository
public class ProviderDataBaseRepository extends BaseRepository<ProviderDataEntity> {
	...
	/**
	 * Get user-scoped collection for provider data Returns users/{userId}/provider_data
	 */
	...
	public List<ProviderDataEntity> findAllByUserId(String userId) {
		try {
			...
			return docs.stream().map(doc -> {
				ProviderDataEntity entity = doc.toObject(ProviderDataEntity.class);
				entity.setId(doc.getId());
				log.info("=====> Mapped document {} to ProviderDataEntity: providerId={}", doc.getId(),
	...
	public List<ProviderDataEntity> findByAccountType(String userId, String accountType) {
		try {
			List<QueryDocumentSnapshot> docs = getUserScopedCollection(userId).whereEqualTo("account_type", accountType)
				.get()
				.get()
				.getDocuments();
	...
	public boolean existsByProviderId(String userId, String providerId) {
		try {
			...
			Thread.currentThread().interrupt();
			throw new ProviderIntegrationException(DataProviderErrorDetails.ENTITY_EXISTENCE_CHECK_FAILED, e,
					"ProviderDataEntity", providerId);
		}
		catch (ExecutionException e) {
			throw new ProviderIntegrationException(DataProviderErrorDetails.ENTITY_EXISTENCE_CHECK_FAILED, e,
					"ProviderDataEntity", providerId);
	...
	private void validateInputs(ProviderDataEntity entity, String userId) {
		if (entity == null) {
			throw new ProviderIntegrationException(DataProviderErrorDetails.INVALID_ARGUMENT, "ProviderDataEntity",
