...
public class PortfolioAnalysisContextProvider {
	...
	public PortfolioAnalysisContextProvider(PortfolioContextProvider portfolioContextProvider,
			FmpNewsClient newsClient) {
	...
	private void appendPortfolioData(StringBuilder context, String userId) {
		try {
				...
				for (Object holdingObj : holdingsList) {
					if (holdingObj instanceof Map<?, ?> holding) {
						context.append(String.format("| %s | %s | %s | %s |\n", holding.get("symbol"),
								truncateName(String.valueOf(holding.get("name"))), holding.get("value"),
			...
			if (risk instanceof Map<?, ?> riskMap && !riskMap.isEmpty()) {
				...
				riskMap.forEach((k, v) -> context.append("  • ")
					.append(formatKey(String.valueOf(k)))
					.append(": ")
					.append(v)
					.append("\n"));
				context.append("\n");
	...
	private void appendHoldingsNews(StringBuilder context, String userId) {
		if (newsClient == null || !newsClient.isConfigured()) {
			return;
		}
		...
		try {
			...
			if (holdings instanceof List<?> holdingsList && !holdingsList.isEmpty()) {
				...
				if (!symbols.isEmpty()) {
					List<FmpNewsArticle> news = newsClient.getStockNews(symbols, 5);
					if (!news.isEmpty()) {
