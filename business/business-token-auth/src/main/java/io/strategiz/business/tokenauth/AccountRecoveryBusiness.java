...
public class AccountRecoveryBusiness {
    ...
    public RecoveryInitiationResult initiateRecovery(String email, String ipAddress, String userAgent) {
        ...
        log.info("Created recovery request {} for user {} (MFA required: {})",
                savedRequest.getId(), userId, mfaRequired);

        return RecoveryInitiationResult.success(
                savedRequest.getId(),
                mfaRequired,
                phoneNumberHint,
                emailCode // In production, this would be sent via email, not returned
        );
    }
    ...
    public RecoveryStepResult verifyEmailCode(String recoveryId, String code) {
        ...
        if (Boolean.TRUE.equals(request.getMfaRequired()) && request.getPhoneNumber() != null) {
            ...
            return RecoveryStepResult.needsSms(
                    request.getPhoneNumberHint(),
                    smsCode // In production, this would be sent via SMS, not returned
            );
        }
    ...
    public RecoveryStatusResult getRecoveryStatus(String recoveryId) {
        ...
        if (request.isExpired() && request.getStatus() != RecoveryStatus.EXPIRED
                && request.getStatus() != RecoveryStatus.COMPLETED
                && request.getStatus() != RecoveryStatus.CANCELLED) {
            request.markExpired();
            recoveryRequestRepository.update(request, SYSTEM_USER);
        }

        return RecoveryStatusResult.success(
                request.getStatus(),
                request.getMfaRequired(),
                request.getEmailVerified(),
                request.getSmsVerified(),
                request.getPhoneNumberHint()
        );
    }
    ...
    private boolean isMfaMethod(AuthenticationMethodType type) {
        return type == AuthenticationMethodType.TOTP
                || type == AuthenticationMethodType.PASSKEY
    ...
    public record RecoveryInitiationResult(
            boolean success,
            String recoveryId,
            boolean mfaRequired,
            String phoneNumberHint,
            String message,
            String emailCode // For development only - remove in production
    ) {
        public static RecoveryInitiationResult success(String recoveryId, boolean mfaRequired,
                                                       String phoneNumberHint, String emailCode) {
    ...
    public record RecoveryStepResult(
            boolean success,
            String nextStep, // "SMS" or "COMPLETE"
            String phoneNumberHint,
            String recoveryToken,
            String message,
            String smsCode // For development only - remove in production
    ) {
    ...
    public record RecoveryCompletionResult(
        ...
        public static RecoveryCompletionResult success(String recoveryToken) {
            return new RecoveryCompletionResult(true, recoveryToken, "Recovery completed. You can now reset your account.");
        }
    ...
    public record RecoveryStatusResult(
            boolean found,
