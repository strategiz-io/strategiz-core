 ...
 *
 * Architecture Responsibilities: - PASETO token generation and validation (business
 * logic) - Session creation delegation to data-session module (orchestration) -
 * Authentication flow coordination
 *
 * Flow: Service Layer → Business Layer (this) → Data Layer (data-session)
 *
 * This class: ✅ Creates PASETO tokens (business logic) ✅ Orchestrates authentication flow
 * ✅ Delegates session persistence to data-session module ❌ NO servlet dependencies - pure
 * business logic ❌ NO direct HTTP handling - service layer responsibility
 */
@Component
public class SessionAuthBusiness {
	...
	@Value("${session.cleanup.expired.days:7}") // Days to keep expired sessions before
												// deletion
	private int expiredSessionRetentionDays;

	@Value("${session.cleanup.revoked.days:1}") // Days to keep revoked sessions before
												// deletion
	...
	@Value("${app.cookie.access-token-max-age:1800}") // 30 minutes default (matches
														// cookie)
	private int accessTokenExpirySeconds;

	// Refresh token: Long-lived for session continuity
	@Value("${app.cookie.refresh-token-max-age:604800}") // 7 days default (matches
															// cookie)
	...
	// Optional to avoid circular dependency (DeviceTrustBusiness depends on
	// SessionAuthBusiness)
	@Autowired(required = false)
	private DeviceTrustBusiness deviceTrustBusiness;

	@Autowired
	public SessionAuthBusiness(PasetoTokenIssuer tokenIssuer, PasetoTokenValidator tokenValidator,
	...
	/**
	 * Get access token expiry duration. Uses uniform expiry time for all authentication
	 * levels. This matches the cookie max-age to ensure token and cookie expire together.
	...
	/**
	 * Get refresh token expiry duration. Uses uniform expiry time for session continuity.
	...
	/**
	 * Create authentication tokens AND delegate session creation to data layer This is
	 * the main method all service layer authenticators should call
	 *
	 * Flow: 1. Create PASETO tokens (business logic) 2. Store tokens in
	 * PasetoTokenRepository 3. Delegate session creation to data-session module
	...
	public AuthResult createAuthentication(AuthRequest authRequest) {
		...
		log.info("Creating unified auth for user: {} with ACR: {} and methods: {}", authRequest.userId(), acr,
		...
		String accessToken = tokenIssuer.createAuthenticationToken(authRequest.userId(),
				authRequest.authenticationMethods(), acr, accessValidity, authRequest.demoMode());
		...
		log.info("Successfully created unified auth for user: {} with {} methods (session: {})", authRequest.userId(),
				authRequest.authenticationMethods().size(), accessSession != null ? "created" : "disabled");
		...
		if (deviceTrustBusiness != null && authRequest.deviceId() != null
			...
			try {
				deviceTrustBusiness.establishTrust(authRequest.deviceId(), authRequest.userId(),
						authRequest.deviceFingerprint());
			}
			catch (Exception e) {
	 ...
	 */
	private SessionEntity storeRefreshTokenSession(String refreshToken, AuthRequest authRequest,
	 ...
	 *
	 * This method validates the token cryptographically using PASETO. If the signature is
	 * valid and the token is not expired, the token is valid.
	 *
	 * Session database lookup is NOT required for basic token validation. The PASETO
	 * token contains all the information needed for authorization.
	 */
	public Optional<SessionValidationResult> validateToken(String accessToken) {
		try {
			...
			// FIRST LAYER: Validate PASETO token signature and expiration via
			// framework-authorization
			// This is the ONLY required validation - cryptographic proof that the token
			// is authentic
			...
			return Optional.of(new SessionValidationResult(publicUserId, "user@example.com", // Email
																								// not
																								// stored
																								// in
																								// token
																								// for
																								// privacy
					tokenId != null ? tokenId : "token_" + accessToken.substring(0, 8), acr, amr, demoMode, issuedAt,
					expiresAt, true));
		}
		catch (Exception e) {
	...
	/**
	 * Validate token AND check session in database (optional second layer) Use this when
	 * you need session management features like: - Session revocation checking - Session
	 * tracking/audit - Last accessed time updates
	 */
	public Optional<SessionValidationResult> validateTokenWithSession(String accessToken) {
		...
		// If session not found in database, token is still valid (PASETO signature is the
		// authority)
	...
	private Optional<SessionValidationResult> createValidationResultFromToken(String accessToken, String userId) {
		try {
			...
			return Optional.of(new SessionValidationResult(userId, "unknown@example.com",
					"token_" + accessToken.substring(0, 8), acr, amr, demoMode, issuedAt, expiresAt, true));
		}
		catch (Exception e) {
	...
	/**
	 * Legacy method for compatibility with TokenSessionService Creates authentication
	 * token pair using the new unified approach
	 */
	public TokenPair createAuthenticationTokenPair(String userId, List<String> authMethods, String acr, String deviceId,
			String ipAddress) {
		AuthRequest authRequest = new AuthRequest(userId, null, authMethods, false, deviceId, deviceId, ipAddress,
				"Legacy Client", false);
	...
	/**
	 * Creates an identity token for signup/profile creation flow. Identity tokens use a
	 * separate identity-key for security isolation.
	 *
	 * Two-Phase Token Flow: Phase 1 (Signup): createIdentityToken() → identity-key →
	 * scope="profile:create", acr="0" Phase 2 (After Auth):
	 * createAuthenticationTokenPair() → session-key → full scopes, acr="1"+"
	...
	public TokenPair createIdentityTokenPair(String userId) {
		...
		// Identity tokens don't have refresh tokens - they're short-lived and limited
		// scope
	...
	/**
	 * Refresh access token using refresh token Maintains the same ACR, AAL, and AMR from
	...
	public Optional<String> refreshAccessToken(String refreshToken, String ipAddress) {
		try {
			...
			// Check idle timeout based on access token's last activity (not refresh
			// token)
			...
			if (lastAccessed != null) {
				...
				if (idleDuration.getSeconds() > idleTimeoutSeconds) {
					log.warn(
							"Session idle timeout exceeded. Last accessed: {}, idle for: {} seconds, max allowed: {} seconds",
							lastAccessed, idleDuration.getSeconds(), idleTimeoutSeconds);
					return Optional.empty();
				}
				log.debug("Idle check passed. Last accessed: {}, idle for: {} seconds", lastAccessed,
						idleDuration.getSeconds());
			}
			...
			// Generate new access token with same auth context using
			// framework-token-issuance
			Duration accessValidity = Duration.ofHours(24);
			String newAccessToken = tokenIssuer.createAuthenticationToken(userId, authMethods, acr, accessValidity,
					demoMode);

			// Store the new access token as a session
			AuthRequest authRequest = new AuthRequest(userId, null, authMethods, false, refreshSession.getDeviceId(),
					null, ipAddress != null ? ipAddress : refreshSession.getIpAddress(), null, demoMode);
	...
	/**
	 * Find and refresh an existing active session for a user. This is the
	 * industry-standard approach for OAuth callbacks when the user is already
	 * authenticated - we refresh their existing session instead of creating a new one.
	 ...
	 * @return Optional containing the refreshed session's access token, or empty if no
	 * valid session found
	 */
	public Optional<RefreshedSession> findAndRefreshActiveSession(String userId, String clientIp, String userAgent) {
		try {
			...
			List<SessionEntity> activeSessions = sessionRepository.findByUserIdAndTokenTypeAndRevokedFalse(userId,
					"ACCESS");
			...
			log.info("Refreshed existing session {} for user {} - new expiry: {}", validSession.getSessionId(), userId,
					newExpiry);

			return Optional
				.of(new RefreshedSession(validSession.getTokenValue(), validSession.getSessionId(), newExpiry));
	 ...
	 */
	public record RefreshedSession(String accessToken, String sessionId, Instant newExpiry) {
	}

	/**
	 * Add an authentication method to an existing session and update tokens Used when
	 * adding 2FA during an active session (e.g., TOTP registration)
	...
	public Map<String, Object> addAuthenticationMethod(String accessToken, String authMethod, int newAcrLevel) {
		try {
			// Parse the current token to get user and existing methods using
			// framework-authorization
			...
			// Create new tokens with updated authentication context using
			// framework-token-issuance
			Duration accessValidity = Duration.ofHours(24);
			String newAccessToken = tokenIssuer.createAuthenticationToken(userId, updatedMethods,
					String.valueOf(newAcrLevel), accessValidity, demoMode);
			String newRefreshToken = tokenIssuer.createRefreshToken(userId);

			// Store the new tokens as sessions
			AuthRequest authRequest = new AuthRequest(userId, null, updatedMethods, false, null, null, null, null,
					demoMode // Preserve demo mode
			);
			SessionEntity newAccessSession = storeAccessTokenSession(newAccessToken, authRequest,
			...
			log.info("Added authentication method '{}' for user {} with new ACR level {}", authMethod, userId,
					newAcrLevel);
	...
	/**
	 * Cleanup expired and revoked sessions based on retention policies Industry standard:
	 * - Expired sessions: kept for 7 days after expiration for audit - Revoked sessions:
	 * kept for 1 day for immediate audit needs
	 */
	...
	public void cleanupExpiredAuth() {
		...
		log.info(
				"Session cleanup completed - deleted {} expired sessions (older than {} days) and {} revoked sessions (older than {} days)",
	 ...
	 */
	public record AuthRequest(String userId, String userEmail, List<String> authenticationMethods,
			boolean isPartialAuth, String deviceId, String deviceFingerprint, String ipAddress, String userAgent,
	 ...
	 */
	public record AuthResult(String accessToken, String refreshToken, SessionEntity session) {
