...
/**
 * Test class for demonstrating the Coinbase client integration This connects to the real
 * Coinbase API to fetch actual data
 */
public class CoinbaseClientTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// testAuthenticatedEndpoints(client, "your-api-key", "your-private-key",
			// "your-passphrase");
	...
	private static void testPublicEndpoints(CoinbaseClient client) {
		...
		CoinbaseResponse<TickerPrice> btcResponse = client.publicRequest(HttpMethod.GET, "/prices/BTC-USD/spot", params,
		...
		CoinbaseResponse<TickerPrice> ethResponse = client.publicRequest(HttpMethod.GET, "/prices/ETH-USD/spot", params,
				new ParameterizedTypeReference<CoinbaseResponse<TickerPrice>>() {
				});
	...
	private static void testAuthenticatedEndpoints(CoinbaseClient client, String apiKey, String privateKey,
			String passphrase) {
		...
		CoinbaseResponse<Map<String, Object>> accountsResponse = client.signedRequest(HttpMethod.GET, "/accounts", null,
				apiKey, privateKey, new ParameterizedTypeReference<CoinbaseResponse<Map<String, Object>>>() {
				});
		...
		for (Map<String, Object> account : accountsResponse.getData()) {
			System.out.println("  - Account: " + account.get("name") + " (" + account.get("currency") + ")"
