...
/**
 * Client for retrieving GCP billing data. Supports two modes: 1. BigQuery billing export
 * (accurate, requires setup) 2. Estimated costs (fallback when BigQuery not configured)
 *
 * Enable with: gcp.billing.enabled=true Configure mode with Vault secrets: -
 * use-bigquery=true: Use BigQuery billing export (accurate) - use-billing-api=true: Use
 * estimated costs (no BigQuery required)
 */
@Component
...
public class GcpBillingClient {
	...
	public GcpBillingClient(GcpBillingProperties properties, @Autowired(required = false) BigQuery bigQuery) {
		...
		log.info("GcpBillingClient initialized - Project: {}, Mode: {}, BigQuery Client: {}", properties.projectId(),
	...
	public GcpCostSummary getCostSummary(LocalDate startDate, LocalDate endDate) {
		log.info("Fetching GCP cost summary from {} to {} (BigQuery: {}, API: {})", startDate, endDate,
				properties.useBigQuery(), properties.useBillingApi());
	...
	private GcpCostSummary getCostSummaryFromBigQuery(LocalDate startDate, LocalDate endDate) {
		...
		try {
			String query = String.format("""
					...
					""", properties.getBillingTableFullName(), startDate.format(DATE_FORMAT),
	...
	/**
	 * Return zero costs when BigQuery billing export is not configured. We do NOT return
	 * estimates because they are misleading - only real billing data should be shown.
	...
	private List<GcpDailyCost> getDailyCostsFromBigQuery(LocalDate startDate, LocalDate endDate) {
		...
		try {
			...
			for (FieldValueList row : result.iterateAll()) {
				...
				dailyServiceCosts.computeIfAbsent(date, k -> new HashMap<>()).merge(serviceName, cost, BigDecimal::add);
			...
			for (Map.Entry<LocalDate, Map<String, BigDecimal>> entry : dailyServiceCosts.entrySet()) {
				...
				BigDecimal totalCost = serviceCosts.values().stream().reduce(BigDecimal.ZERO, BigDecimal::add);
	...
	public BigDecimal getServiceCost(String serviceName, LocalDate startDate, LocalDate endDate) {
		...
		try {
			String query = String.format("""
					...
					""", properties.getBillingTableFullName(), startDate.format(DATE_FORMAT),
					endDate.format(DATE_FORMAT), serviceName.toLowerCase());
