 ...
 *
 * These tests verify that the AI can correctly extract trading constants (SYMBOL,
 * TIMEFRAME, STOP_LOSS, TAKE_PROFIT, POSITION_SIZE) from natural language prompts.
 */
@SpringBootTest
public class AIStrategyExtractionSteps {
	...
	public void theExtractedSYMBOLShouldBe(String expectedSymbol) {
		...
		assertEquals(expectedSymbol, extractedSymbol, String
			.format("Expected SYMBOL='%s' but found '%s' in generated code", expectedSymbol, extractedSymbol));
	}
	...
	public void theExtractedTIMEFRAMEShouldBe(String expectedTimeframe) {
		...
		assertEquals(expectedTimeframe, extractedTimeframe, String
			.format("Expected TIMEFRAME='%s' but found '%s' in generated code", expectedTimeframe, extractedTimeframe));
	}
	...
	public void theExtractedSTOP_LOSSShouldBe(Double expectedStopLoss) {
		...
		assertEquals(expectedStopLoss, extractedStopLoss, 0.01, String
			.format("Expected STOP_LOSS=%.1f but found %.1f in generated code", expectedStopLoss, extractedStopLoss));
	}
	...
	public void theExtractedTAKE_PROFITShouldBe(Double expectedTakeProfit) {
		...
		assertEquals(expectedTakeProfit, extractedTakeProfit, 0.01, String.format(
				"Expected TAKE_PROFIT=%.1f but found %.1f in generated code", expectedTakeProfit, extractedTakeProfit));
	}
	...
	public void theExtractedPOSITION_SIZEShouldBe(Integer expectedPositionSize) {
		...
		assertEquals(expectedPositionSize, extractedPositionSize,
				String.format("Expected POSITION_SIZE=%d but found %d in generated code", expectedPositionSize,
						extractedPositionSize));
	}
	...
	public void stopLossMustBeAFloatPercentageNotDecimal() {
		...
		assertTrue(stopLoss >= 0.1 && stopLoss <= 100, "STOP_LOSS must be a percentage (e.g., 3.0 for 3%, not 0.03)");
	}
	...
	/**
	 * Extract string constant value from Python code. Example: SYMBOL = 'AAPL' → returns
	 * "AAPL"
	 */
	...
	/**
	 * Extract numeric constant value from Python code. Example: STOP_LOSS = 3.0 → returns
	 * 3.0
	 */
	...
	/**
	 * Extract integer constant value from Python code. Example: POSITION_SIZE = 5 →
	 * returns 5
	 */
