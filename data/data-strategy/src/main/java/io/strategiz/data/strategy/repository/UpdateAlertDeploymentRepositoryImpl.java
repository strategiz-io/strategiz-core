...
public class UpdateAlertDeploymentRepositoryImpl implements UpdateAlertDeploymentRepository {
	...
	@Override
	public Optional<AlertDeployment> updateNotificationChannels(String id, String userId,
	...
	private Optional<AlertDeployment> updateField(String id, String userId,
			java.util.function.Consumer<AlertDeployment> updater) {
		Optional<AlertDeployment> existing = baseRepository.findById(id);
		if (existing.isEmpty() || !userId.equals(existing.get().getUserId())
				|| !Boolean.TRUE.equals(existing.get().getIsActive())) {
