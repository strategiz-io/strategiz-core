...
/**
 * Base repository for AlertDeployment entities using Firestore Used internally by CRUD
...
public class AlertDeploymentBaseRepository extends BaseRepository<AlertDeployment> {
	 ...
	 */
	public java.util.List<AlertDeployment> findActiveAlertsByTierAndType(String subscriptionTier,
			String deploymentType) {
		try {
			com.google.cloud.firestore.Query query = getCollection().whereEqualTo("status", "ACTIVE")
				.whereEqualTo("subscriptionTier", subscriptionTier)
				.whereEqualTo("deploymentType", deploymentType)
				.whereEqualTo("isActive", true);
