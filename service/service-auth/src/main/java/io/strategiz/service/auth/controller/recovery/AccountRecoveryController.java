...
public class AccountRecoveryController {
	 ...
	 *
	 * <p>
	 * After successful email (and optional SMS) verification, this endpoint creates a
	 * full session and sets authentication cookies.
	 * </p>
	...
	public ResponseEntity<RecoveryAuthResponse> completeRecoveryAndAuthenticate(
		...
		AccountRecoveryBusiness.RecoveryAuthResult result = accountRecoveryBusiness
			.completeRecoveryAndAuthenticate(recoveryId, ipAddress, userAgent);
	 ...
	 *
	 * <p>
	 * Sends a verification code to the email address if it's registered. Returns the same
	 * response regardless of whether the email exists to prevent email enumeration
	 * attacks.
	 * </p>
	...
	public ResponseEntity<RecoveryInitiateResponse> initiateRecovery(
			@Valid @RequestBody RecoveryInitiateRequest request, HttpServletRequest httpRequest) {
		...
		AccountRecoveryBusiness.RecoveryInitiationResult result = accountRecoveryBusiness
			.initiateRecovery(request.email(), ipAddress, userAgent);
	 ...
	 *
	 * <p>
	 * If email verification succeeds and MFA is enabled, an SMS code will be sent to the
	 * recovery phone. If MFA is not enabled, a recovery token will be issued immediately.
	 * </p>
	 * @param recoveryId the recovery request ID
	...
	public ResponseEntity<RecoveryStepResponse> verifyEmailCode(
		...
		AccountRecoveryBusiness.RecoveryStepResult result = accountRecoveryBusiness.verifyEmailCode(recoveryId,
				request.code());
	 ...
	 *
	 * <p>
	 * This step is only required if MFA is enabled on the account. On success, a recovery
	 * token is issued.
	 * </p>
	 * @param recoveryId the recovery request ID
	 * @param request the verification request containing the code
	...
	@Operation(summary = "Verify SMS code", description = "Verify the SMS verification code (required if MFA enabled).")
	...
	public ResponseEntity<RecoveryCompletionResponse> verifySmsCode(
		...
		AccountRecoveryBusiness.RecoveryCompletionResult result = accountRecoveryBusiness.verifySmsCode(recoveryId,
				request.code());
	...
	@Operation(summary = "Get recovery status", description = "Get the current status of a recovery request.")
	...
	public ResponseEntity<RecoveryStatusResponse> getRecoveryStatus(
		...
		AccountRecoveryBusiness.RecoveryStatusResult result = accountRecoveryBusiness.getRecoveryStatus(recoveryId);
	...
	@Operation(summary = "Cancel recovery", description = "Cancel an active recovery request.")
	...
	@Operation(summary = "Resend email code", description = "Resend the email verification code.")
	...
	@Operation(summary = "Resend SMS code", description = "Resend the SMS verification code.")
	@ApiResponse(responseCode = "200", description = "Resend result")
