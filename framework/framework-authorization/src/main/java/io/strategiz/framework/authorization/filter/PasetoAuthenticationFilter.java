 ...
 * </ol>
 *
 * <p>If a valid token is found, the authenticated user is set in {@link SecurityContextHolder}. If
 * no token is found or the token is invalid, the request proceeds without authentication (the
 * authorization aspects will enforce authentication requirements).
 *
 * <p>This filter delegates ALL token validation to {@link PasetoTokenValidator}, which is the
 * single source of truth for token validation in the system.
 */
