...
/**
 * Service for handling TOTP authentication operations Manages TOTP code verification and
 * authentication
 */
@Service
...
public class TotpAuthenticationService extends BaseService {
	...
	public boolean verifyCode(String userId, String code) {
		...
		if (totpAuth == null
				|| totpAuth.getMetadataAsString(AuthenticationMethodMetadata.TotpMetadata.SECRET_KEY) == null) {
	...
	public ApiTokenResponse authenticateWithTotp(String userId, String code, String ipAddress, String deviceId) {
		...
		if (userOpt.isPresent()) {
			...
			if (totpAuth != null) {
				...
				totpAuth.putMetadata(AuthenticationMethodMetadata.TotpMetadata.LAST_USED_TIME,
						Instant.now().toString());
				totpAuth.putMetadata(AuthenticationMethodMetadata.TotpMetadata.VERIFICATION_TIME,
						Instant.now().toString());
			...
			return new ApiTokenResponse(authResult.accessToken(), authResult.refreshToken(), "Bearer");
		}
