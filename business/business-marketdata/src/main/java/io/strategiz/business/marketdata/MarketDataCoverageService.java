 ...
 *
 * Calculates aggregate statistics about data completeness: - Coverage percentage by
 * timeframe (how many symbols have data) - Storage metrics (ClickHouse size, row counts)
 * - Data quality distribution (good, partial, poor quality symbols) - Identified gaps in
 * data coverage
 *
 * Snapshots are stored in ClickHouse and calculated: - Daily (via scheduled job at 2 AM)
...
public class MarketDataCoverageService {
	...
	@Autowired
	public MarketDataCoverageService(MarketDataCoverageClickHouseRepository coverageRepository,
			SymbolDataStatusClickHouseRepository symbolStatusRepository,
			MarketDataClickHouseRepository marketDataRepository, SymbolService symbolService) {
	...
	/**
	 * Calculate current coverage statistics and save snapshot to ClickHouse. This is a
	 * heavy operation that queries ClickHouse for aggregate stats.
	 * @param userId User ID triggering the calculation (typically "system" for scheduled
	 * jobs)
	...
	public MarketDataCoverageEntity calculateAndSaveCoverage(String userId) {
		...
		try {
			...
			for (String timeframe : timeframes) {
				MarketDataCoverageEntity.TimeframeCoverage coverage = calculateTimeframeCoverage(timeframe,
						totalSymbols);
			...
			log.info("Coverage calculation completed in {}ms - snapshot: {}, {} symbols, {} timeframes", duration,
					saved.getSnapshotId(), totalSymbols, timeframes.length);
			...
			throw new StrategizException(MarketDataErrorDetails.DATA_FETCH_FAILED, "business-marketdata", e,
	...
	private MarketDataCoverageEntity.TimeframeCoverage calculateTimeframeCoverage(String timeframe, int totalSymbols) {
		...
		double coveragePercent = (totalSymbols > 0) ? (double) symbolsWithData / totalSymbols * 100.0 : 0.0;
		...
		log.debug("Timeframe {} coverage: {}/{} symbols ({:.2f}%)", timeframe, symbolsWithData, totalSymbols,
				coveragePercent);
	...
	private MarketDataCoverageEntity.QualityStats calculateQualityStats() {
		...
		log.debug("Quality distribution - Good: {}, Partial: {}, Poor: {}", goodQuality, partialQuality, poorQuality);

		return quality;
	}

	/**
	 * Detect gaps in market data coverage. This is a placeholder - would implement proper
	 * gap detection logic.
	...
	public Optional<MarketDataCoverageEntity> getLatestSnapshot() {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved latest coverage snapshot in {}ms, found: {}", duration, latest.isPresent());
	...
	public List<MarketDataCoverageEntity> getSnapshotsByDateRange(Timestamp start, Timestamp end) {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved {} snapshots in date range in {}ms", snapshots.size(), duration);
	...
	public List<MarketDataCoverageEntity> getRecentSnapshots(int limit) {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved {} recent snapshots in {}ms", snapshots.size(), duration);
	...
	public int cleanupOldSnapshots(int retentionDays) {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.info("Cleaned up {} old coverage snapshots (older than {} days) in {}ms", deleted, retentionDays, duration);
