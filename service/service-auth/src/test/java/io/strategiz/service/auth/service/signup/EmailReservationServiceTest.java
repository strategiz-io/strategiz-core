 ...
 *
 * Tests email reservation logic for signup uniqueness enforcement.
 * Uses mocks to isolate the service from repository implementations.
 */
...
class EmailReservationServiceTest {
    ...
    class ReserveEmailTests {
        ...
        void reserveEmail_AlreadyReserved_ThrowsException() {
            ...
            when(userRepository.getUserByEmail(TEST_EMAIL)).thenReturn(Optional.empty());
            when(emailReservationRepository.reserve(any(EmailReservationEntity.class)))
                .thenThrow(new DataRepositoryException(DataRepositoryErrorDetails.DUPLICATE_ENTITY,
    ...
    class DuplicateEmailSignupTests {
        ...
        void reserveEmail_SecondAttemptSameEmail_Fails() {
            ...
            when(userRepository.getUserByEmail(TEST_EMAIL)).thenReturn(Optional.empty());
            when(emailReservationRepository.reserve(any(EmailReservationEntity.class)))
                .thenAnswer(invocation -> invocation.getArgument(0)) // First call succeeds
                .thenThrow(new DataRepositoryException(DataRepositoryErrorDetails.DUPLICATE_ENTITY,
                    "EmailReservation", TEST_EMAIL)); // Second call fails
    ...
    class ConfirmReservationTests {
        ...
        void confirmReservation_Success() {
            ...
            EmailReservationEntity reservation = new EmailReservationEntity(
                TEST_EMAIL, "user-123", TEST_SIGNUP_TYPE, TEST_SESSION_ID);
            EmailReservationEntity confirmedReservation = new EmailReservationEntity(
                TEST_EMAIL, "user-123", TEST_SIGNUP_TYPE, TEST_SESSION_ID);
        ...
        void confirmReservation_NotFound_ThrowsException() {
            // Given
            when(emailReservationRepository.confirm(anyString()))
                .thenThrow(new DataRepositoryException(DataRepositoryErrorDetails.ENTITY_NOT_FOUND,
                    "EmailReservation", TEST_EMAIL));
    ...
    class GetReservedUserIdTests {
        ...
        void getReservedUserId_ValidReservation_ReturnsUserId() {
            ...
            EmailReservationEntity reservation = new EmailReservationEntity(
                TEST_EMAIL, expectedUserId, TEST_SIGNUP_TYPE, TEST_SESSION_ID);

            when(emailReservationRepository.findByEmail(TEST_EMAIL.toLowerCase()))
