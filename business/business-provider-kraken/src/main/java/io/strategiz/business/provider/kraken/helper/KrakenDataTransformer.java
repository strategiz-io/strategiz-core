...
/**
 * Component responsible for transforming Kraken API responses into
 * ProviderHoldingsEntity. Handles data normalization, calculation of portfolio metrics,
...
public class KrakenDataTransformer {
	 ...
	 */
	public ProviderHoldingsEntity transformKrakenData(String userId, Map<String, Object> balanceResponse,
			Map<String, Object> tradesResponse, Map<String, BigDecimal> currentPrices) {

		log.debug("Transforming Kraken data for user: {}", userId);

		try {
			// Create entity - ProviderHoldingsEntity only contains holdings data
			// Provider metadata (name, type, category) is stored in
			...
			log.debug("Transformed data: {} holdings, total value: {}, cash: {}", holdings.size(), totalValue,
					cashBalance);
			...
			throw new StrategizException(KrakenProviderErrorDetails.DATA_TRANSFORMATION_FAILED, MODULE_NAME, userId,
	...
	private List<ProviderHoldingsEntity.Holding> createHoldings(Map<String, Object> balances,
			Map<String, BigDecimal> currentPrices, Map<String, Object> tradesResponse) {
		...
		for (Map.Entry<String, Object> entry : balances.entrySet()) {
			...
			try {
				...
				if (quantity.compareTo(BigDecimal.ZERO) > 0) {
					...
					log.debug("Created holding: {} qty={} price={} value={}", normalizedAsset, quantity, currentPrice,
							currentValue);
				}
	...
	private String getAssetFullName(String asset) {
		...
		Map<String, String> assetNames = Map.of("BTC", "Bitcoin", "ETH", "Ethereum", "XRP", "Ripple", "LTC", "Litecoin",
				"ADA", "Cardano", "DOT", "Polkadot", "LINK", "Chainlink", "USD", "US Dollar", "EUR", "Euro");
	...
	/**
	 * Transform enriched Kraken data into ProviderHoldingsEntity. This method uses
	 * pre-enriched data from the enrichment service.
	 * @param userId User ID for audit fields
	 ...
	 * @return Transformed ProviderHoldingsEntity ready for storage
	 */
	public ProviderHoldingsEntity transformEnrichedData(String userId, EnrichedKrakenData enrichedData,
			Map<String, Object> tradesResponse) {
		...
		try {
			...
			log.debug("Transformed enriched data: {} holdings, total value: {}, cash: {}", holdings.size(), totalValue,
					cashBalance);
	...
	private List<ProviderHoldingsEntity.Holding> createEnrichedHoldings(EnrichedKrakenData enrichedData,
		...
		for (EnrichedKrakenData.AssetInfo assetInfo : enrichedData.getAssetInfo().values()) {
			...
			String assetForCostBasis = assetInfo.getOriginalSymbol() != null ? assetInfo.getOriginalSymbol()
					: assetInfo.getNormalizedSymbol();
			...
			log.debug("Created enriched holding: {} ({}) qty={} value={} staked={}", holding.getAsset(),
					holding.getAssetType(), holding.getQuantity(), holding.getCurrentValue(), holding.getIsStaked());
		}
	...
	private BigDecimal calculateCashBalance(EnrichedKrakenData enrichedData) {
		...
		for (EnrichedKrakenData.AssetInfo assetInfo : enrichedData.getAssetInfo().values()) {
			if (assetInfo.isCash()) {
				...
				BigDecimal usdValue = assetInfo.getQuantity()
					.multiply(assetInfo.getCurrentPrice() != null ? assetInfo.getCurrentPrice() : BigDecimal.ONE);
	...
	private void calculateProfitLoss(ProviderHoldingsEntity data, Map<String, Object> tradesResponse) {
		...
		if (totalCostBasis.compareTo(BigDecimal.ZERO) > 0) {
			BigDecimal profitLossPercent = totalProfitLoss.divide(totalCostBasis, 4, RoundingMode.HALF_UP)
				.multiply(new BigDecimal("100"));
