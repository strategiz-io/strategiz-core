 ...
 *
 * Key rules: - Only strategy owner can publish/unpublish - Publishing requires pricing
 * configuration - Once published, pricing can be updated but not removed
 */
@Service
public class StrategyPublishService extends BaseService {
	...
	@Autowired
	public StrategyPublishService(ReadStrategyRepository readStrategyRepo, UpdateStrategyRepository updateStrategyRepo,
	 ...
	 */
	public Strategy publishStrategy(String strategyId, String userId, PricingType pricingType, BigDecimal oneTimePrice,
		...
		switch (pricingType) {
			...
			case ONE_TIME:
				if (oneTimePrice == null || oneTimePrice.compareTo(BigDecimal.ZERO) <= 0) {
					throw new StrategizException(MarketplaceErrorDetails.INVALID_ONE_TIME_PRICE, MODULE_NAME,
			...
			case SUBSCRIPTION:
				if (monthlyPrice == null || monthlyPrice.compareTo(BigDecimal.ZERO) <= 0) {
					throw new StrategizException(MarketplaceErrorDetails.INVALID_MONTHLY_PRICE, MODULE_NAME,
							"Monthly price is required and must be positive");
				}
				pricing = StrategyPricing.subscription(monthlyPrice, "USD", null);
				break;
			default:
				throw new StrategizException(MarketplaceErrorDetails.INVALID_PRICING_TYPE, MODULE_NAME,
	 ...
	 *
	 * This method supports the marketplace "Sell Strategy" flow where: - Setting a price
	 * automatically sets listedStatus to LISTED - Removing listing sets listedStatus to
	 * NOT_LISTED
	 ...
	 * @return The updated strategy
	 */
	public Strategy updatePricingAndListingStatus(String strategyId, String userId, BigDecimal price,
			String listedStatus) {
	 ...
	 */
	public Strategy updatePricing(String strategyId, String userId, PricingType pricingType, BigDecimal oneTimePrice,
			BigDecimal monthlyPrice) {
