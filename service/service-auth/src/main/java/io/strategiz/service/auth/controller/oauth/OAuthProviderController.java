...
/**
 * Generic OAuth provider controller that delegates to specific OAuth services. This
 * controller provides a unified interface for all OAuth providers.
 */
@RestController
...
public class OAuthProviderController extends BaseController {
	...
	public ResponseEntity<Map<String, Object>> handleSignupCallbackJson(@PathVariable String provider,
		...
		try {
			...
			Map<String, Object> errorResponse = Map.of("success", false, "error", e.getMessage(), "message",
					provider + " OAuth sign-up callback failed");
	...
	public ResponseEntity<Map<String, Object>> handleSigninCallbackJson(@PathVariable String provider,
			@RequestBody Map<String, String> callbackRequest) {
		...
		try {
			...
			Map<String, Object> errorResponse = Map.of("success", false, "error", e.getMessage(), "message",
					provider + " OAuth sign-in callback failed");
			return ResponseEntity.ok(errorResponse);
		}
	}
	...
	public RedirectView handleSignupCallback(@PathVariable String provider, @RequestParam String code,
			@RequestParam String state, @RequestParam(required = false) String deviceId) {
		...
		try {
			...
			if (!success) {
				...
				return new RedirectView(
						String.format("%s/auth/oauth/%s/callback?error=%s", frontendUrl, provider, error));
			}

			return new RedirectView(
					String.format("%s/auth/oauth/%s/callback?code=%s&state=%s", frontendUrl, provider, code, state));
		}
		catch (Exception e) {
			logger.error("Error in {} OAuth sign-up callback", provider, e);
			String frontendUrl = oauthProviderService.getFrontendUrl();
			return new RedirectView(
					String.format("%s/auth/oauth/%s/callback?error=%s", frontendUrl, provider, e.getMessage()));
		}
	}
	...
	public RedirectView handleSigninCallback(@PathVariable String provider, @RequestParam String code,
			@RequestParam String state, @RequestParam(required = false) String deviceId) {
