 ...
 *
 * Endpoints: - POST /v1/strategies/{strategyId}/publish - Publish strategy with pricing -
 * POST /v1/strategies/{strategyId}/unpublish - Make strategy private - PUT
 * /v1/strategies/{strategyId}/pricing - Update pricing - GET
 * /v1/strategies/{strategyId}/status - Get publishing status
 */
@RestController
...
public class StrategyPublishController extends BaseController {
	 ...
	 *
	 * Request body: { "pricingType": "FREE" | "ONE_TIME" | "SUBSCRIPTION",
	 * "oneTimePrice": 29.99, // required for ONE_TIME "monthlyPrice": 9.99 // required
	 * for SUBSCRIPTION }
	 */
	@PostMapping("/{strategyId}/publish")
	@CrossOrigin(origins = { "http://localhost:3000", "http://localhost:3001", "https://strategiz.io" },
			allowedHeaders = "*")
	public ResponseEntity<Object> publishStrategy(@PathVariable String strategyId,
			@RequestBody Map<String, Object> requestBody, @AuthUser AuthenticatedUser user) {
		try {
			...
			if (pricingTypeStr == null) {
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("error", "Pricing type is required"));
			}
			...
			Strategy published = publishService.publishStrategy(strategyId, userId, pricingType, oneTimePrice,
					monthlyPrice);

			return ResponseEntity.ok(Map.of("message", "Strategy published successfully", "strategy", published));
		}
		catch (IllegalArgumentException e) {
	...
	@CrossOrigin(origins = { "http://localhost:3000", "http://localhost:3001", "https://strategiz.io" },
			allowedHeaders = "*")
	public ResponseEntity<Object> unpublishStrategy(@PathVariable String strategyId, @AuthUser AuthenticatedUser user) {
		try {
			String userId = user.getUserId();

			Strategy unpublished = publishService.unpublishStrategy(strategyId, userId);
			return ResponseEntity.ok(Map.of("message", "Strategy unpublished successfully", "strategy", unpublished));
		}
		catch (Exception e) {
	 ...
	 *
	 * Format 1 - Simple (for marketplace listing): { "price": 99.99, // optional - the
	 * sale price "listedStatus": "LISTED" // optional - "LISTED" or "NOT_LISTED" }
	 *
	 * Format 2 - Full pricing configuration: { "pricingType": "ONE_TIME", // "FREE",
	 * "ONE_TIME", or "SUBSCRIPTION" "oneTimePrice": 29.99, // for ONE_TIME
	 * "monthlyPrice": 9.99 // for SUBSCRIPTION }
	 */
	@PutMapping("/{strategyId}/pricing")
	@CrossOrigin(origins = { "http://localhost:3000", "http://localhost:3001", "https://strategiz.io" },
			allowedHeaders = "*")
	public ResponseEntity<Object> updatePricing(@PathVariable String strategyId,
			@RequestBody Map<String, Object> requestBody, @AuthUser AuthenticatedUser user) {
		try {
			String userId = user.getUserId();
			...
			if (price != null || listedStatus != null) {
				...
				updated = publishService.updatePricingAndListingStatus(strategyId, userId, price, listedStatus);
			}
				...
				updated = publishService.updatePricing(strategyId, userId, pricingType, oneTimePrice, monthlyPrice);
			}
			else {
				return ResponseEntity.status(HttpStatus.BAD_REQUEST)
			...
			return ResponseEntity.ok(Map.of("message", "Pricing updated successfully", "strategy", updated));
		}
		catch (IllegalArgumentException e) {
	...
	public ResponseEntity<Object> getPublishStatus(@PathVariable String strategyId) {
		try {
			...
			// Note: publishedAt field removed from data model - use createdDate or track
			// separately
			return ResponseEntity
				.ok(Map.of("strategyId", strategyId, "isPublished", Boolean.TRUE.equals(strategy.getIsPublished()),
						"pricing", strategy.getPricing() != null ? strategy.getPricing() : "FREE", "subscriberCount",
						strategy.getSubscriberCount() != null ? strategy.getSubscriberCount() : 0, "commentCount",
						strategy.getCommentCount() != null ? strategy.getCommentCount() : 0));
		}
		catch (Exception e) {
