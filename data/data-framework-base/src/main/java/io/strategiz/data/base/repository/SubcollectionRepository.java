...
/**
 * Base repository for entities stored in subcollections. Extends BaseRepository to
 * support documents nested under parent documents.
 *
...
public abstract class SubcollectionRepository<T extends BaseEntity> extends BaseRepository<T> {
	...
	protected CollectionReference getSubcollection(String parentId) {
		...
		if (parentId == null || parentId.trim().isEmpty()) {
			throw new DataRepositoryException(DataRepositoryErrorDetails.ENTITY_ID_REQUIRED, getSubcollectionName(),
					"parentId");
		}
		...
		if (!isUUID) {
			log.warn("CRITICAL: parentId [{}] is NOT in UUID format! This may cause incorrect Firestore paths.",
					parentId);
			log.warn("Expected format: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx (UUID v4)");
			log.warn("This will create path: {}/{}/{} which may be incorrect!", getParentCollectionName(), parentId,
					getSubcollectionName());
		}

		log.info("SubcollectionRepository.getSubcollection - FULL PATH: {}/{}/{}", getParentCollectionName(), parentId,
				getSubcollectionName());

		return firestore.collection(getParentCollectionName()).document(parentId).collection(getSubcollectionName());
	}
	...
	public T saveInSubcollection(String parentId, T entity, String userId) {
		try {
			...
			if (io.strategiz.data.base.transaction.FirestoreTransactionHolder.isTransactionActive()) {
				com.google.cloud.firestore.Transaction transaction = io.strategiz.data.base.transaction.FirestoreTransactionHolder
					.getTransaction();
				...
				log.debug("Saving {} in subcollection outside transaction for parent {}", entityClass.getSimpleName(),
						parentId);
				getSubcollection(parentId).document(entity.getId()).set(entity).get();
			}

			log.debug("Saved {} in subcollection under parent {} by user {}", entityClass.getSimpleName(), parentId,
					userId);
			...
			throw new DataRepositoryException(DataRepositoryErrorDetails.SUBCOLLECTION_ACCESS_FAILED, e,
					entityClass.getSimpleName(), parentId);
		}
	}
	...
	public List<T> findAllInSubcollection(String parentId) {
		try {
			var query = getSubcollection(parentId).whereEqualTo("isActive", true);
	...
	protected CollectionReference getCollection() {
		throw new DataRepositoryException(DataRepositoryErrorDetails.SUBCOLLECTION_ACCESS_FAILED,
				entityClass.getSimpleName(), "Use subcollection-specific methods with parentId");
	}
