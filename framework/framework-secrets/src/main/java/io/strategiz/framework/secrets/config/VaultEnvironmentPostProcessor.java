...
/**
 * Environment post-processor that loads OAuth credentials from Vault early in the Spring Boot
 * startup process, before beans are created. This ensures @Value annotations work correctly.
 */
public class VaultEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    try {
      ...
      String vaultToken =
          environment.getProperty(
              "spring.cloud.vault.token", environment.getProperty("VAULT_TOKEN", "root"));
      ...
      VaultTemplate vaultTemplate =
          new VaultTemplate(endpoint, new TokenAuthentication(vaultToken));
      ...
      log.error(
          "Failed to load OAuth credentials from Vault. Application will start but OAuth features"
              + " may not work.",
          e);
    }
  }
  ...
  private void loadProviderOAuthCredentials(
      VaultTemplate vaultTemplate, Map<String, Object> properties) {
    // Coinbase OAuth
  ...
  @SuppressWarnings("AbbreviationAsWordInName")
  private void loadAuthOAuthCredentials(
      VaultTemplate vaultTemplate, Map<String, Object> properties) {
  ...
  @SuppressWarnings("AbbreviationAsWordInName")
  private void loadOAuthProvider(
      VaultTemplate vaultTemplate, String provider, Map<String, Object> properties) {
    try {
      ...
      if (response != null && response.getData() != null) {
        ...
        if (data != null) {
          ...
          if (clientSecret != null) {
            properties.put(
                String.format("oauth.providers.%s.client-secret", provider), clientSecret);
          }
