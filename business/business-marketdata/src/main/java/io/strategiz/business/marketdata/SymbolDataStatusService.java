 ...
 *
 * Tracks the status of each symbol/timeframe combination: - Last update timestamp -
 * Record count (number of bars collected) - Consecutive failure tracking - Status
 * classification (ACTIVE, STALE, FAILED)
 *
 * Enables: - Stale symbol detection for incremental jobs - Data quality monitoring -
 * Failure pattern analysis - Symbol-level diagnostics
 */
@Service
...
public class SymbolDataStatusService {
	...
	private static final long STALE_THRESHOLD_HOURS_INTRADAY = 24; // 24 hours for
																	// intraday data

	private static final int FAILURE_THRESHOLD = 3; // Consider failed after 3 consecutive
													// failures
	...
	public void recordSuccessfulUpdate(String symbol, String timeframe, long recordCount, Instant lastBarTimestamp) {
		...
		log.debug("Updated status for {} {} - {} records, last bar: {}", symbol, timeframe, recordCount,
				lastBarTimestamp);
	}
	...
	public void recordFailedUpdate(String symbol, String timeframe, String errorMessage) {
		...
		log.warn("Recorded failure for {} {} - consecutive failures: {}, status: {}", symbol, timeframe,
	...
	public List<Map<String, Object>> getSymbolStatus(String symbol) {
		...
		log.debug("Retrieved status for {} in {}ms - {} timeframes", symbol, duration, statusList.size());
	...
	public List<Map<String, Object>> getStaleSymbols(String timeframe, int page, int pageSize) {
		...
		List<Map<String, Object>> staleSymbols = symbolDataStatusRepository.findStaleSymbols(timeframe, staleThreshold,
				pageable);

		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved {} stale symbols for {} in {}ms (page {} of size {})", staleSymbols.size(), timeframe,
				duration, page, pageSize);
	...
	public List<Map<String, Object>> getFailingSymbols(int minFailures) {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved {} failing symbols (>= {} failures) in {}ms", failingSymbols.size(), minFailures,
				duration);
	...
	public List<Object[]> getFreshnessStats() {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved freshness stats for {} timeframes in {}ms", stats.size(), duration);
	...
	public List<Map<String, Object>> getSymbolsByTimeframe(String timeframe, int page, int pageSize) {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved {} symbols for {} in {}ms (page {} of size {})", symbols.size(), timeframe, duration, page,
				pageSize);
	...
	public long countSymbolsByStatus(String timeframe, String status) {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Count for {} with status {} in {}ms: {} symbols", timeframe, status, duration, count);
	...
	public List<Map<String, Object>> searchSymbols(String pattern, int page, int pageSize) {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Symbol search for pattern '{}' in {}ms: {} results (page {} of size {})", pattern, duration,
				symbols.size(), page, pageSize);
	...
	public List<Object[]> getStatusDistribution() {
		...
		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved status distribution in {}ms - {} statuses", duration, distribution.size());

		return distribution;
	}

	/**
	 * Calculate freshness metrics for specific timeframes. This is the key method for the
	 * consolidated coverage view showing "% fresh within 15 minutes".
	...
	public List<Map<String, Object>> calculateFreshnessMetrics(List<String> timeframes, int freshnessThresholdMinutes) {
		...
		List<Map<String, Object>> metrics = symbolDataStatusRepository.calculateFreshnessMetrics(timeframes,
				freshnessThresholdMinutes);

		long duration = System.currentTimeMillis() - startTime;
		log.info("Calculated freshness metrics for {} timeframes ({} min threshold) in {}ms", timeframes.size(),
				freshnessThresholdMinutes, duration);

		return metrics;
	}

	/**
	 * Get all symbols with their status across multiple timeframes. Used for the
	 * consolidated symbol view showing each symbol's freshness across all target
	 * timeframes.
	...
	public List<Map<String, Object>> getSymbolsWithAllTimeframes(List<String> timeframes, int page, int pageSize) {
		...
		Pageable pageable = PageRequest.of(page, pageSize);
		List<Map<String, Object>> symbols = symbolDataStatusRepository.findSymbolsWithAllTimeframes(timeframes,
				pageable);

		long duration = System.currentTimeMillis() - startTime;
		log.debug("Retrieved {} symbols with {} timeframes in {}ms (page {} of size {})", symbols.size(),
				timeframes.size(), duration, page, pageSize);
