...
/**
 * Firestore implementation of SessionRepository Stores sessions in the 'sessions'
 * collection for persistent storage Following SOLID principles - Single Responsibility:
 * Only handles Firestore persistence
 */
@Repository
@Primary
public class FirestoreSessionRepository extends BaseRepository<SessionEntity> implements SessionRepository {
	...
	public List<SessionEntity> findAll() {
		try {
			...
			throw new StrategizException(DataRepositoryErrorDetails.QUERY_EXECUTION_FAILED, "data-session", e,
					"findAll");
		}
	}
	...
	public void deleteById(String id) {
		try {
			...
			Thread.currentThread().interrupt();
			throw new StrategizException(DataRepositoryErrorDetails.ENTITY_DELETE_FAILED, "data-session", e,
					"SessionEntity", id);
		}
	}
	...
	public void deleteAll() {
		try {
			...
			Thread.currentThread().interrupt();
			throw new StrategizException(DataRepositoryErrorDetails.BULK_OPERATION_FAILED, "data-session", e,
					"deleteAll");
		}
	}
	...
	public Optional<SessionEntity> findByTokenValue(String tokenValue) {
		try {
			Query query = getCollection().whereEqualTo("tokenValue", tokenValue)
	...
	public List<SessionEntity> findByUserIdAndTokenType(String userId, String tokenType) {
		try {
			...
			throw new StrategizException(DataRepositoryErrorDetails.QUERY_EXECUTION_FAILED, "data-session", e,
					"findByUserIdAndTokenType", userId, tokenType);
		}
	}
	...
	public List<SessionEntity> findByUserIdAndTokenTypeAndRevokedFalse(String userId, String tokenType) {
		try {
			...
			throw new StrategizException(DataRepositoryErrorDetails.QUERY_EXECUTION_FAILED, "data-session", e,
					"findByUserIdAndTokenTypeAndRevokedFalse", userId, tokenType);
		}
	}
	...
	public List<SessionEntity> findByDeviceId(String deviceId) {
		try {
			Query query = getCollection().whereEqualTo("deviceId", deviceId).whereEqualTo("isActive", true);
	...
	public List<SessionEntity> findByDeviceIdAndRevokedFalse(String deviceId) {
		try {
			Query query = getCollection().whereEqualTo("deviceId", deviceId)
				.whereEqualTo("revoked", false)
				.whereEqualTo("isActive", true);
			...
			throw new StrategizException(DataRepositoryErrorDetails.QUERY_EXECUTION_FAILED, "data-session", e,
					"findByDeviceIdAndRevokedFalse", deviceId);
		}
	}
	...
	public List<SessionEntity> findByExpiresAtBefore(Instant expiresBefore) {
		try {
			Query query = getCollection().whereLessThan("expiresAt", expiresBefore).whereEqualTo("isActive", true);
	...
	public List<SessionEntity> findByRevokedTrue() {
		try {
			Query query = getCollection().whereEqualTo("revoked", true).whereEqualTo("isActive", true);
	...
	public List<SessionEntity> findByExpiresAtBeforeOrRevokedTrue(Instant expiresBefore) {
		try {
			// Firestore doesn't support OR queries directly, so we need to run two
			// queries
			...
			throw new StrategizException(DataRepositoryErrorDetails.QUERY_EXECUTION_FAILED, "data-session", e,
					"findByExpiresAtBeforeOrRevokedTrue");
		}
	}
