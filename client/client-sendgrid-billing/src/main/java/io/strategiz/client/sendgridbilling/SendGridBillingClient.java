 ...
 *
 * Note: SendGrid does not have a public billing API.
 * Billing API keys exist but endpoints return "resource not found" errors.
 * This implementation returns $0.00 (no estimates/fake data).
 *
 * If SendGrid releases a public billing API in the future, this client will
...
public class SendGridBillingClient {
    ...
    public SendGridBillingClient(
            @Qualifier("sendgridBillingRestTemplate") RestTemplate restTemplate,
    ...
    public SendGridCostSummary getCostSummary(LocalDate startDate, LocalDate endDate) {
        ...
        try {
            ...
            String url = String.format("%s/billing?start_date=%s&end_date=%s",
                    properties.apiUrl(),
                    startDate,
                    endDate);
            ...
            log.warn("⚠️ SendGrid billing API not available: {} - returning $0.00 (real data not available)", e.getMessage());
        }
    ...
    public SendGridUsage getCurrentUsage() {
        ...
        try {
            ...
            log.warn("⚠️ SendGrid usage API not available: {} - returning empty usage (real data not available)", e.getMessage());
        }
    ...
    private SendGridUsage getEmptyUsage() {
        return new SendGridUsage(
                "unavailable",
                "SendGrid Email Service",
                BigDecimal.ZERO, // No email data
    ...
    private SendGridCostSummary parseBillingResponse(
