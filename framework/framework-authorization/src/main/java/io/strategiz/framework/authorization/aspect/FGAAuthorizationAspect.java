 ...
 *
 * <p>This provides Layer 2 authorization - relationship-based access control via FGA (OpenFGA). It
 * checks if the authenticated user has a specific relation to a resource.
 *
 * <p>Order: 200 (runs after authentication and scope checks)
 */
@Aspect
@Component
@Order(200)
public class FGAAuthorizationAspect {
  ...
  public void checkAuthorization(JoinPoint joinPoint, Authorize authorize) {
    ...
    if (!allowed) {
      ...
      log.warn(
          "FGA check failed: user={} relation={} resource={}:{}",
          user.getUserId(),
          relation,
    ...
    log.debug(
        "FGA check passed: user={} relation={} resource={}:{}",
        user.getUserId(),
        relation,
        resourceType.getTypeName(),
        resourceId);
