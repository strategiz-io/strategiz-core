...
/**
 * Service for fetching strategy execution metrics from Grafana Cloud. Queries Python
 * execution service metrics exported via OpenTelemetry.
 */
@Service
public class ExecutionMetricsService {
	...
	public Map<String, Object> getExecutionHealth() {
		...
		PrometheusQueryResult requestRate = grafanaClient.query("sum(rate(strategy_requests_total[5m]))");
		health.put("requestsPerSecond", requestRate.getFirstValue().orElse(0.0));

		// P99 latency (milliseconds)
		PrometheusQueryResult p99Latency = grafanaClient
			.query("histogram_quantile(0.99, sum(rate(strategy_execution_time_bucket[5m])) by (le))");
		...
		// Active executions
		PrometheusQueryResult activeExecutions = grafanaClient.query("strategy_executions_active");
		health.put("activeExecutions", activeExecutions.getFirstValue().orElse(0.0).intValue());

		// Error rate (errors/sec)
		PrometheusQueryResult errorRate = grafanaClient.query("sum(rate(strategy_errors_total[5m]))");
	...
	public Map<String, Object> getExecutionLatency(int durationMinutes) {
		...
		PrometheusQueryResult p50 = grafanaClient.queryRange(
				"histogram_quantile(0.50, sum(rate(strategy_execution_time_bucket[5m])) by (le))", start, end, "1m");
		latency.put("p50", convertToTimeSeries(p50));

		// P95 latency
		PrometheusQueryResult p95 = grafanaClient.queryRange(
				"histogram_quantile(0.95, sum(rate(strategy_execution_time_bucket[5m])) by (le))", start, end, "1m");
		latency.put("p95", convertToTimeSeries(p95));

		// P99 latency
		PrometheusQueryResult p99 = grafanaClient.queryRange(
				"histogram_quantile(0.99, sum(rate(strategy_execution_time_bucket[5m])) by (le))", start, end, "1m");
	...
	public Map<String, Object> getExecutionThroughput(int durationMinutes) {
		...
		// Success rate
		PrometheusQueryResult successRate = grafanaClient
			.queryRange("sum(rate(strategy_requests_total{status=\"success\"}[5m]))", start, end, "1m");
		throughput.put("success", convertToTimeSeries(successRate));

		// Failure rate
		PrometheusQueryResult failureRate = grafanaClient
			.queryRange("sum(rate(strategy_requests_total{status=\"failure\"}[5m]))", start, end, "1m");
	...
	public Map<String, Object> getCachePerformance(int durationMinutes) {
		...
		// Cache hits
		PrometheusQueryResult hits = grafanaClient.queryRange("sum(rate(strategy_cache_hits[5m]))", start, end, "1m");
		cache.put("hits", convertToTimeSeries(hits));

		// Cache misses
		PrometheusQueryResult misses = grafanaClient.queryRange("sum(rate(strategy_cache_misses[5m]))", start, end,
				"1m");
	...
	public Map<String, Object> getExecutionErrors(int durationMinutes) {
		...
		// Errors by type
		PrometheusQueryResult errorsByType = grafanaClient
			.queryRange("sum(rate(strategy_errors_total[5m])) by (error_type)", start, end, "1m");

		// Group by error type
		errorsByType.getResults().forEach(result -> {
			String errorType = result.getLabels().getOrDefault("error_type", "unknown");
			errors.put(errorType,
					result.getValues()
						.stream()
	...
	private Object convertToTimeSeries(PrometheusQueryResult result) {
		...
		return result.getResults()
			...
			.getValues()
			.stream()
			.map(v -> Map.of("timestamp", v.getTimestamp() * 1000, // Convert to
																	// milliseconds
					"value", v.getValue()))
			.toList();
	}
