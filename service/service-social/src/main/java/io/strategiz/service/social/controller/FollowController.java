...
import io.strategiz.data.social.entity.UserFollowEntity;
 ...
 *
 * Endpoints: - POST /v1/social/users/{userId}/follow - Follow a user - DELETE
 * /v1/social/users/{userId}/follow - Unfollow a user - GET
 * /v1/social/users/{userId}/followers - Get user's followers - GET
 * /v1/social/users/{userId}/following - Get who user follows - GET
 * /v1/social/users/{userId}/follow-status - Check if following - GET
 * /v1/social/users/{userId}/stats - Get follower/following counts
 */
@RestController
...
public class FollowController extends BaseController {
	...
	@CrossOrigin(origins = { "http://localhost:3000", "http://localhost:3001", "https://strategiz.io" },
			allowedHeaders = "*")
	public ResponseEntity<Object> followUser(@PathVariable String userId, @AuthUser AuthenticatedUser user) {
		try {
			String currentUserId = user.getUserId();

			UserFollowEntity follow = followService.follow(currentUserId, userId);
			return ResponseEntity.status(HttpStatus.CREATED)
				.body(Map.of("message", "Successfully followed user", "follow", follow));
		}
		catch (Exception e) {
	...
	@CrossOrigin(origins = { "http://localhost:3000", "http://localhost:3001", "https://strategiz.io" },
			allowedHeaders = "*")
	public ResponseEntity<Object> unfollowUser(@PathVariable String userId, @AuthUser AuthenticatedUser user) {
		try {
			String currentUserId = user.getUserId();
	...
	public ResponseEntity<Object> getFollowers(@PathVariable String userId,
		...
		try {
			...
			return ResponseEntity.ok(Map.of("followers", followers, "count", followers.size()));
		}
		catch (Exception e) {
	...
	@CrossOrigin(origins = { "http://localhost:3000", "http://localhost:3001", "https://strategiz.io" },
			allowedHeaders = "*")
	public ResponseEntity<Object> getFollowing(@PathVariable String userId,
			@RequestParam(defaultValue = "50") int limit) {
		try {
			List<UserFollowEntity> following = followService.getFollowing(userId, limit);
			return ResponseEntity.ok(Map.of("following", following, "count", following.size()));
		}
		catch (Exception e) {
	...
	@CrossOrigin(origins = { "http://localhost:3000", "http://localhost:3001", "https://strategiz.io" },
			allowedHeaders = "*")
	public ResponseEntity<Object> getFollowStatus(@PathVariable String userId, @AuthUser AuthenticatedUser user) {
		try {
			String currentUserId = user.getUserId();

			boolean isFollowing = followService.isFollowing(currentUserId, userId);
			return ResponseEntity.ok(Map.of("isFollowing", isFollowing, "userId", userId));
		}
		catch (Exception e) {
