...
/**
 * Service for sending alert notifications through multiple channels. Supports email
 * (SendGrid), push notifications (FCM), and in-app notifications (Firestore).
 */
@Service
public class AlertNotificationService extends BaseService {
	...
	@Autowired
	public AlertNotificationService(Firestore firestore,
	 ...
	 */
	public void sendSignalNotification(AlertDeployment alert, ExecutionResult.Signal signal, String symbol,
			Double currentPrice) {

		log.info("Sending {} signal notification for alert {} (symbol: {}, price: ${})", signal.getType(),
				alert.getAlertName(), symbol, currentPrice);
		...
		for (String channel : alert.getNotificationChannels()) {
			try {
				...
				log.error("Failed to send {} notification for alert {}: {}", channel, alert.getId(), e.getMessage(), e);
			}
		}
	}
	 ...
	 */
	private void sendEmailNotification(AlertDeployment alert, ExecutionResult.Signal signal, String symbol,
			Double currentPrice) {
		...
		try {
			...
			if (userEmail == null || userEmail.isEmpty()) {
				log.warn("No email address available for user {}, skipping email notification", alert.getUserId());
				return;
			}
			...
			String htmlBody = String.format(
					"""
							...
							""",
					alert.getAlertName(), signal.getType().equals("BUY") ? "#10b981" : "#ef4444", signal.getType(),
					symbol, currentPrice, signal.getReason() != null ? signal.getReason() : "Strategy logic triggered",
			...
			if (response.getStatusCode() >= 200 && response.getStatusCode() < 300) {
				log.info("Email notification sent successfully to {} for {} signal on {}", userEmail, signal.getType(),
						symbol);
			}
			else {
				log.error("Failed to send email notification. Status: {}, Body: {}", response.getStatusCode(),
						response.getBody());
			}
	 ...
	 */
	private void sendSmsNotification(AlertDeployment alert, ExecutionResult.Signal signal, String symbol,
			Double currentPrice) {
		...
		try {
			...
			String message = String.format("[Strategiz] %s %s signal for %s at $%.2f. Alert: %s", signalEmoji,
					signal.getType(), symbol, currentPrice, alert.getAlertName());
			...
			if (result.isSuccess()) {
				...
				log.error("Failed to send SMS notification. Status: {}, Error: {}", result.getStatus(),
	 ...
	 */
	private void sendPushNotification(AlertDeployment alert, ExecutionResult.Signal signal, String symbol,
			Double currentPrice) {
		...
		try {
			// COMING SOON: Push notifications require mobile app with device token
			// registration
			...
			// 3. Each device document should contain: { token, platform, appVersion,
			// createdAt }

			log.info("Push notification SKIPPED - mobile app not yet available. "
					+ "Would send {} signal for {} to user {}", signal.getType(), symbol, alert.getUserId());
			return;

			// TODO: Uncomment when mobile app is ready:
			// String title = String.format("%s Signal: %s", signal.getType(), symbol);
			// String body = String.format("Your alert \"%s\" triggered at $%.2f",
			// alert.getAlertName(), currentPrice);
			//
	 ...
	 */
	private void sendInAppNotification(AlertDeployment alert, ExecutionResult.Signal signal, String symbol,
			Double currentPrice) {
		...
		try {
			...
			log.info("In-app notification created for user {} - {} signal on {}", alert.getUserId(), signal.getType(),
					symbol);
	...
	/**
	 * Resolve user email for notifications. Priority: alert.notificationEmail >
	 * preferences.emailForAlerts > user.profile.email
	...
	private String resolveUserEmail(AlertDeployment alert) {
		...
		if (alertPreferencesService != null) {
			try {
				...
				log.warn("Failed to fetch alert email from preferences for user {}: {}", alert.getUserId(),
						e.getMessage());
			}
		}
		...
		if (userRepository != null) {
			try {
				...
				log.warn("Failed to fetch user profile email for user {}: {}", alert.getUserId(), e.getMessage());
			}
		}
