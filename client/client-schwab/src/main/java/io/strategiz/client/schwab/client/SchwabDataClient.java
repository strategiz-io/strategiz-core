...
/**
 * Client for fetching data from Charles Schwab using OAuth access tokens.
 * This class handles all account, position, and transaction data retrieval operations.
 *
 * API Documentation: https://developer.schwab.com/products/trader-api--individual
 *
 * Key endpoints:
 * - /trader/v1/accounts - List all accounts
...
public class SchwabDataClient {
    ...
    public List<Map<String, Object>> getAccounts(String accessToken) {
        Object response = makeAuthenticatedRequest(HttpMethod.GET,
            TRADER_API_BASE + "/accounts", accessToken, null);
        return extractAccountsList(response);
    }

    /**
     * Get all accounts with positions included.
     * This is the primary method for fetching portfolio data.
     *
     * @param accessToken OAuth access token
    ...
    public List<Map<String, Object>> getAccountsWithPositions(String accessToken) {
        Object response = makeAuthenticatedRequest(HttpMethod.GET,
            TRADER_API_BASE + "/accounts?fields=positions", accessToken, null);
        return extractAccountsList(response);
    }
    ...
    public Map<String, Object> getAccountBalance(String accessToken, String accountHash) {
        Object response = makeAuthenticatedRequest(HttpMethod.GET,
            TRADER_API_BASE + "/accounts/" + accountHash, accessToken, null);
        return extractSingleAccount(response);
    }

    /**
     * Get transactions for an account.
     * Note: Schwab only allows fetching up to 1 year of transaction history.
     *
     * @param accessToken OAuth access token
     * @param accountHash The account hash/ID
     ...
     * @param types Transaction types to filter (optional, e.g., "TRADE", "DIVIDEND_OR_INTEREST")
    ...
    public List<Map<String, Object>> getTransactions(String accessToken, String accountHash,
        ...
        try {
            ...
            Object response = makeAuthenticatedRequest(HttpMethod.GET,
                TRADER_API_BASE + "/accounts/" + accountHash + "/transactions",
                accessToken, params);
    ...
    public Map<String, Object> getTransaction(String accessToken, String accountHash, String transactionId) {
        Object response = makeAuthenticatedRequest(HttpMethod.GET,
            TRADER_API_BASE + "/accounts/" + accountHash + "/transactions/" + transactionId,
            accessToken, null);
        return extractSingleAccount(response);
    }
    ...
    public Map<String, Object> getQuote(String accessToken, String symbol) {
        ...
        Object response = makeAuthenticatedRequest(HttpMethod.GET,
            MARKET_DATA_BASE + "/quotes", accessToken, params);
     ...
     * Convenience method that extracts just the lastPrice from quote data.
     *
     * @param accessToken OAuth access token
     ...
     */
    private Object makeAuthenticatedRequest(HttpMethod method, String endpoint,
        ...
        try {
            ...
            if (response.getBody() == null) {
                throw new StrategizException(SchwabErrors.SCHWAB_INVALID_RESPONSE,
                    "Empty response from Schwab API");
            }
            ...
            if (statusCode == 400) {
                String errorMsg = responseBody != null && !responseBody.isEmpty()
                    ? responseBody
                    : "Bad request - check API parameters and authentication";
                throw new StrategizException(SchwabErrors.SCHWAB_API_ERROR,
                    "Schwab API error (400): " + errorMsg);
            }

            throw new StrategizException(SchwabErrors.SCHWAB_API_ERROR,
                "Schwab API error: " + (responseBody != null && !responseBody.isEmpty() ? responseBody : "HTTP " + statusCode));
     ...
     * Schwab returns accounts as a list directly or wrapped in an object.
     */
