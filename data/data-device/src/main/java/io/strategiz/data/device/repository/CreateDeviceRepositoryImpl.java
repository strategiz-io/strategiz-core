...
/**
 * Implementation of CreateDeviceRepository that directly connects to Firestore THIS is
 * where the database connection happens - IN the data module!
 */
@Repository
public class CreateDeviceRepositoryImpl implements CreateDeviceRepository {
	...
	public DeviceIdentity createAuthenticatedDevice(DeviceIdentity device, String userId) {
		...
		try {
			...
			throw new StrategizException(DataRepositoryErrorDetails.ENTITY_SAVE_FAILED, "data-device", e,
					"DeviceIdentity", device.getDeviceId());
		}
	}
