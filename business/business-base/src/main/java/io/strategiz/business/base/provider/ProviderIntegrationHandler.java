...
public interface ProviderIntegrationHandler {
    ...
    /**
     * Test connection to the provider using stored credentials.
     * For API key providers: Retrieves credentials from Vault and tests the connection.
     * For OAuth providers: Validates the OAuth token stored in Vault.
     * 
     * @param request Provider integration request (credentials may be ignored for API providers
     *                as they use stored credentials from Vault)
     ...
     * @return Integration result with metadata (e.g., success status, OAuth URL for OAuth providers)
     */
