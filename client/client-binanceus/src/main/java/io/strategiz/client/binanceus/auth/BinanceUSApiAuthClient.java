...
/**
 * Binance US API Authentication Client Handles API key authentication and request signing
 * for Binance US API
 */
@Service
public class BinanceUSApiAuthClient {
	...
	public Mono<Map<String, Object>> makeAuthenticatedRequest(String endpoint, String apiKey, String apiSecret,
		...
		if (params != null && !params.isEmpty()) {
			params.forEach((key, value) -> queryString.append("&").append(key).append("=").append(value));
		}
		...
		// Make request
		WebClient webClient = webClientBuilder.baseUrl(BINANCE_US_API_BASE_URL).build();
	...
	public Mono<Map<String, Object>> getAccountBalances(String apiKey, String apiSecret) {
		return makeAuthenticatedRequest("/api/v3/account", apiKey, apiSecret, null).map(response -> {
	...
	public Mono<Map<String, Object>> getTradeHistory(String apiKey, String apiSecret, String symbol, Integer limit) {
		Map<String, String> params = Map.of("symbol", symbol, "limit", limit != null ? limit.toString() : "500");
