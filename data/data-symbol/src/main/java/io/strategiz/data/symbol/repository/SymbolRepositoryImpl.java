...
/**
 * Firestore implementation of SymbolRepository extending BaseRepository. Symbols are
 * system-wide (not user-scoped), so we use "SYSTEM" as the userId.
 */
@Repository
public class SymbolRepositoryImpl extends BaseRepository<SymbolEntity> implements SymbolRepository {
	...
	public List<SymbolEntity> saveAll(List<SymbolEntity> entities) {
		...
		try {
			...
			throw new StrategizException(DataRepositoryErrorDetails.BULK_OPERATION_FAILED, "data-symbol", e,
					"SymbolEntity", entities.size());
		}
	}
	...
	public Optional<SymbolEntity> findByProviderSymbol(String provider, String providerSymbol) {
		try {
			// Query for symbols where providerSymbols map contains the provider key with
			// matching value
			// Firestore supports querying map fields using dot notation
			String fieldPath = "providerSymbols." + provider.toUpperCase();

			Query query = getCollection().whereEqualTo(fieldPath, providerSymbol)
	...
	public List<SymbolEntity> findByAssetType(String assetType) {
		try {
			Query query = getCollection().whereEqualTo("assetType", assetType.toUpperCase())
				.whereEqualTo("isActive", true)
				.orderBy("id");
	...
	public List<SymbolEntity> findActiveForCollection() {
		try {
			Query query = getCollection().whereEqualTo("collectionActive", true)
	...
	public List<SymbolEntity> findByCategory(String category) {
		try {
			Query query = getCollection().whereEqualTo("category", category.toLowerCase())
				.whereEqualTo("isActive", true)
				.orderBy("id");
