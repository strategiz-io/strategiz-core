...
public class PasskeySignatureVerifier {
	 ...
	 */
	public static boolean verifySignature(String publicKeyBase64, String authenticatorData, String clientDataJSON,
			String signatureBase64) {

		try {
			log.info("=== Starting signature verification ===");
			log.info("Public key (first 50 chars): {}",
			...
			log.info("Raw authenticator data (first 50 chars): {}",
					authenticatorData.substring(0, Math.min(50, authenticatorData.length())));
			log.info("Raw signature (first 50 chars): {}",
					signatureBase64.substring(0, Math.min(50, signatureBase64.length())));
			...
			log.info("Signature first 4 bytes: {}",
					Arrays.toString(Arrays.copyOf(signatureBytes, Math.min(4, signatureBytes.length))));
			...
			log.info("  - Client data hash offset: {} to {}", authenticatorDataBytes.length,
	...
	private static PublicKey convertCoseKeyToPublicKey(COSEKey coseKey)
		...
		if (coseKey instanceof EC2COSEKey) {
			...
			// Get curve parameters for P-256 (secp256r1) - same method as main
			// verification
	...
	private static byte[] hashClientData(String clientDataJSON) {
		try {
			...
			throw new StrategizException(ServiceAuthErrorDetails.CONFIGURATION_ERROR, "service-auth", e,
