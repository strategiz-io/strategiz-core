 ...
 *
 * Features: - Automatic pagination (10,000 bars per request limit) - Supports multiple
 * timeframes (1Min, 5Min, 15Min, 1Hour, 1Day) - Rate limiting with configurable delay
 */
@Component
public class AlpacaHistoricalClient {
	...
	public void init() {
		...
		try {
			...
			log.error("Failed to load Alpaca credentials from Vault: {} - AlpacaHistoricalClient will be unavailable",
					e.getMessage());
			return;
		}
		...
		log.info("API Key: {}", apiKey != null && !apiKey.isEmpty()
				? apiKey.substring(0, Math.min(8, apiKey.length())) + "..." : "NOT SET");
		log.info("API Secret: {}",
				apiSecret != null && !apiSecret.isEmpty() ? "SET (length: " + apiSecret.length() + ")" : "NOT SET");
		log.info("Feed: {}", feed);
	...
	private HttpHeaders createHeaders() {
		...
		log.debug("Created headers with API Key: {}",
				apiKey != null && !apiKey.isEmpty() ? apiKey.substring(0, 8) + "..." : "MISSING");
		return headers;
	}
	...
	public List<AlpacaBar> getBars(String symbol, LocalDateTime startDate, LocalDateTime endDate, String timeframe) {
		if (!isAvailable()) {
			throw new StrategizException(AlpacaErrorDetails.CONFIGURATION_ERROR, MODULE_NAME,
					"AlpacaHistoricalClient is not available - credentials not configured");
		}
		...
		do {
			try {
				...
				throw new StrategizException(AlpacaErrorDetails.PAGINATION_ERROR, MODULE_NAME, e,
	...
	private AlpacaBarsResponse getBarsPage(String symbol, LocalDateTime startDate, LocalDateTime endDate,
		...
		StringBuilder path = new StringBuilder(
				String.format("/v2/stocks/%s/bars?start=%s&end=%s&timeframe=%s&limit=10000&feed=%s", symbol, start, end,
						timeframe, feed));
		...
		while (retryCount <= maxRetries) {
			try {
				...
				ResponseEntity<String> responseEntity = restTemplate.exchange(fullUrl, HttpMethod.GET, entity,
						String.class);

				String responseBody = responseEntity.getBody();
				log.debug(">>> ALPACA RAW RESPONSE (first 500 chars): {}", responseBody != null
						? responseBody.substring(0, Math.min(500, responseBody.length())) : "NULL");

				if (responseBody == null || responseBody.isEmpty()) {
					throw new StrategizException(AlpacaErrorDetails.NO_DATA_AVAILABLE, MODULE_NAME,
				...
				log.warn("Rate limited (429) for {}, attempt {}/{}, waiting {}ms before retry", symbol, retryCount,
				...
				try {
					...
					Thread.currentThread().interrupt();
					throw new StrategizException(AlpacaErrorDetails.PAGINATION_ERROR, MODULE_NAME, ie,
							"Thread interrupted during rate limit backoff");
				}
			}
			catch (Exception e) {
				log.error("Error fetching bars for {}: {}", symbol, e.getMessage());
				throw new StrategizException(AlpacaErrorDetails.API_ERROR_RESPONSE, MODULE_NAME, e,
						String.format("Failed to fetch bars for symbol: %s", symbol));
			}
		}

		// All retries exhausted
		throw new StrategizException(AlpacaErrorDetails.API_ERROR_RESPONSE, MODULE_NAME, lastException,
				String.format("Failed to fetch bars for %s after %d retries due to rate limiting", symbol, maxRetries));
	}

	/**
	 * Get latest quotes for multiple stock symbols. Uses Alpaca's snapshots endpoint for
	 * accurate daily change calculation. Snapshots include prevDailyBar which gives us
	 * yesterday's close for proper % change.
	...
	public Map<String, LatestQuote> getLatestStockQuotes(List<String> symbols) {
		...
		try {
			...
			// Use snapshots endpoint which includes prevDailyBar for accurate daily
			// change
			...
			// Parse the response - snapshots returns { "AAPL": { "latestTrade": {...},
			...
			for (Map.Entry<String, Object> entry : snapshots.entrySet()) {
				...
				try {
					...
					log.debug("Snapshot for {}: price={}, prevClose={}, change={}%, volume={}", symbol, currentPrice,
							prevClose, changePercent, volume);
	...
	/**
	 * Fetch all historical crypto bars with automatic pagination. Uses Alpaca's crypto
	 * data endpoint: /v1beta3/crypto/us/bars
	 ...
	 */
	public List<AlpacaBar> getCryptoBars(String symbol, LocalDateTime startDate, LocalDateTime endDate,
			String timeframe) {
		if (!isAvailable()) {
			throw new StrategizException(AlpacaErrorDetails.CONFIGURATION_ERROR, MODULE_NAME,
					"AlpacaHistoricalClient is not available - credentials not configured");
		...
		log.debug("Fetching crypto bars for {} ({}) from {} to {} ({})", symbol, cryptoPair, startDate, endDate,
				timeframe);
		...
		do {
			try {
				...
				AlpacaBarsResponse response = getCryptoBarsPage(cryptoPair, symbol, startDate, endDate, timeframe,
						nextPageToken);

				if (response.getBars() != null && !response.getBars().isEmpty()) {
					allBars.addAll(response.getBars());
					log.debug("Page {}: fetched {} crypto bars for {}", pageCount + 1, response.getBars().size(),
							symbol);
				}
	...
	private AlpacaBarsResponse getCryptoBarsPage(String cryptoPair, String canonicalSymbol, LocalDateTime startDate,
		...
		StringBuilder path = new StringBuilder(
				String.format("/v1beta3/crypto/us/bars?symbols=%s&start=%s&end=%s&timeframe=%s&limit=10000", cryptoPair,
						start, end, timeframe));
		...
		while (retryCount <= maxRetries) {
			try {
				...
				String responseBody = responseEntity.getBody();
				log.debug(">>> ALPACA CRYPTO RAW RESPONSE (first 500 chars): {}", responseBody != null
						? responseBody.substring(0, Math.min(500, responseBody.length())) : "NULL");
				...
				retryCount++;
				if (retryCount > maxRetries) {
					log.error("Max retries ({}) exceeded for crypto {} - rate limit still in effect", maxRetries,
							canonicalSymbol);
					break;
				}

				long backoffMs = retryDelayMs * (long) Math.pow(2, retryCount - 1);
				log.warn("Rate limited (429) for crypto {}, attempt {}/{}, waiting {}ms before retry", canonicalSymbol,
		...
		throw new StrategizException(AlpacaErrorDetails.API_ERROR_RESPONSE, MODULE_NAME, lastException,
				String.format("Failed to fetch crypto bars for %s after %d retries due to rate limiting",
						canonicalSymbol, maxRetries));
	}

	/**
	 * Get latest quotes for crypto symbols. Uses Alpaca's crypto snapshots endpoint for
	 * accurate daily change calculation.
	...
	public Map<String, LatestQuote> getLatestCryptoQuotes(List<String> symbols) {
		...
		try {
			...
			List<String> cryptoPairs = symbols.stream().map(s -> s + "/USD").toList();
			...
			// Parse the response - crypto snapshots returns { "snapshots": { "BTC/USD": {
			// "latestTrade": {...}, "prevDailyBar": {...} }, ... } }
			...
			if (snapshots != null) {
				for (Map.Entry<String, Object> entry : snapshots.entrySet()) {
					...
					try {
						...
						result.put(symbol, new LatestQuote(symbol, currentPrice, change, changePercent, volume));
						log.debug("Crypto snapshot for {}: price={}, prevClose={}, change={}%, volume={}", symbol,
								currentPrice, prevClose, changePercent, volume);
