 ...
 *
 * Uses the userEmails collection where document ID = normalized email.
 * This leverages Firestore's native document ID uniqueness guarantee.
 *
 * Transaction-aware: When a Firestore transaction is active (via FirestoreTransactionHolder),
 * operations participate in that transaction for atomic user creation.
 */
@Repository
public class EmailReservationRepositoryImpl implements EmailReservationRepository {
    ...
    private EmailReservationEntity reserveInTransaction(Transaction transaction, DocumentReference docRef,
                                                         EmailReservationEntity reservation, String email)
    ...
    public EmailReservationEntity confirm(String email) {
        ...
        try {
            ...
            throw new DataRepositoryException(DataRepositoryErrorDetails.ENTITY_UPDATE_FAILED, e,
                "EmailReservation", normalizedEmail);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new DataRepositoryException(DataRepositoryErrorDetails.FIRESTORE_OPERATION_INTERRUPTED, e,
    ...
    private EmailReservationEntity confirmInTransaction(Transaction transaction, DocumentReference docRef,
                                                         String email) throws InterruptedException, ExecutionException {
        DocumentSnapshot doc = transaction.get(docRef).get();
        ...
        if (reservation == null) {
            throw new DataRepositoryException(DataRepositoryErrorDetails.ENTITY_CONVERSION_FAILED,
                "EmailReservation", email);
        }
    ...
    private String normalizeEmail(String email) {
        if (email == null) {
            throw new DataRepositoryException(DataRepositoryErrorDetails.INVALID_ARGUMENT,
                "EmailReservation", "Email cannot be null");
        }
