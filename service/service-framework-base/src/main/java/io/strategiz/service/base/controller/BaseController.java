 ...
 *
 * Controllers should: - Return clean data objects with ResponseEntity.ok(data) - Throw
 * StrategizException for business errors (handled by GlobalExceptionHandler) - Use
 * response filtering to remove null/empty fields - Follow consistent logging patterns
 *
 * For authentication-specific functionality, extend BaseAuthenticationController instead.
 */
@OpenAPIDefinition(info = @Info(title = "Strategiz Core API", version = "1.0",
		description = "Core backend API for Strategiz platform providing authentication, user management, and business logic services.",
		contact = @Contact(name = "Strategiz Development Team", email = "dev@strategiz.io")),
...
public abstract class BaseController {
	...
	/**
	 * Get the module name for this controller. Each controller must implement this to
	 * identify its module.
	...
	/**
	 * Spring-managed ObjectMapper with global configuration (NON_NULL, date/time support,
	 * etc.) This ensures consistent JSON serialization across all controllers.
	 */
	@Autowired
	...
	protected void validateAlphanumeric(String input, String fieldName) {
		if (input == null || !ALPHANUMERIC_PATTERN.matcher(input).matches()) {
			throwModuleException(ServiceBaseErrorDetails.VALIDATION_ERROR, fieldName,
	...
	protected void validateRequiredParam(String paramName, Object paramValue) {
		...
		if (paramValue instanceof String && ((String) paramValue).trim().isEmpty()) {
			throwModuleException(ServiceBaseErrorDetails.VALIDATION_ERROR, paramName,
	...
	protected void checkRateLimit(String userId, String operation) {
		...
		// Example: throwModuleException(ServiceBaseErrorDetails.TOO_MANY_REQUESTS,
		// operation);
	}
	...
	protected void logRequestSuccess(String operation, String userId, Object result) {
		log.info("[{}] SUCCESS - Operation: {}, User: {}, Result: {}", getModuleName(), operation, userId,
	...
	protected void logRequestSuccess(String operation, String userId, Object result, String context) {
		log.info("[{}] SUCCESS - Operation: {}, User: {}, Result: {}, Context: {}", getModuleName(), operation, userId,
				result.getClass().getSimpleName(), context);
	}
	...
	protected Map<String, Object> createErrorContext(String operation, String userId, String errorType) {
		return Map.of("operation", operation, "userId", userId != null ? userId : "unknown", "errorType", errorType,
				"timestamp", System.currentTimeMillis());
	}

	/**
	 * Handle StrategizException (business exceptions) in controllers This is the main
	 * exception handler for the hybrid approach
	 */
	...
	public ResponseEntity<StandardErrorResponse> handleStrategizException(StrategizException ex) {
		...
		log.warn("Business exception [{}] in module [{}]: {}", traceId, ex.getModuleName(), ex.getErrorCode());
	 ...
	 * This method is used by controllers to convert generic exceptions to business
	 * exceptions
	...
	protected StrategizException handleException(Exception e, String errorKey) {
		...
		ErrorDetails errorDetails = new ErrorDetails() {
			...
			public HttpStatus getHttpStatus() {
				...
				else if (errorKey.contains("creation-failed") || errorKey.contains("update-failed")
						|| errorKey.contains("deletion-failed")) {
	...
	/**
	 * Fallback exception handler for unexpected exceptions GlobalExceptionHandler should
	 * handle most cases, but this provides a safety net
	 */
	...
	public ResponseEntity<StandardErrorResponse> handleUnexpectedException(Exception ex) {
		...
		StandardErrorResponse errorResponse = new StandardErrorResponse("INTERNAL_ERROR",
	...
	// These methods provide HTTP-specific logging using the StructuredLogger from
	// framework-logging
	 ...
	 */
	protected void logHttpResponse(String method, String endpoint, String userId, int statusCode, long durationMs,
	 ...
	 */
	protected void logApiAuthz(String userId, String operation, String resourceId, boolean success,
			Map<String, Object> authzData) {
