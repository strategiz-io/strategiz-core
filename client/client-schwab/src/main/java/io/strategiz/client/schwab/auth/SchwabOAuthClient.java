...
/**
 * Client for handling Charles Schwab OAuth operations. This class handles OAuth token
 * exchange, refresh, and revocation.
 *
 * Note: Schwab uses Basic Authentication for token requests with client credentials in
 * the Authorization header, not in the request body.
 *
 * Token expiration: - Access tokens expire after 30 minutes - Refresh tokens expire after
 * 7 days
 */
@Component
public class SchwabOAuthClient {
	...
	/**
	 * Exchange authorization code for access token. Schwab requires Basic Authentication
	 * with client credentials in header.
	...
	public Map<String, Object> exchangeCodeForTokens(String authorizationCode) {
		try {
			...
			log.info("Token exchange config - client_id: {}, redirect_uri: {}, token_url: {}",
					clientId != null ? clientId.substring(0, 8) + "..." : "null", redirectUri, tokenUrl);
			...
			if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
				...
				throw new StrategizException(SchwabErrors.SCHWAB_AUTH_FAILED, "Failed to exchange authorization code");
			}

		}
		catch (RestClientResponseException e) {
			log.error("Schwab OAuth token exchange failed with status {}: {}", e.getStatusCode(),
	...
	/**
	 * Refresh access token using refresh token. Should be called every 29 minutes to keep
	 * the access token valid. Note: Refresh tokens expire after 7 days and require
	 * re-authentication.
	...
	public Map<String, Object> refreshAccessToken(String refreshToken) {
		try {
			...
			if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
				...
				throw new StrategizException(SchwabErrors.SCHWAB_TOKEN_EXPIRED, "Failed to refresh access token");
			}

		}
		catch (RestClientResponseException e) {
			log.error("Schwab token refresh failed with status {}: {}", e.getStatusCode(), e.getResponseBodyAsString());
	...
	public String generateAuthorizationUrl(String state) {
		...
		return String.format("%s/oauth/authorize?client_id=%s&redirect_uri=%s&response_type=code&state=%s&scope=%s",
				AUTH_BASE_URL, clientId, redirectUri, state, scope);
	}
