...
/**
 * Enricher responsible for detecting and enriching staking information. Identifies staked
 * assets and adds APR information.
 *
...
public class KrakenStakingEnricher {
		...
		for (Map.Entry<String, Object> entry : rawBalances.entrySet()) {
			...
			if (symbolEnricher.isStakedAsset(originalSymbol)) {
				...
				if (assetInfo != null) {
					...
					log.debug("Enriched staking info for {}: APR {}%, Period: {}", normalizedSymbol, apr, period);
				}
			}
		}
	}
	...
	/**
	 * Get staking APR for an asset. These are approximate current rates as of 2025.
	 */
	private BigDecimal getStakingAPR(String symbol, String stakingType) {
		...
		Map<String, BigDecimal> stakingAPRs = Map.ofEntries(
				...
				Map.entry("ETH", new BigDecimal(isLiquid ? "3.5" : "4.0")), Map.entry("STETH", new BigDecimal("3.5")),
				Map.entry("RETH", new BigDecimal("3.3")), Map.entry("CBETH", new BigDecimal("3.2")),
				...
				Map.entry("GRT", new BigDecimal(isLiquid ? "8.0" : "10.0")), Map.entry("LDO", new BigDecimal("4.0")),
	...
	private String getStakingPeriod(String symbol, String stakingType) {
		...
		Map<String, String> stakingPeriods = Map.ofEntries(
				...
				Map.entry("DOT", "28 days unbonding"), Map.entry("KSM", "7 days unbonding"),
				Map.entry("ATOM", "21 days unbonding"), Map.entry("ADA", "Flexible"),
				Map.entry("SOL", "2-3 days warmup/cooldown"), Map.entry("AVAX", "14 days minimum"),
				Map.entry("NEAR", "2-3 days unbonding"), Map.entry("FTM", "7 days unbonding"),
				Map.entry("ALGO", "Flexible"), Map.entry("XTZ", "Flexible"), Map.entry("MATIC", "3-4 days unbonding"),
				Map.entry("TRX", "3 days unbonding"),

				// Cosmos ecosystem typically 21 days
				Map.entry("OSMO", "14 days unbonding"), Map.entry("JUNO", "28 days unbonding"),
				Map.entry("SCRT", "21 days unbonding"), Map.entry("AKT", "21 days unbonding"),
				Map.entry("INJ", "21 days unbonding"), Map.entry("TIA", "21 days unbonding"),
				Map.entry("DYM", "21 days unbonding"), Map.entry("SEI", "21 days unbonding"),
