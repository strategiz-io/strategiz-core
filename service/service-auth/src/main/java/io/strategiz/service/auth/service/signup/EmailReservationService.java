 ...
 * reservations in the userEmails collection before user creation. The document ID is the
 * normalized email, leveraging Firestore's native uniqueness guarantee.
 *
 * Flow: 1. initiateSignup: reserveEmail() creates PENDING reservation, returns
 * pre-generated userId 2. User completes verification (OTP, OAuth callback, etc.) 3.
 * verifySignup: confirmReservation() + create user in single transaction 4. On failure:
 * releaseReservation() cleans up 5. Scheduled job: cleans up expired PENDING reservations
 *
 * This prevents race conditions where two users could sign up with the same email
 * simultaneously, with both passing the initial email check but only one succeeding in
 * user creation.
 */
@Service
public class EmailReservationService extends BaseService {
	...
	public EmailReservationService(EmailReservationRepository emailReservationRepository,
	...
	/**
	 * Reserve an email address for signup. This should be called at the start of the
	 * signup flow before sending OTP.
	 *
	 * The method generates a userId that should be used throughout the signup process to
	 * ensure consistency.
	...
	public String reserveEmail(String email, String signupType, String sessionId) {
		...
		// First check if a user with this email already exists (for lazy migration
		// support)
		...
		try {
			EmailReservationEntity reservation = new EmailReservationEntity(normalizedEmail, userId, signupType,
					sessionId);
	...
	/**
	 * Reserve an email with a pre-specified userId. Useful for OAuth flows where the
	 * userId might be determined by the provider.
	 * @param email The email address to reserve
	...
	/**
	 * Create a CONFIRMED email reservation directly (single write, no read). Used after
	 * OTP verification to create the userEmails document atomically with user creation
	...
	public void createConfirmedReservation(String email, String userId, String signupType) {
		...
		EmailReservationEntity reservation = new EmailReservationEntity(normalizedEmail, userId, signupType, null);
		emailReservationRepository.createConfirmed(reservation);
		log.info("Confirmed email reservation created: {}", normalizedEmail);
	}

	/**
	 * Confirm an email reservation during signup completion. This should be called within
	...
	/**
	 * Release (delete) an email reservation. Call this when signup fails or is cancelled
	 * to free up the email.
	...
	/**
	 * Check if an email is available for registration. This performs both user lookup and
	 * reservation check.
	...
	/**
	 * Scheduled job to clean up expired PENDING reservations. Runs every 5 minutes.
	 */
