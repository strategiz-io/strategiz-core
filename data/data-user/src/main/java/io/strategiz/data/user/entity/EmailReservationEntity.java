...
/**
 * Entity for email reservations used to ensure email uniqueness during signup.
 * The document ID is the normalized lowercase email, which leverages Firestore's
 * native document ID uniqueness to prevent duplicate registrations.
 *
 * Stored in Firestore collection: userEmails
 *
 * Flow:
 * 1. User initiates signup - PENDING reservation created
 * 2. If reservation fails (doc exists) - email already taken
 * 3. User completes verification
 * 4. Reservation confirmed within transaction along with user creation
 ...
 * since reservations are either ephemeral (PENDING) or permanent (CONFIRMED).
 */
public class EmailReservationEntity {

    /**
     * The normalized lowercase email address.
     * This is also the document ID, guaranteeing uniqueness.
     */
    @DocumentId
    private String email;

    /**
     * Pre-generated UUID for the user account.
     * Generated early so it can be used consistently throughout signup.
     */
    ...
    /**
     * Epoch second when a PENDING reservation expires.
     * After this time, the reservation can be cleaned up.
     */
    private long expiresAtEpochSecond;

    /**
     * Epoch second when the reservation was confirmed.
     * Only set when status is CONFIRMED.
     */
    private Long confirmedAtEpochSecond;

    /**
     * Type of signup that created this reservation.
     * Examples: "email_otp", "oauth_google", "oauth_facebook"
     */
    private String signupType;

    /**
     * Session ID linking to the pending signup process.
     * Allows correlation with in-memory pending signup data.
     */
    ...
    public boolean isExpired() {
        return status == EmailReservationStatus.PENDING
            && Instant.now().getEpochSecond() > expiresAtEpochSecond;
    }
    ...
    public String toString() {
        return "EmailReservationEntity{" +
            ...
            (confirmedAtEpochSecond != null ? ", confirmedAt=" + Instant.ofEpochSecond(confirmedAtEpochSecond) : "") +
            '}';
    }
}
