...
public class ExecuteStrategyController extends BaseController {
	...
	private final io.strategiz.service.labs.service.StrategyExecutionService strategyExecutionService; // Service
																										// layer
																										// for
																										// gRPC
																										// execution
	...
	public ExecuteStrategyController(ExecutionEngineService executionEngineService,
			...
			ReadStrategyService readStrategyService, PythonStrategyExecutor pythonStrategyExecutor,
			BacktestCalculatorBusiness backtestCalculatorBusiness, MarketDataRepository marketDataRepository,
	...
	@Operation(summary = "Execute a strategy",
			description = "Executes a strategy with given parameters and returns signals/indicators")
	public ResponseEntity<ExecuteStrategyResponse> executeStrategy(@PathVariable String strategyId,
			@Valid @RequestBody ExecuteStrategyRequest request, @AuthUser AuthenticatedUser user) {

		String userId = user.getUserId();
		logger.info("Executing strategy: {} for user: {} with symbol: {}", strategyId, userId, request.getSymbol());
		...
		try {
			...
			// TODO Phase 8: Consider if execution should require subscription even for
			// PUBLIC strategies
			java.util.Optional<io.strategiz.data.strategy.entity.Strategy> strategyOpt = readStrategyService
			...
			io.strategiz.business.strategy.execution.model.ExecutionRequest executionRequest = buildExecutionRequest(
					request, strategy, userId);
	...
	public ResponseEntity<ExecuteStrategyResponse> executeCode(@Valid @RequestBody ExecuteStrategyRequest request,
			@AuthUser AuthenticatedUser user) {

		String userId = user.getUserId();
		logger.info("Executing code for user: {} with language: {} and symbol: {}", userId, request.getLanguage(),
				request.getSymbol());
		...
		try {
			...
			if ("python".equalsIgnoreCase(request.getLanguage())) {
				...
				if (symbol == null || symbol.trim().isEmpty()) {
					throwModuleException(ServiceStrategyErrorDetails.STRATEGY_EXECUTION_FAILED,
							"Symbol is required for strategy execution");
				}

				// Delegate to service layer (handles market data fetching, gRPC call, and
				// DTO mapping)
				ExecuteStrategyResponse response = strategyExecutionService.executeStrategy(request.getCode(), "python",
						symbol, request.getTimeframe() != null ? request.getTimeframe() : "1D",
						request.getPeriod() != null ? request.getPeriod() : "3y", // Default
																					// to
																					// 3
																					// years
						userId, null // No saved strategy for direct code execution
				);
			...
			io.strategiz.business.strategy.execution.model.ExecutionRequest executionRequest = buildDirectCodeExecutionRequest(
					request, userId);
			ExecutionResult result = executionEngineService.executeStrategy(executionRequest);
			ExecuteStrategyResponse response = convertToResponse(result, request);
	...
	@Operation(summary = "Get available data providers",
			description = "Returns list of available market data providers")
	...
	private io.strategiz.business.strategy.execution.model.ExecutionRequest buildExecutionRequest(
			ExecuteStrategyRequest request, io.strategiz.data.strategy.entity.Strategy strategy, String userId) {
		...
		// executionRequest.setMode(request.getMode() != null ? request.getMode() :
	...
	private io.strategiz.business.strategy.execution.model.ExecutionRequest buildDirectCodeExecutionRequest(
			ExecuteStrategyRequest request, String userId) {
		io.strategiz.business.strategy.execution.model.ExecutionRequest executionRequest = new io.strategiz.business.strategy.execution.model.ExecutionRequest();
	...
	/**
	 * Fetch real market data from the repository for strategy execution Fetches last 180
	 * days of daily OHLCV data for the specified symbol
	...
	private String fetchMarketDataForSymbol(String symbol) {
		try {
			...
			java.util.List<MarketDataEntity> marketData = marketDataRepository.findBySymbolAndDateRange(symbol,
					startDate, endDate);

			// If no data found, throw proper exception
			if (marketData == null || marketData.isEmpty()) {
				logger.warn("No market data found for symbol: {}", symbol);
				throwModuleException(ServiceStrategyErrorDetails.MARKET_DATA_NOT_FOUND, String.format(
						"No market data found for symbol: %s. Please ensure the batch job has run to populate data.",
						symbol));
			}
	...
	private ExecuteStrategyResponse.Performance convertBacktestPerformance(
		...
		if (backtest.getTrades() != null) {
			List<ExecuteStrategyResponse.Trade> trades = backtest.getTrades().stream().map(bt -> {
	...
	/**
	 * Fetch fundamentals data for strategy execution. Returns empty JSON object if no
	 * fundamentals are available (e.g., for crypto symbols).
