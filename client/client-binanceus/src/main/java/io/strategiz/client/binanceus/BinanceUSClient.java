...
/**
 * Client for interacting with the Binance US exchange API. Implements the ProviderClient
 * abstract class for standardized provider interactions. Ensures we ONLY work with real
...
public class BinanceUSClient extends ProviderClient {
	 ...
	 * Binance US does not have a sandbox/test mode, so we just check if credentials are
	 * valid.
	...
	/**
	 * Gets raw account data from Binance US API. This returns exactly what the API sends,
	 * ensuring we work with real data.
	...
	/**
	 * Gets raw position/balance data from Binance US API. This returns exactly what the
	 * API sends, ensuring we work with real data.
	...
	/**
	 * Check if the Binance US API is available. Uses a simple ping request to verify API
	 * availability.
