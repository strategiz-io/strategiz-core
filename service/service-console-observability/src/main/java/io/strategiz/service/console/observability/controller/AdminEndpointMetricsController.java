 ...
 *
 * <p>Provides availability and performance metrics for EVERY REST endpoint by querying
...
public class AdminEndpointMetricsController extends BaseController {
    ...
    @GetMapping
    @Operation(
        summary = "Get all endpoint metrics",
    ...
    public ResponseEntity<EndpointMetrics> getEndpoint(
            HttpServletRequest request,
            @Parameter(description = "HTTP method (GET, POST, etc.)")
            @RequestParam String method,
            @Parameter(description = "URI pattern (e.g., /v1/auth/login)")
    ...
    @Operation(
        summary = "Get aggregated metrics by service module",
    ...
    @Operation(
        summary = "Quick health check for critical endpoints",
