...
/**
 * Service for managing alert notification preferences. Handles phone verification,
 * channel configuration, and quiet hours.
 */
@Service
public class AlertPreferencesService {
	...
	public AlertNotificationPreferences updatePreferences(String userId, AlertNotificationPreferences preferences) {
		...
		if (preferences.getEnabledChannels() != null) {
			for (String channel : preferences.getEnabledChannels()) {
				if (!VALID_CHANNELS.contains(channel.toLowerCase())) {
					throw new StrategizException(PreferencesErrorDetails.INVALID_NOTIFICATION_CHANNEL,
							"business-preferences", channel);
				}
			}
		}
		...
		if (preferences.getPhoneNumber() != null && !preferences.getPhoneNumber().isEmpty()) {
			if (!isValidE164(preferences.getPhoneNumber())) {
				throw new StrategizException(PreferencesErrorDetails.INVALID_PHONE_NUMBER, "business-preferences",
	...
	/**
	 * Update phone number for SMS alerts. Marks phone as unverified until verification is
	 * completed.
	 * @param userId The user ID
	...
	/**
	 * Mark phone number as verified. Called after successful OTP verification.
	 * @param userId The user ID
	 * @return The updated preferences
	 */
	...
	public AlertNotificationPreferences updateChannels(String userId, List<String> channels) {
		...
		if (channels.contains("sms")) {
			...
			if (!current.isSmsConfigured()) {
				throw new StrategizException(PreferencesErrorDetails.SMS_NOT_CONFIGURED, "business-preferences",
	...
	private void validateQuietHours(AlertNotificationPreferences prefs) {
		if (prefs.getQuietHoursStart() == null || prefs.getQuietHoursEnd() == null) {
			throw new StrategizException(PreferencesErrorDetails.INVALID_QUIET_HOURS, "business-preferences",
		...
		try {
			...
			throw new StrategizException(PreferencesErrorDetails.INVALID_TIME_FORMAT, "business-preferences",
		...
		if (prefs.getQuietHoursTimezone() != null) {
			try {
				...
				throw new StrategizException(PreferencesErrorDetails.INVALID_TIMEZONE, "business-preferences",
