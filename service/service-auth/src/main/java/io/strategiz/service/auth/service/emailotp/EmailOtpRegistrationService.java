 ...
 *
 * Handles email address registration and verification for user accounts. This service
 * manages the setup and verification of email addresses as authentication methods.
 *
...
public class EmailOtpRegistrationService extends BaseService {
	...
	// In-memory store for registration OTP codes (replace with persistent storage in
	// production)
	...
	// Note: EmailOtpAuthenticationService dependency removed to avoid circular
	// dependencies

	/**
	 * Register an email address for a user account Sends verification OTP to confirm
	 * email ownership
	 */
	public boolean registerEmailAddress(String userId, String email) {
		...
		Optional<AuthenticationMethodEntity> existing = existingMethods.stream()
			.filter(method -> email
				.equals(method.getMetadataAsString(AuthenticationMethodMetadata.EmailOtpMetadata.EMAIL_ADDRESS)))
			.findFirst();
		...
		if (existing.isPresent()) {
			...
			boolean isVerified = Boolean.TRUE
				.equals(existingMethod.getMetadata(AuthenticationMethodMetadata.EmailOtpMetadata.IS_VERIFIED));
			if (isVerified) {
		...
		AuthenticationMethodEntity emailOtpMethod = new AuthenticationMethodEntity(AuthenticationMethodType.EMAIL_OTP,
				"Email: " + email);
		...
		emailOtpMethod.putMetadata(AuthenticationMethodMetadata.EmailOtpMetadata.LAST_EMAIL_DATE,
		...
		if (otpSent) {
			...
			saved.putMetadata(AuthenticationMethodMetadata.EmailOtpMetadata.LAST_EMAIL_SENT,
					java.time.Instant.now().toString());
			int currentCount = (Integer) saved.getMetadata()
				.getOrDefault(AuthenticationMethodMetadata.EmailOtpMetadata.DAILY_EMAIL_COUNT, 0);
	...
	/**
	 * Verify email address ownership with OTP code Completes the email address
	...
	public boolean verifyEmailRegistration(String userId, String email, String otpCode) {
		...
		if (methodOpt.isEmpty()) {
			throw new StrategizException(AuthErrors.OTP_NOT_FOUND, "No email registration found for this user");
		}

		AuthenticationMethodEntity method = methodOpt.get();

		boolean isVerified = Boolean.TRUE
			.equals(method.getMetadata(AuthenticationMethodMetadata.EmailOtpMetadata.IS_VERIFIED));
		if (isVerified) {
			log.info("Email address already verified for user: {}", userId);
			return true;
		}
		...
		if (otpValid) {
			...
			method.putMetadata(AuthenticationMethodMetadata.EmailOtpMetadata.VERIFICATION_TIME,
					java.time.Instant.now().toString());
	...
	public boolean resendVerificationOtp(String userId, String email) {
		...
		if (methodOpt.isEmpty()) {
			throw new StrategizException(AuthErrors.OTP_NOT_FOUND, "No email registration found");
		}

		AuthenticationMethodEntity method = methodOpt.get();

		// Check daily email limit
		int dailyCount = (Integer) method.getMetadata()
			.getOrDefault(AuthenticationMethodMetadata.EmailOtpMetadata.DAILY_EMAIL_COUNT, 0);
		String lastEmailDate = method
			.getMetadataAsString(AuthenticationMethodMetadata.EmailOtpMetadata.LAST_EMAIL_DATE);
		...
		if (otpSent) {
			method.putMetadata(AuthenticationMethodMetadata.EmailOtpMetadata.LAST_EMAIL_SENT,
					java.time.Instant.now().toString());
			...
			throw new StrategizException(AuthErrors.EMAIL_SEND_FAILED, "Failed to resend verification email");
		}
	}
	...
	private boolean sendRegistrationOtp(String email, String purpose) {
		...
		log.info("Generated registration OTP for {}, purpose: {}, expires at: {}", email, purpose, expiration);
	...
	private boolean sendOtpEmail(String email, String otpCode, String purpose) {
		try {
			...
			String htmlContent = "<div style=\"font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;\">"
					...
					+ "<span style=\"font-size: 32px; font-weight: bold; color: #00ff88; letter-spacing: 8px;\">"
					+ otpCode + "</span>" + "</div>" + "<p>Use this code to complete your account registration.</p>"
			...
			if (result.isSuccess()) {
				...
				log.error("Failed to send registration OTP email to {}: {} - {}", email, result.getErrorCode(),
