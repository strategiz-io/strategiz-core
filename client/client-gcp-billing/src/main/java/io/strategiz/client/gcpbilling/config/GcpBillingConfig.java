...
/**
 * Configuration for GCP Billing and Monitoring API clients. Loads credentials from Vault
 * for secure access.
 *
...
public class GcpBillingConfig {
	...
	public MetricServiceClient metricServiceClient() throws IOException {
		...
		// Path format: "gcp-billing.credentials" -> path: secret/strategiz/gcp-billing,
		// field: credentials
		...
		if (serviceAccountBase64 == null || serviceAccountBase64.isEmpty()) {
			log.warn(
					"No service account credentials found in Vault at secret/strategiz/gcp-billing, using Application Default Credentials");
			return MetricServiceClient.create();
		}

		log.info("Using service account credentials from Vault");
		log.debug("Credentials length: {} characters, first 50 chars: '{}'", serviceAccountBase64.length(),
		...
		if (trimmed.startsWith("{")) {
			...
			try {
				...
				// Last resort: maybe it's already JSON but with leading/trailing
				// whitespace or BOM
	...
	public GcpBillingProperties gcpBillingProperties() {
		...
		try {
			if (vaultSecretService != null) {
				...
				// Read billing table name from Vault (important: actual table has billing
				// account suffix)
		...
		log.info("GCP Billing Config - Project: {}, Dataset: {}, Table: {}, Billing API: {}, BigQuery: {}, Account: {}",
				projectId, billingDataset, vaultBillingTable, vaultUseBillingApi, vaultUseBigQuery,
				vaultBillingAccountId);

		return new GcpBillingProperties(projectId, billingDataset, vaultBillingTable, vaultUseBillingApi,
				vaultUseBigQuery, vaultBillingAccountId);
	}

	/**
	 * Properties holder for GCP billing configuration
	 */
	public record GcpBillingProperties(String projectId, String billingDataset, String billingTable,
			boolean useBillingApi, boolean useBigQuery, String billingAccountId) {
