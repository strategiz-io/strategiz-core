 ...
 *
 * <p>This implementation stores tuples in a thread-safe set in memory.
 * It is automatically used when no other FGAClient bean is present.</p>
 *
 * <p>When ready for production, add the {@code client-openfga} module which provides
 * an implementation using the real OpenFGA SDK. That implementation will take
 * precedence over this mock due to @Primary annotation on the production bean.</p>
 *
 * <p><b>Note:</b> This mock does NOT implement relationship inheritance (e.g., owner implies editor).
 * Each relationship must be explicitly granted. The production OpenFGA implementation
 * will handle relationship hierarchies based on the authorization model.</p>
 */
@Component
public class FGAMockClient implements FGAClient {
    ...
    public List<String> listObjects(String user, String relation, String type) {
        ...
        List<String> result = tuples.stream()
                ...
                .map(TupleKey::object)
                .toList();
        log.debug("FGA listObjects: user={} relation={} type={} -> {} results",
                user, relation, type, result.size());
        return result;
    }
