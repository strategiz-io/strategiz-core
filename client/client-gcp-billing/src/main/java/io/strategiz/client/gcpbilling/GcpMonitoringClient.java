...
/**
 * Client for retrieving GCP Cloud Monitoring metrics. Provides real-time usage data for
 * Firestore, Cloud Run, and other services.
 *
...
public class GcpMonitoringClient {
	...
	public List<GcpServiceUsage> getFirestoreMetrics() {
		...
		try {
			...
			metrics.add(new GcpServiceUsage("Cloud Firestore", "document_reads", BigDecimal.valueOf(readCount), "count",
					estimateFirestoreReadCost(readCount), Instant.now()));

			// Get write count
			long writeCount = getMetricSum(FIRESTORE_WRITE_COUNT, Duration.ofHours(1));
			metrics.add(new GcpServiceUsage("Cloud Firestore", "document_writes", BigDecimal.valueOf(writeCount),
					"count", estimateFirestoreWriteCost(writeCount), Instant.now()));

			// Get delete count
			long deleteCount = getMetricSum(FIRESTORE_DELETE_COUNT, Duration.ofHours(1));
			metrics.add(new GcpServiceUsage("Cloud Firestore", "document_deletes", BigDecimal.valueOf(deleteCount),
					"count", estimateFirestoreDeleteCost(deleteCount), Instant.now()));
	...
	public List<GcpServiceUsage> getCloudRunMetrics() {
		...
		try {
			...
			metrics.add(new GcpServiceUsage("Cloud Run", "requests", BigDecimal.valueOf(requestCount), "count",
	...
	public List<GcpServiceUsage> getDailyFirestoreMetrics(int days) {
		...
		try {
			...
			// Average per day
			metrics.add(new GcpServiceUsage("Cloud Firestore", "avg_daily_reads",
					BigDecimal.valueOf(totalReads / Math.max(days, 1)), "count/day",
					estimateFirestoreReadCost(totalReads / Math.max(days, 1)), Instant.now()));

			metrics.add(new GcpServiceUsage("Cloud Firestore", "avg_daily_writes",
					BigDecimal.valueOf(totalWrites / Math.max(days, 1)), "count/day",
					estimateFirestoreWriteCost(totalWrites / Math.max(days, 1)), Instant.now()));
		}
		catch (Exception e) {
	...
	/**
	 * Estimate Firestore read cost based on pricing Firestore pricing: $0.06 per 100,000
	 * document reads (us-central1)
	 */
	...
	/**
	 * Estimate Firestore write cost based on pricing Firestore pricing: $0.18 per 100,000
	 * document writes (us-central1)
	 */
	...
	/**
	 * Estimate Firestore delete cost based on pricing Firestore pricing: $0.02 per
	 * 100,000 document deletes (us-central1)
	 */
