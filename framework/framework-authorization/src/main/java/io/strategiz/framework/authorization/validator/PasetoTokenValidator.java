...
/**
 * PASETO Token Validator - Single source of truth for ALL token validation. This is the ONLY class
 * that should validate PASETO tokens in the system.
 *
 * <p>Validation includes:
 *
 * <ul>
 *   <li>PASETO signature verification
 *   <li>Token expiration checking
 *   <li>Token type verification
 *   <li>Claims extraction
 * </ul>
 *
 * <p>This class does NOT perform database lookups. Token validation is purely cryptographic.
 * Session revocation checking is a separate concern handled by the business layer if needed.
 */
@Component
public class PasetoTokenValidator {
  ...
  public void init() {
    ...
    if (vaultSecretService != null) {
      try {
        ...
        log.error(
            "Failed to load keys from Vault: {} - {}",
    ...
    if (identityKey == null || sessionKey == null) {
      log.error(
          "CRITICAL: Token keys not found in Vault. Token validation will fail until keys are"
              + " loaded.");
      log.error(
          "Check Vault connectivity and ensure keys exist at: tokens.{}.identity-key and"
              + " tokens.{}.session-key",
          env,
          env);
      keysLoaded = false;
  ...
  /**
   * Validates a token and returns the claims if valid. This method performs ONLY cryptographic
   * validation - no database lookups.
   *
  ...
  private Map<String, Object> parseWithKey(String token, SecretKey key) throws PasetoException {
    Paseto paseto = Pasetos.parserBuilder().setSharedSecret(key).build().parse(token);
  ...
  /**
   * Returns the session key for internal use (e.g., by the filter). Package-private to restrict
   * access.
   */
  ...
  /**
   * Returns the identity key for internal use (e.g., by the filter). Package-private to restrict
   * access.
   */
