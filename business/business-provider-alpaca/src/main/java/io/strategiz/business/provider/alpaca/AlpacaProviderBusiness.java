...
/**
 * Business logic for Alpaca provider integration. Handles OAuth flows, API interactions,
 * and business rules specific to Alpaca.
 */
@Component
public class AlpacaProviderBusiness implements ProviderIntegrationHandler {
	...
	private static final String DEFAULT_ENVIRONMENT = "paper"; // Default to paper for
																// safety
	...
	@Autowired
	public AlpacaProviderBusiness(AlpacaClient alpacaClient, AlpacaDataClient alpacaDataClient,
			AlpacaOAuthClient alpacaOAuthClient, PortfolioProviderRepository portfolioProviderRepository,
	...
	public String generateAuthorizationUrl(String userId, String state) {
		...
		try {
			...
			String authorizationUrl = String.format(
					"%s?response_type=code&client_id=%s&redirect_uri=%s&state=%s&scope=%s", authUrl, clientId,
					encodedRedirectUri, state, encodedScope);
			...
			throw new StrategizException(AlpacaProviderErrorDetails.OAUTH_URL_GENERATION_FAILED,
					"business-provider-alpaca", e);
		}
	}
	...
	public Map<String, Object> getAccountInfo(String userId, String accessToken) {
		...
		try {
			Map<String, Object> accountData = makeAuthenticatedRequest("/v2/account", accessToken, null);
	...
	public Map<String, Object> getPositions(String userId, String accessToken) {
		...
		try {
			Map<String, Object> positionsData = makeAuthenticatedRequest("/v2/positions", accessToken, null);
	...
	private void validateOAuthConfiguration() {
		if (clientId == null || clientId.trim().isEmpty()) {
			throw new StrategizException(ErrorCode.VALIDATION_ERROR, "Alpaca OAuth client ID is not configured");
		}
		if (clientSecret == null || clientSecret.trim().isEmpty()) {
			throw new StrategizException(ErrorCode.VALIDATION_ERROR, "Alpaca OAuth client secret is not configured");
		}
		if (redirectUri == null || redirectUri.trim().isEmpty()) {
			throw new StrategizException(ErrorCode.VALIDATION_ERROR, "Alpaca OAuth redirect URI is not configured");
		}
	}
	...
	private Map<String, Object> exchangeCodeForTokens(String authorizationCode) {
		try {
			...
			throw new StrategizException(ErrorCode.EXTERNAL_SERVICE_ERROR, "Token exchange failed: " + e.getMessage());
		}
		catch (Exception e) {
	...
	private Map<String, Object> makeAuthenticatedRequest(String endpoint, String accessToken,
		...
		try {
			...
			if (endpoint.equals("/v2/account")) {
				...
				return alpacaClient.oauthRequest(org.springframework.http.HttpMethod.GET, endpoint, accessToken, params,
	...
	private Map<String, Object> refreshTokens(String refreshToken) {
		try {
			...
			throw new StrategizException(ErrorCode.EXTERNAL_SERVICE_ERROR, "Token refresh failed: " + e.getMessage());
		}
		catch (Exception e) {
	...
	public ProviderIntegrationResult createIntegration(CreateProviderIntegrationRequest request, String userId) {
		...
		log.info("OAuth configuration for {} - clientId: {}, redirectUri: {}", environment,
				clientId != null ? "configured" : "null", redirectUri != null ? redirectUri : "null");
		...
		try {
			...
			throw new StrategizException(AlpacaProviderErrorDetails.INTEGRATION_CREATION_FAILED,
					"business-provider-alpaca", e, userId);
		}
	}
	...
	public void completeOAuthFlow(String userId, String authorizationCode, String state) {
		...
		try {
			...
			storeTokensInVault(userId, environment, result.getAccessToken(), result.getRefreshToken(),
					result.getExpiresAt());
			...
			// Fetch and store portfolio data after OAuth completion (synchronous, like
			// Coinbase)
			try {
				fetchAndStorePortfolioData(userId, result.getAccessToken());
				log.info("Successfully fetched and stored Alpaca {} portfolio data during OAuth for user: {}",
						environment, userId);
			}
			catch (Exception e) {
				log.error("Failed to fetch portfolio data for user: {} (OAuth still succeeded)", userId, e);
				// Don't throw - OAuth is complete, credentials are stored, data can be
				// synced later
			}
			...
			throw new StrategizException(AlpacaProviderErrorDetails.OAUTH_FLOW_FAILED, "business-provider-alpaca", e,
					userId);
		}
	}

	/**
	 * Sync provider data for a specific user Retrieves access token from Vault and
	 * fetches latest portfolio data from Alpaca
	...
	public ProviderHoldingsEntity syncProviderData(String userId) {
		...
		try {
			...
			if (secretData == null || secretData.isEmpty()) {
				throw new StrategizException(AlpacaProviderErrorDetails.TOKENS_NOT_FOUND, "business-provider-alpaca",
						userId);
			}

			String accessToken = (String) secretData.get("accessToken");
			if (accessToken == null || accessToken.isEmpty()) {
				throw new StrategizException(AlpacaProviderErrorDetails.ACCESS_TOKEN_NOT_FOUND,
						"business-provider-alpaca", userId);
			}
			...
			if (holdings.isPresent()) {
				...
				throw new StrategizException(AlpacaProviderErrorDetails.NO_DATA_AFTER_SYNC, "business-provider-alpaca",
						userId);
			}
	...
	/**
	 * Fetch portfolio data from Alpaca and store in new portfolio structure. Path:
	 * users/{userId}/portfolio/providers/{providerId}/holdings/current This is called
	 * after OAuth completion to pre-populate the dashboard
	 */
	private void fetchAndStorePortfolioData(String userId, String accessToken) {
		...
		try {
			...
			ProviderHoldingsEntity holdingsEntity = transformToProviderHoldingsEntity(accountInfo, positions, userId,
					accessToken);
			...
			log.info("Successfully stored Alpaca portfolio data for user: {} with {} holdings", userId,
	...
	/**
	 * Update the provider summary document with totals from holdings. This keeps the
	 * provider doc lightweight for fast Connect Providers page loads.
	 */
	...
	private ProviderHoldingsEntity transformToProviderHoldingsEntity(Map<String, Object> accountInfo,
			List<Map<String, Object>> positions, String userId, String accessToken) {
		...
		if (positions != null && !positions.isEmpty()) {
			for (Map<String, Object> position : positions) {
				try {
					...
					log.debug("Added Alpaca position: {} - qty={}, value=${}", symbol, quantity,
	...
	private void storeTokensInVault(String userId, String environment, String accessToken, String refreshToken,
			Instant expiresAt) {
		try {
			...
			throw new StrategizException(AlpacaProviderErrorDetails.TOKEN_STORAGE_FAILED, "business-provider-alpaca", e,
					userId);
		}
	}

	/**
	 * Extract environment from state parameter State format: userId-environment-uuid
	 */
	...
	private void loadOAuthConfigForEnvironment(String vaultKey) {
		try {
			...
			// Use redirect-uri based on active profile (prod uses prod URI, dev uses
			// local)
