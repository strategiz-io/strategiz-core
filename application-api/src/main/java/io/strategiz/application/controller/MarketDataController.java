...
/**
 * REST controller for market data endpoints Provides access to historical OHLCV data for
...
public class MarketDataController extends BaseController {
	...
	/**
	 * Get market data bars for a symbol GET
	 * /v1/market-data/bars?symbol=AAPL&timeframe=1D&period=7y GET
	 * /v1/market-data/bars?symbol=AAPL&timeframe=1D&startDate=2024-01-01T00:00:00Z&endDate=2024-12-31T23:59:59Z
	 * @param symbol Stock symbol (required, e.g., "AAPL")
	 * @param timeframe Timeframe for the bars (required, e.g., "1D", "1h")
	 * @param period Period shorthand (optional, e.g., "7d", "30d", "90d", "6m", "1y",
	 * "2y", "3y", "5y", "7y", "max")
	 * @param startDate Start date in ISO 8601 format (optional, e.g.,
	 ...
	 */
	@GetMapping("/bars")
	@Operation(summary = "Get market data bars",
	...
	public ResponseEntity<List<MarketDataBarDTO>> getMarketDataBars(
			@Parameter(description = "Stock symbol (e.g., AAPL)",
					required = true) @RequestParam(required = true) String symbol,

			@Parameter(description = "Timeframe (e.g., 1D, 1H)",
					required = true) @RequestParam(required = true) String timeframe,

			@Parameter(
					description = "Period shorthand (REQUIRED if startDate/endDate not provided). Examples: 7d, 30d, 90d, 6m, 1y, 2y, 3y, 5y, 7y, max") @RequestParam(
							required = false) String period,

			@Parameter(
					description = "Start date in ISO 8601 format (REQUIRED if period not provided). Example: 2024-01-01T00:00:00Z") @RequestParam(
							required = false) String startDate,

			@Parameter(
					description = "End date in ISO 8601 format (REQUIRED if period not provided). Example: 2024-12-31T23:59:59Z") @RequestParam(
							required = false) String endDate) {
		try {
			log.info("GET /v1/market-data/bars - symbol={}, timeframe={}, period={}, startDate={}, endDate={}", symbol,
					timeframe, period, startDate, endDate);
			...
			if ((period == null || period.isEmpty()) && (startDate == null || startDate.isEmpty())
					&& (endDate == null || endDate.isEmpty())) {

				throwModuleException(ServiceBaseErrorDetails.VALIDATION_ERROR, "date parameters",
			...
			if (period != null && !period.isEmpty()) {
				...
				log.debug("Calculated date range from period={}: {} to {}", period, calculatedStartDate,
						calculatedEndDate);
			}

			// Fetch market data from service
			List<MarketDataEntity> entities = marketDataService.getMarketDataBars(symbol, timeframe,
	...
	/**
	 * Get the latest market data for a symbol GET /v1/market-data/latest?symbol=AAPL
	 * @param symbol Stock symbol (required)
	 * @return Latest market data bar
	 */
	@GetMapping("/latest")
	@Operation(summary = "Get latest market data",
			description = "Retrieve the most recent market data bar for a symbol")
	public ResponseEntity<MarketDataBarDTO> getLatestMarketData(@Parameter(description = "Stock symbol (e.g., AAPL)",
			required = true) @RequestParam(required = true) String symbol) {
		try {
	...
	/**
	 * Get all available symbols in the database GET /v1/market-data/symbols
	 * @return List of distinct symbols
	 */
	@GetMapping("/symbols")
	@Operation(summary = "Get available symbols",
