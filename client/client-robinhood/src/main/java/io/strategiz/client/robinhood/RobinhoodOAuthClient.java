 ...
 *
 * Robinhood uses a password grant flow (not authorization code flow):
 * 1. User provides username/password
 * 2. Request to /oauth2/token/ - may fail with 400 if MFA required
 * 3. If MFA required, Robinhood sends SMS/email with code
 * 4. Submit challenge response to /challenge/{id}/respond/
 * 5. Retry token request with challenge response header
 *
...
public class RobinhoodOAuthClient {
     ...
     * This will either succeed with tokens or trigger MFA.
     *
    ...
    public boolean respondToChallenge(String challengeId, String code) {
        try {
            ...
            log.error("MFA challenge response failed with status {}: {}",
    ...
    public Map<String, Object> refreshAccessToken(String refreshToken) {
        try {
            ...
            log.error("Robinhood token refresh failed with status {}: {}",
                e.getStatusCode(), e.getResponseBodyAsString());
            throw new StrategizException(RobinhoodErrors.ROBINHOOD_TOKEN_EXPIRED,
    ...
    /**
     * Handle login error responses from Robinhood.
     * Parses the error to determine if MFA is required or if it's a real error.
     */
    private RobinhoodLoginResult handleLoginError(RestClientResponseException e,
                                                   String deviceToken, String challengeType) {
