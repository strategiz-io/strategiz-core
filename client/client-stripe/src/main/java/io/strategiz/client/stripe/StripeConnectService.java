...
/**
 * Service for Stripe Connect operations. Handles connected account creation, onboarding,
 * and management for owner subscriptions.
 *
...
public class StripeConnectService {
	...
	public Account createConnectAccount(String userId, String userEmail) {
		...
		try {
			...
			throw new StrategizException(StripeErrorDetails.CONNECT_ACCOUNT_CREATION_FAILED, "client-stripe", e,
					userId);
		}
	}

	/**
	 * Create an onboarding link for a Connect account. This link allows the user to
	 * complete Stripe's identity verification and bank setup.
	...
	public String createOnboardingLink(String accountId) {
		...
		try {
			...
			throw new StrategizException(StripeErrorDetails.CONNECT_ONBOARDING_LINK_FAILED, "client-stripe", e,
					accountId);
		}
	}

	/**
	 * Create a login link for an Express Connect account dashboard. Allows the owner to
	...
	public Account getAccount(String accountId) {
		...
		try {
			...
			throw new StrategizException(StripeErrorDetails.CONNECT_ACCOUNT_RETRIEVAL_FAILED, "client-stripe", e,
					accountId);
		}
	}
	...
	/**
	 * Create a checkout session for subscribing to a strategy owner. Payments are routed
	 * to the owner's Connect account with platform fee deducted.
	 ...
	 */
	public OwnerSubscriptionCheckoutResult createOwnerSubscriptionCheckout(String subscriberId, String subscriberEmail,
			String ownerId, String ownerConnectAccountId, long monthlyPriceInCents, String customerId) {

		if (!config.isConfigured()) {
			throw new StrategizException(StripeErrorDetails.NOT_CONFIGURED, "client-stripe");
		}

		logger.info("Creating owner subscription checkout: subscriber={}, owner={}, price={}", subscriberId, ownerId,
				monthlyPriceInCents);
		...
		try {
			...
			// Build checkout session with destination charge (payment to connected
			// account)
			SessionCreateParams.Builder paramsBuilder = SessionCreateParams.builder()
				.setMode(SessionCreateParams.Mode.SUBSCRIPTION)
				.setCustomer(stripeCustomerId)
				.setSuccessUrl(config.getAppBaseUrl() + "/profile/" + ownerId
			...
			logger.info("Created owner subscription checkout session {} for subscriber {}", session.getId(),
					subscriberId);

			return new OwnerSubscriptionCheckoutResult(session.getId(), session.getUrl(), stripeCustomerId,
					monthlyPriceInCents, platformFee);
		}
		catch (StripeException e) {
			logger.error("Failed to create owner subscription checkout for subscriber {}: {}", subscriberId,
					e.getMessage());
	...
	/**
	 * Cancel an owner subscription in Stripe. Cancels at the end of the current billing
	 * period.
	...
	public void cancelOwnerSubscriptionImmediately(String stripeSubscriptionId) {
		...
		try {
			...
			logger.error("Failed to cancel owner subscription {} immediately: {}", stripeSubscriptionId,
					e.getMessage());
			throw new StrategizException(StripeErrorDetails.SUBSCRIPTION_CANCELLATION_FAILED, "client-stripe", e,
					stripeSubscriptionId);
		}
	 ...
	 */
	public record OwnerSubscriptionCheckoutResult(String sessionId, String url, String customerId, long priceInCents,
	 ...
	 */
	public record ConnectAccountStatus(String accountId, String status, // not_started,
																		// pending,
																		// active,
																		// restricted
			boolean detailsSubmitted, boolean chargesEnabled, boolean payoutsEnabled, List<String> pendingRequirements,
		...
		public static ConnectAccountStatus fromAccount(Account account) {
			...
			else if (account.getRequirements() != null && account.getRequirements().getCurrentlyDue() != null
			...
			List<String> pendingRequirements = account.getRequirements() != null
					? account.getRequirements().getPendingVerification() : List.of();

			List<String> currentlyDue = account.getRequirements() != null ? account.getRequirements().getCurrentlyDue()
					: List.of();

			return new ConnectAccountStatus(account.getId(), status, account.getDetailsSubmitted(),
