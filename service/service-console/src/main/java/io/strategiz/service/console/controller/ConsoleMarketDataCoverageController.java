...
/**
 * Console admin controller for market data coverage monitoring.
 * Provides endpoints for viewing coverage snapshots and triggering calculations.
 *
 * Endpoints:
 * - GET /v1/console/marketdata/coverage - Get latest coverage snapshot
 * - POST /v1/console/marketdata/coverage/calculate - Trigger new calculation
 * - GET /v1/console/marketdata/coverage/history - Get historical snapshots
 */
@RestController
...
public class ConsoleMarketDataCoverageController extends BaseController {
     ...
     */
    @GetMapping
    @Operation(summary = "Get latest coverage snapshot", description = "Retrieves the most recent market data coverage snapshot")
    ...
    /**
     * Trigger a new coverage calculation.
     * This is a heavy operation that aggregates data across all symbols and timeframes.
     *
     * @param request HTTP request containing adminUserId attribute
     * @return The newly calculated coverage snapshot
     */
    @PostMapping("/calculate")
    @Operation(summary = "Calculate coverage snapshot", description = "Triggers a new coverage calculation (heavy operation)")
    ...
    public ResponseEntity<List<MarketDataCoverageResponse>> getCoverageHistory(
    ...
    public ResponseEntity<List<MarketDataCoverageResponse.DataGap>> getDataGaps(HttpServletRequest request) {
        ...
        List<MarketDataCoverageResponse.DataGap> gaps = latest.get().getGaps().stream()
    ...
    private MarketDataCoverageResponse convertToResponse(MarketDataCoverageEntity entity) {
        ...
        if (entity.getGaps() != null) {
            List<MarketDataCoverageResponse.DataGap> gaps = entity.getGaps().stream()
                .map(this::convertDataGap)
                .collect(Collectors.toList());
    ...
    private MarketDataCoverageResponse.StorageStats convertStorageStats(
            MarketDataCoverageEntity.StorageStats entity) {
    ...
    private MarketDataCoverageResponse.QualityStats convertQualityStats(
            MarketDataCoverageEntity.QualityStats entity) {
