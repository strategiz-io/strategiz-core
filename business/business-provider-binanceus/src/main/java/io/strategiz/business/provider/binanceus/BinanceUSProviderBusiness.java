...
/**
 * Business logic for Binance US provider integration Implements dual storage: credentials
 * in Vault, metadata in Firestore
 */
@Service
public class BinanceUSProviderBusiness extends BaseApiKeyProviderHandler {
	...
	public BinanceUSProviderBusiness(BinanceUSApiAuthClient apiAuthClient,
	...
	public ProviderIntegrationResult createIntegration(CreateProviderIntegrationRequest request, String userId) {
		...
		try {
			...
			// ProviderIntegrationEntity savedEntity =
			// providerIntegrationRepository.saveForUser(userId, entity);
			// log.info("Saved Binance US provider integration to Firestore for user: {}",
			// userId);
			...
			throw new StrategizException(BinanceUSProviderErrorDetails.INTEGRATION_CREATION_FAILED,
					"business-provider-binanceus", e, userId);
		}
	}
