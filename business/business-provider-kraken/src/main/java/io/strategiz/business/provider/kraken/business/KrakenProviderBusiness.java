...
/**
 * Business logic for Kraken provider integration Implements dual storage: credentials in
 * Vault, metadata in Firestore
 */
@Service
public class KrakenProviderBusiness extends BaseApiKeyProviderHandler {
	...
	@Autowired
	public KrakenProviderBusiness(KrakenApiAuthClient krakenApiAuthClient,
			KrakenCredentialManager krakenCredentialManager, KrakenDataInitializer dataInitializer,
	...
	public ProviderIntegrationResult createIntegration(CreateProviderIntegrationRequest request, String userId) {
		...
		try {
			...
			try {
				PortfolioProviderEntity entity = new PortfolioProviderEntity(KrakenConstants.PROVIDER_ID, "api_key",
						userId);
			...
			try {
				...
				log.info("Successfully initialized portfolio data for user: {}, total value: {}", userId,
						portfolioData.getTotalValue());
			}
			catch (Exception e) {
			...
			if (portfolioData != null) {
				...
				metadata.put("holdingsCount",
						portfolioData.getHoldings() != null ? portfolioData.getHoldings().size() : 0);
			...
			throw new StrategizException(KrakenProviderErrorDetails.CONNECTION_FAILED, MODULE_NAME, userId,
					e.getMessage());
		}
	}
	...
	public boolean testExistingIntegration(String userId) {
		...
		try {
			...
			boolean isConnected = response != null && !response.containsKey("error") && response.containsKey("result");

			// Update last tested timestamp
			// if (providerIntegrationRepository.existsByUserIdAndProviderId(userId,
			// PROVIDER_ID)) {
			// var entity =
			// providerIntegrationRepository.findByUserIdAndProviderId(userId,
			// PROVIDER_ID);
			// if (entity.isPresent()) {
	...
	public boolean disconnectIntegration(String userId) {
		...
		try {
			...
			// boolean deleted =
			// providerIntegrationRepository.deleteByUserIdAndProviderId(userId,
			// PROVIDER_ID);
	...
	/**
	 * Sync provider data for a user - fetches latest portfolio data from Kraken and
	 * stores it in Firestore with enriched information including cost basis.
	...
	public ProviderHoldingsEntity syncProviderData(String userId) {
		...
		try {
			...
			Map<String, String> credentials = getStoredCredentials(userId);
			if (credentials == null || credentials.isEmpty()) {
				throw new StrategizException(KrakenProviderErrorDetails.INVALID_CREDENTIALS, MODULE_NAME, userId,
			...
			log.info("Successfully synced Kraken data for user: {}, total value: {}, holdings: {}", userId,
					syncedData.getTotalValue(), syncedData.getHoldings() != null ? syncedData.getHoldings().size() : 0);
