 ...
 *
 * Supports: - Purchase strategy (one-time purchase for full ownership) - View ownership
 * history - Manual transfer (gifts/transfers)
 */
@RestController
...
public class OwnershipTransferController extends BaseController {
	...
	@Autowired
	public OwnershipTransferController(OwnershipTransferService ownershipTransferService, StripeService stripeService) {
	...
	public ResponseEntity<PurchaseResponse> purchaseStrategy(@PathVariable String strategyId,
		...
		// TODO: Validate user can purchase (not already owner, strategy is for sale,
		// etc.)
		...
		StripeService.CheckoutResult checkoutResult = stripeService.createStrategyCheckoutSession(user.getUserId(),
				null, // Email not in AuthenticatedUser - fetch from user profile if
						// needed
				strategyId, "Strategy " + strategyId, // TODO: Get actual strategy name
				priceInCents, "USD", null // Let Stripe create customer if needed
		);

		PurchaseResponse response = new PurchaseResponse(checkoutResult.sessionId(), checkoutResult.url());
	...
	public ResponseEntity<OwnershipTransferResponse> completeTransfer(@PathVariable String strategyId,
			@io.strategiz.framework.authorization.annotation.AuthUser AuthenticatedUser user,
			@RequestBody TransferCompletionRequest request) {

		log.info("Completing ownership transfer of strategy {} to user {}", strategyId, user.getUserId());

		// Execute the ownership transfer
		StrategyOwnershipTransfer transfer = ownershipTransferService.transferOwnership(strategyId, user.getUserId(),
				request.purchasePrice(), request.transactionId());

		OwnershipTransferResponse response = new OwnershipTransferResponse(transfer.getId(), transfer.getStrategyId(),
				transfer.getFromOwnerId(), transfer.getToOwnerId(), transfer.getPurchasePrice(),
				transfer.getSubscribersTransferred(), transfer.getMonthlyRevenueTransferred(),
	...
	public ResponseEntity<List<OwnershipTransferResponse>> getOwnershipHistory(@PathVariable String strategyId) {
	...
	@RequireAuth(minAcr = "1")
	@Operation(summary = "Manual ownership transfer", description = "Transfer ownership without payment (gifts, etc.)")
	public ResponseEntity<OwnershipTransferResponse> manualTransfer(@PathVariable String strategyId,
			@io.strategiz.framework.authorization.annotation.AuthUser AuthenticatedUser user,
			@RequestBody ManualTransferRequest request) {

		log.info("User {} transferring strategy {} to user {}", user.getUserId(), strategyId, request.newOwnerId());

		// TODO: Validate current user is owner (use StrategyAccessService)

		// Execute the ownership transfer (no payment)
		StrategyOwnershipTransfer transfer = ownershipTransferService.transferOwnership(strategyId,
				request.newOwnerId(), BigDecimal.ZERO, // No payment for manual transfer
	...
	public record PurchaseRequest(String customerId // Optional - existing Stripe customer
													// ID
	) {
	}

	public record PurchaseResponse(String checkoutSessionId, String checkoutUrl) {
	}

	public record TransferCompletionRequest(BigDecimal purchasePrice, String transactionId) {
	}

	public record ManualTransferRequest(String newOwnerId, String reason // Optional -
																			// gift,
																			// internal
																			// transfer,
																			// etc.
	...
	public record OwnershipTransferResponse(String id, String strategyId, String fromOwnerId, String toOwnerId,
			BigDecimal purchasePrice, Integer subscribersTransferred, BigDecimal monthlyRevenueTransferred,
