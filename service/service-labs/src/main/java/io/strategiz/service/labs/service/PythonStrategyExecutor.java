...
/**
 * Service for executing Python trading strategies
 * Handles subprocess execution, code injection, and result parsing
 */
@Service
public class PythonStrategyExecutor {
    ...
    public ExecuteStrategyResponse executePythonCode(String userCode, String marketDataJson) {
        ...
        try {
            ...
            logger.info("Python execution completed successfully in {} ms",
    ...
    private ExecuteStrategyResponse.Indicator parseIndicator(JsonNode node) {
        ...
        if (node.has("data")) {
            for (JsonNode dataNode : node.get("data")) {
                ExecuteStrategyResponse.Indicator.DataPoint dataPoint =
