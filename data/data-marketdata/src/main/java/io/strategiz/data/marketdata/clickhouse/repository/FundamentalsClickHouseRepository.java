...
/**
 * ClickHouse implementation of fundamentals repository. Stores company financial
...
public class FundamentalsClickHouseRepository {

	private static final Logger log = LoggerFactory.getLogger(FundamentalsClickHouseRepository.class);

	private final JdbcTemplate jdbcTemplate;

	private final FundamentalsRowMapper rowMapper = new FundamentalsRowMapper();

	public FundamentalsClickHouseRepository(@Qualifier("clickHouseJdbcTemplate") JdbcTemplate jdbcTemplate) {
	 ...
	 */
	public FundamentalsTimescaleEntity save(FundamentalsTimescaleEntity entity) {
		String sql = """
	 ...
	 */
	public void saveAll(List<FundamentalsTimescaleEntity> entities) {
		if (entities.isEmpty()) {
			return;
		}

		log.info("Batch saving {} fundamentals records to ClickHouse", entities.size());

		for (FundamentalsTimescaleEntity entity : entities) {
			save(entity);
		}
	 ...
	 */
	public Optional<FundamentalsTimescaleEntity> findLatestBySymbol(String symbol) {
		String sql = """
		...
		List<FundamentalsTimescaleEntity> results = jdbcTemplate.query(sql, rowMapper, symbol);
	 ...
	 */
	public Optional<FundamentalsTimescaleEntity> findFirstBySymbolAndPeriodTypeOrderByFiscalPeriodDesc(String symbol,
			String periodType) {
		String sql = """
				SELECT * FROM company_fundamentals
		...
		List<FundamentalsTimescaleEntity> results = jdbcTemplate.query(sql, rowMapper, symbol, periodType);
		return results.isEmpty() ? Optional.empty() : Optional.of(results.get(0));
	}
	 ...
	 */
	public List<FundamentalsTimescaleEntity> findBySymbolOrderByFiscalPeriodDesc(String symbol) {
		String sql = """
				SELECT * FROM company_fundamentals
				WHERE symbol = ?
				ORDER BY fiscal_period DESC
	 ...
	 */
	public List<FundamentalsTimescaleEntity> findBySymbolAndPeriodTypeOrderByFiscalPeriodDesc(String symbol,
			String periodType) {
		String sql = """
				SELECT * FROM company_fundamentals
				WHERE symbol = ? AND period_type = ?
	 ...
	 */
	public List<FundamentalsTimescaleEntity> findBySymbolAndDateRange(String symbol, LocalDate startDate,
			LocalDate endDate) {
		String sql = """
				SELECT * FROM company_fundamentals
				WHERE symbol = ?
	 ...
	 */
	public List<FundamentalsTimescaleEntity> findLatestBySymbols(List<String> symbols) {
		if (symbols.isEmpty()) {
			return List.of();
		}
	...
	/**
	 * Row mapper for FundamentalsTimescaleEntity.
	 */
	private static class FundamentalsRowMapper implements RowMapper<FundamentalsTimescaleEntity> {

		@Override
		public FundamentalsTimescaleEntity mapRow(ResultSet rs, int rowNum) throws SQLException {
			FundamentalsTimescaleEntity entity = new FundamentalsTimescaleEntity();
