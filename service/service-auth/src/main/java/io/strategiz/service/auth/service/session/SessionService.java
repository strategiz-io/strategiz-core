...
/**
 * Service implementation for session management operations Adapts SessionAuthBusiness for
...
public class SessionService extends BaseService {
	 ...
	 */
	public SessionEntity createSession(String userId, String userEmail, HttpServletRequest request, String acr,
			String aal, List<String> amr) {
		...
		SessionAuthBusiness.TokenPair tokenPair = sessionBusiness.createAuthenticationTokenPair(userId, amr, acr,
				deviceId, ipAddress);
	...
	public boolean terminateSession(HttpServletRequest request, HttpServletResponse response) {
		...
		if (token != null) {
			try {
				// Revoke the token/session in the business layer (marks session as
				// revoked in database)
	...
	public boolean enforceConcurrentSessionLimit(String userId, int maxConcurrentSessions) {
		log.debug("Enforcing concurrent session limit for user: {} (max: {}) - not supported in token-based approach",
				userId, maxConcurrentSessions);
		return true;
	}

	/**
	 * Refresh access token using a valid refresh token Issues a new access token and sets
