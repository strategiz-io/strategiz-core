...
/**
 * Controller for handling Google OAuth sign-up flow Specifically for new users creating
 * accounts via Google
 */
@RestController
...
public class GoogleOAuthSignUpController extends BaseController {
	...
	public GoogleOAuthSignUpController(GoogleOAuthService googleOAuthService, CookieUtil cookieUtil) {
	...
	public ResponseEntity<Map<String, String>> getAuthorizationUrl() {
		Map<String, String> authInfo = googleOAuthService.getAuthorizationUrl(true); // isSignup
																						// =
																						// true
	...
	public ResponseEntity<Map<String, Object>> handleCallbackJson(@RequestBody Map<String, String> callbackRequest,
		...
		try {
			...
			logger.info("Authentication cookies set for Google OAuth sign-up");
			...
			Map<String, Object> errorResponse = Map.of("success", false, "error", e.getMessage(), "message",
					"Google OAuth sign-up callback failed");
			return ResponseEntity.ok(errorResponse);
		}
	}

	/**
	 * Handle OAuth callback from Google for sign-up (redirect to frontend)
	 *
	 * This endpoint receives the authorization code from Google and redirects to the
	...
	public RedirectView handleCallback(@RequestParam(required = false) String code,
		...
		if (error != null) {
			...
			return new RedirectView(
					String.format("%s/auth/oauth/google/callback?error=%s", googleOAuthService.getFrontendUrl(),
		...
		if (code == null || code.isEmpty()) {
			...
			return new RedirectView(String.format("%s/auth/oauth/google/callback?error=%s",
					googleOAuthService.getFrontendUrl(), java.net.URLEncoder.encode("No authorization code received",
