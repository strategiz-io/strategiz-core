 ...
 *
 * Handles SMS OTP authentication for users with verified phone numbers. This is used for
...
public class SmsOtpAuthenticationController extends BaseController {
	...
	public ResponseEntity<Map<String, Object>> requestAuthenticationOtp(
		...
		String sessionId = smsOtpAuthenticationService.sendAuthenticationOtp(request.userId, request.phoneNumber,
				request.countryCode != null ? request.countryCode : "US");

		Map<String, Object> response = Map.of("success", true, "sessionId", sessionId, "phoneNumber",
				maskPhoneNumber(request.phoneNumber), "message", "Authentication code sent to your phone");
	...
	/**
	 * Request authentication OTP by phone number only (for sign-in page) This endpoint
	 * allows users to enter just their phone number
	 *
	...
	public ResponseEntity<Map<String, Object>> requestAuthenticationByPhone(
		...
		String sessionId = smsOtpAuthenticationService.sendAuthenticationOtpByPhone(request.phoneNumber,
				request.countryCode != null ? request.countryCode : "US");

		Map<String, Object> response = Map.of("success", true, "sessionId", sessionId, "phoneNumber",
				maskPhoneNumber(request.phoneNumber), "message", "Verification code sent to your phone");
	...
	public ResponseEntity<Map<String, Object>> verifyAuthenticationOtp(@PathVariable String sessionId,
			@RequestBody @Valid AuthenticationVerifyRequest request, HttpServletResponse httpResponse) {

		logRequest("verifyAuthenticationOtp", request.phoneNumber);

		// Authenticate with OTP
		Map<String, Object> authResult = smsOtpAuthenticationService.authenticateWithOtp(request.phoneNumber,
				request.otpCode, sessionId);
	 ...
	 */
	public record AuthenticationOtpRequest(@NotBlank(message = "User ID is required") String userId,
	 ...
	 */
	public record PhoneAuthenticationRequest(@NotBlank(message = "Phone number is required") String phoneNumber,
	...
	public ResponseEntity<Map<String, Object>> verifyAuthenticationOtpWithoutSession(
		...
		Map<String, Object> authResult = smsOtpAuthenticationService.authenticateWithOtp(request.phoneNumber,
				request.otpCode, null // No sessionId provided, will lookup by phone
		);
	 ...
	 */
	public record AuthenticationVerifyRequest(@NotBlank(message = "Phone number is required") String phoneNumber,
