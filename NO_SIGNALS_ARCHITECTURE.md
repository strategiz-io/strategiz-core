# No-Signals Detection - Architecture Diagrams

## 1. High-Level System Flow

```
┌─────────────────────────────────────────────────────────────────┐
│                         USER ACTIONS                            │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           │ 1. Clicks "Backtest Strategy"
                           ↓
┌─────────────────────────────────────────────────────────────────┐
│                   FRONTEND (React/TypeScript)                   │
│                                                                 │
│  UnifiedStrategyBuilder.tsx                                     │
│  ├─ Sends POST /v1/strategies/execute-code                      │
│  └─ Body: { code, symbol, timeframe, period }                   │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           │ 2. HTTP POST Request
                           ↓
┌─────────────────────────────────────────────────────────────────┐
│                   BACKEND (Spring Boot)                         │
│                                                                 │
│  ExecuteStrategyController                                      │
│  ├─ Validates request                                           │
│  ├─ Checks authentication                                       │
│  └─ Delegates to service layer                                  │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           │ 3. Service call
                           ↓
┌─────────────────────────────────────────────────────────────────┐
│              StrategyExecutionService (Service Layer)           │
│                                                                 │
│  executeStrategy(code, symbol, timeframe, period)               │
│  ├─ Fetches market data (DB query)                             │
│  ├─ Calls gRPC Python executor                                 │
│  ├─ Maps gRPC response to REST DTO                             │
│  └─ Checks: buyCount == 0 && sellCount == 0?                   │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                ┌──────────┴──────────┐
                │                     │
         NO SIGNALS            HAS SIGNALS
                │                     │
                ↓                     ↓
┌───────────────────────────┐  ┌─────────────────────────────┐
│ 🆕 TRIGGER AI ANALYSIS    │  │ Return ExecuteStrategyResponse│
│                           │  │ - signals                   │
│ NoSignalAnalysisBusiness  │  │ - performance               │
│  analyzeNoSignals()       │  │ - indicators                │
└───────────┬───────────────┘  └─────────────────────────────┘
            │
            │ 4. Business logic
            ↓
┌─────────────────────────────────────────────────────────────────┐
│          NoSignalAnalysisBusiness (Business Layer)              │
│                                                                 │
│  Step 1: Get Market Context                                     │
│  ├─ Query HistoricalInsightsService (Historical Insights)               │
│  ├─ Volatility regime, trend direction                         │
│  └─ Returns: MarketContext                                      │
│                                                                 │
│  Step 2: Analyze Strategy Code                                 │
│  ├─ Parse Python code with regex                               │
│  ├─ Detect entry/exit rules, indicators, conditions            │
│  └─ Returns: StrategyDiagnostic                                 │
│                                                                 │
│  Step 3: Generate AI Suggestions                               │
│  ├─ Build prompt with code + context + diagnostic              │
│  ├─ Call OpenAI GPT-4o                                          │
│  ├─ Parse JSON response                                         │
│  └─ Returns: List<Suggestion>                                   │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           │ 5. Return analysis
                           ↓
┌─────────────────────────────────────────────────────────────────┐
│              StrategyExecutionService (Service Layer)           │
│                                                                 │
│  dto.setNoSignalAnalysis(analysis)                              │
│  return dto;                                                    │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           │ 6. HTTP Response
                           ↓
┌─────────────────────────────────────────────────────────────────┐
│                   FRONTEND (React/TypeScript)                   │
│                                                                 │
│  BacktestResults.tsx                                            │
│  ├─ Check: results.buyCount === 0?                             │
│  ├─ Yes: Render NoSignalsWarning component                     │
│  └─ No: Render charts & performance metrics                    │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           │ 7. Display to user
                           ↓
┌─────────────────────────────────────────────────────────────────┐
│                    NoSignalsWarning Component                   │
│                                                                 │
│  ⚠️ No Signals Generated                                        │
│                                                                 │
│  📊 Market Context                                              │
│  - Trend: Bullish                                               │
│  - Volatility: Moderate (22%)                                   │
│                                                                 │
│  💡 AI Suggestions (3)                                          │
│  1. [High] Conditions too strict → Relax RSI < 30              │
│  2. [Medium] Indicator mismatch → Use 50-day SMA               │
│  3. [Low] Add volume filter                                     │
└─────────────────────────────────────────────────────────────────┘
```

---

## 2. Module Dependency Graph

```
┌──────────────────────────────────────────────────────────────┐
│                  application-api (Main App)                  │
│                                                              │
│  Spring Boot Application                                    │
│  - Serves all REST endpoints                                │
│  - Dependency injection container                           │
└───────────────────────┬──────────────────────────────────────┘
                        │ depends on
                        ↓
┌──────────────────────────────────────────────────────────────┐
│                    service-labs (REST API)                   │
│                                                              │
│  - ExecuteStrategyController                                │
│  - ExecuteStrategyRequest/Response DTOs                     │
└───────────────────────┬──────────────────────────────────────┘
                        │ depends on
                        ↓
┌──────────────────────────────────────────────────────────────┐
│           service-labs/StrategyExecutionService              │
│                                                              │
│  - Lightweight orchestration                                │
│  - Fetches market data                                      │
│  - Calls gRPC executor                                      │
│  - 🆕 Calls NoSignalAnalysisBusiness                        │
└────────────┬─────────────────────────┬─────────────────────┘
             │                         │
             │ depends on              │ depends on
             ↓                         ↓
┌────────────────────────┐  ┌──────────────────────────────────┐
│ client-execution       │  │ 🆕 business-nosignal-analysis    │
│                        │  │                                  │
│ - ExecutionServiceClient│  │ - NoSignalAnalysisBusiness      │
│ - gRPC to Python       │  │ - Model classes                 │
└────────────────────────┘  └──────────┬───────────────────────┘
                                       │ depends on
                        ┌──────────────┼──────────────┐
                        ↓              ↓              ↓
              ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
              │business-     │  │business-     │  │data-         │
              │ai-chat       │  │historical-   │  │marketdata    │
              │              │  │insights      │  │              │
              │-OpenAIService│  │-Historical Insights   │  │-Repository   │
              └──────────────┘  └──────────────┘  └──────────────┘
```

---

## 3. Data Flow Diagram

```
┌──────────────┐
│ User Input   │
│              │
│ - Python code│
│ - Symbol     │
│ - Timeframe  │
│ - Period     │
└──────┬───────┘
       │
       ↓
┌─────────────────────────────────────────────────────────────┐
│                    Strategy Execution                       │
│                                                             │
│  Market Data → Python Executor → Signals & Performance     │
└──────┬──────────────────────────────────────────────────────┘
       │
       ↓
  Are there signals?
       │
       ├─── YES ───→ Display performance metrics & charts
       │
       └─── NO ────→ Trigger Analysis
                          │
                          ↓
           ┌──────────────────────────────────┐
           │   Parallel Data Collection       │
           │                                  │
           │  ┌────────────────────────────┐ │
           │  │ Historical Insights Insights        │ │
           │  │ - Volatility regime        │ │
           │  │ - Trend direction          │ │
           │  │ - Market condition         │ │
           │  │ - Recommended indicators   │ │
           │  └────────────────────────────┘ │
           │                                  │
           │  ┌────────────────────────────┐ │
           │  │ Code Analysis              │ │
           │  │ - Entry rules?             │ │
           │  │ - Exit rules?              │ │
           │  │ - Indicators used          │ │
           │  │ - Condition count          │ │
           │  │ - Logic complexity         │ │
           │  └────────────────────────────┘ │
           └──────────────┬───────────────────┘
                          │
                          ↓
           ┌──────────────────────────────────┐
           │   AI Prompt Construction         │
           │                                  │
           │  Combine:                        │
           │  - Strategy code                 │
           │  - Market context                │
           │  - Code diagnostic               │
           │  - Requested output format       │
           └──────────────┬───────────────────┘
                          │
                          ↓
           ┌──────────────────────────────────┐
           │   OpenAI API Call (GPT-4o)       │
           │                                  │
           │  Input: 1500 tokens              │
           │  Output: 500 tokens (JSON)       │
           │  Timeout: 5 seconds              │
           └──────────────┬───────────────────┘
                          │
                          ↓
           ┌──────────────────────────────────┐
           │   Parse AI Response              │
           │                                  │
           │  Extract:                        │
           │  - Issue descriptions            │
           │  - Recommendations               │
           │  - Code examples                 │
           │  - Priority levels               │
           └──────────────┬───────────────────┘
                          │
                          ↓
           ┌──────────────────────────────────┐
           │   NoSignalAnalysisResult         │
           │                                  │
           │  {                               │
           │    marketContext: {...},         │
           │    diagnostic: {...},            │
           │    suggestions: [...]            │
           │  }                               │
           └──────────────┬───────────────────┘
                          │
                          ↓
           ┌──────────────────────────────────┐
           │   Frontend Rendering             │
           │                                  │
           │  NoSignalsWarning Component      │
           │  - Market context section        │
           │  - Suggestions accordion         │
           │  - Diagnostic details            │
           └──────────────────────────────────┘
```

---

## 4. Class Diagram (Backend)

```
┌─────────────────────────────────────────────────────────────┐
│              ExecuteStrategyController                      │
│                                                             │
│  + executeCode(request): ResponseEntity<Response>           │
└──────────────────────────┬──────────────────────────────────┘
                           │ uses
                           ↓
┌─────────────────────────────────────────────────────────────┐
│              StrategyExecutionService                       │
│                                                             │
│  - executionServiceClient: ExecutionServiceClient           │
│  - noSignalAnalysisBusiness: NoSignalAnalysisBusiness 🆕   │
│  - marketDataRepository: MarketDataRepository               │
│                                                             │
│  + executeStrategy(...): ExecuteStrategyResponse            │
└──────────────────────────┬──────────────────────────────────┘
                           │ uses
                           ↓
┌─────────────────────────────────────────────────────────────┐
│         🆕 NoSignalAnalysisBusiness                         │
│                                                             │
│  - openAIService: OpenAIService                             │
│  - historicalInsightsService: HistoricalInsightsService     │
│  - marketDataRepository: MarketDataRepository               │
│                                                             │
│  + analyzeNoSignals(                                        │
│      code: String,                                          │
│      symbol: String,                                        │
│      timeframe: String,                                     │
│      period: String,                                        │
│      result: ExecutionResult                                │
│    ): NoSignalAnalysisResult                                │
│                                                             │
│  - getMarketContext(symbol, period): MarketContext          │
│  - analyzeStrategyCode(code): StrategyDiagnostic            │
│  - generateAISuggestions(...): List<Suggestion>             │
│  - buildPrompt(...): String                                 │
│  - parseAIResponse(response): List<Suggestion>              │
│  - getFallbackSuggestions(): List<Suggestion>               │
└──────────────────────────┬──────────────────────────────────┘
                           │ returns
                           ↓
┌─────────────────────────────────────────────────────────────┐
│         🆕 NoSignalAnalysisResult                           │
│                                                             │
│  - symbol: String                                           │
│  - timeframe: String                                        │
│  - period: String                                           │
│  - marketContext: MarketContext                             │
│  - diagnostic: StrategyDiagnostic                           │
│  - suggestions: List<Suggestion>                            │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│         🆕 MarketContext                                    │
│                                                             │
│  - volatilityRegime: String                                 │
│  - trendDirection: String                                   │
│  - averageVolatility: Double                                │
│  - recommendedIndicators: List<String>                      │
│  - marketCondition: String                                  │
│                                                             │
│  + createDefault(): MarketContext                           │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│         🆕 StrategyDiagnostic                               │
│                                                             │
│  - hasEntryRules: boolean                                   │
│  - hasExitRules: boolean                                    │
│  - hasIndicators: boolean                                   │
│  - hasComplexConditions: boolean                            │
│  - conditionCount: int                                      │
│  - crossoverDetected: boolean                               │
│  - thresholdComparisons: boolean                            │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│         🆕 Suggestion                                        │
│                                                             │
│  - issue: String                                            │
│  - recommendation: String                                   │
│  - example: String                                          │
│  - priority: String  ("high" | "medium" | "low")            │
└─────────────────────────────────────────────────────────────┘
```

---

## 5. Component Hierarchy (Frontend)

```
UnifiedStrategyBuilder
│
├─ Tabs
│  ├─ Visual Editor
│  └─ Algorithm Editor (Python code)
│
├─ Configuration Panel
│  ├─ Symbol selector
│  ├─ Timeframe selector
│  └─ Period selector
│
├─ Action Buttons
│  ├─ Generate Strategy (AI)
│  ├─ Save Strategy
│  └─ Backtest Strategy  ← TRIGGERS EXECUTION
│
└─ Results Panel
   │
   └─ BacktestResults
      │
      ├─ Check: Has signals?
      │
      ├─ YES: Show Charts & Metrics
      │  ├─ EquityCurveChart
      │  ├─ SummaryTab
      │  │  ├─ Performance metrics
      │  │  ├─ Risk metrics
      │  │  └─ Buy & Hold comparison
      │  └─ TradeDetailsTab
      │     └─ Trade list with P&L
      │
      └─ NO: Show NoSignalsWarning 🆕
         │
         ├─ Alert Banner
         │  └─ "No Signals Generated" message
         │
         ├─ Market Context Accordion
         │  ├─ Trend direction chip
         │  ├─ Volatility regime chip
         │  └─ Market condition description
         │
         ├─ AI Suggestions Accordion (expanded by default)
         │  └─ List of suggestions
         │     └─ For each suggestion:
         │        ├─ Priority badge
         │        ├─ Issue title
         │        ├─ Recommendation text
         │        └─ Code example (monospace box)
         │
         └─ Strategy Diagnostic Accordion
            └─ Checklist
               ├─ Has entry rules?
               ├─ Has exit rules?
               ├─ Uses indicators?
               └─ Condition count
```

---

## 6. Sequence Diagram

```
User         Frontend        Controller      Service         Business        OpenAI
 │               │               │              │              │              │
 │ Click         │               │              │              │              │
 │ "Backtest"    │               │              │              │              │
 │──────────────>│               │              │              │              │
 │               │               │              │              │              │
 │               │ POST          │              │              │              │
 │               │ /execute-code │              │              │              │
 │               │──────────────>│              │              │              │
 │               │               │              │              │              │
 │               │               │ execute()    │              │              │
 │               │               │─────────────>│              │              │
 │               │               │              │              │              │
 │               │               │              │ Fetch        │              │
 │               │               │              │ market data  │              │
 │               │               │              │──────┐       │              │
 │               │               │              │<─────┘       │              │
 │               │               │              │              │              │
 │               │               │              │ Call gRPC    │              │
 │               │               │              │ executor     │              │
 │               │               │              │──────┐       │              │
 │               │               │              │<─────┘       │              │
 │               │               │              │              │              │
 │               │               │              │ Check        │              │
 │               │               │              │ signals==0?  │              │
 │               │               │              │──────┐       │              │
 │               │               │              │<─────┘       │              │
 │               │               │              │   YES        │              │
 │               │               │              │              │              │
 │               │               │              │ analyze()    │              │
 │               │               │              │─────────────>│              │
 │               │               │              │              │              │
 │               │               │              │              │ Get Alpha    │
 │               │               │              │              │ Mode insights│
 │               │               │              │              │──────┐       │
 │               │               │              │              │<─────┘       │
 │               │               │              │              │              │
 │               │               │              │              │ Parse        │
 │               │               │              │              │ code         │
 │               │               │              │              │──────┐       │
 │               │               │              │              │<─────┘       │
 │               │               │              │              │              │
 │               │               │              │              │ Generate     │
 │               │               │              │              │ prompt       │
 │               │               │              │              │──────┐       │
 │               │               │              │              │<─────┘       │
 │               │               │              │              │              │
 │               │               │              │              │ Call API     │
 │               │               │              │              │─────────────>│
 │               │               │              │              │              │
 │               │               │              │              │              │ AI
 │               │               │              │              │              │ Processing
 │               │               │              │              │              │ (2-5s)
 │               │               │              │              │              │
 │               │               │              │              │ JSON Response│
 │               │               │              │<─────────────│              │
 │               │               │              │              │              │
 │               │               │              │              │ Parse        │
 │               │               │              │              │ suggestions  │
 │               │               │              │              │──────┐       │
 │               │               │              │              │<─────┘       │
 │               │               │              │              │              │
 │               │               │              │ Analysis     │              │
 │               │               │              │ Result       │              │
 │               │               │              │<─────────────│              │
 │               │               │              │              │              │
 │               │               │ Response     │              │              │
 │               │               │ with         │              │              │
 │               │               │ analysis     │              │              │
 │               │               │<─────────────│              │              │
 │               │               │              │              │              │
 │               │ 200 OK        │              │              │              │
 │               │ {analysis...} │              │              │              │
 │               │<──────────────│              │              │              │
 │               │               │              │              │              │
 │ Render        │               │              │              │              │
 │ NoSignals     │               │              │              │              │
 │ Warning       │               │              │              │              │
 │<──────────────│               │              │              │              │
 │               │               │              │              │              │
```

---

## 7. Error Handling Flow

```
┌────────────────────────────────────────────────────────┐
│         NoSignalAnalysisBusiness.analyzeNoSignals()    │
└──────────────────────┬─────────────────────────────────┘
                       │
                       ↓
            Try: Get Market Context
                       │
         ┌─────────────┼─────────────┐
         │ SUCCESS                   │ FAILURE
         ↓                           ↓
   Use Historical Insights insights      Use fallback context
   - Volatility regime          - "Moderate" volatility
   - Trend direction            - "Mixed" trend
   - Market condition           - "Normal conditions"
         │                           │
         └─────────────┬─────────────┘
                       │
                       ↓
            Try: Analyze Code
                       │
         ┌─────────────┼─────────────┐
         │ SUCCESS                   │ FAILURE
         ↓                           ↓
   Parse indicators            Mark all as false
   Count conditions            Log warning
         │                           │
         └─────────────┬─────────────┘
                       │
                       ↓
            Try: Generate AI Suggestions
                       │
         ┌─────────────┼─────────────┐
         │ SUCCESS                   │ FAILURE
         ↓                           ↓
   Parse JSON response         Use fallback suggestions
   Return suggestions          - "Conditions too strict"
   Priority-sorted             - "Check indicators"
         │                     - "Market alignment"
         │                           │
         └─────────────┬─────────────┘
                       │
                       ↓
            Always: Return NoSignalAnalysisResult
            (Even if all steps failed, user gets feedback)
                       │
                       ↓
            Frontend: Render NoSignalsWarning
            (Never shows blank screen!)
```

**Key Principle:** Graceful degradation at every step. User ALWAYS gets feedback.

---

## 8. Performance Budget

```
Total Response Time Target: < 6 seconds (95th percentile)

┌──────────────────────────────────────────────┐
│ Component Breakdown:                         │
├──────────────────────────────────────────────┤
│                                              │
│ 1. Market Data Query                  50ms  │
│    └─ Single DB query (ClickHouse)          │
│                                              │
│ 2. Python Strategy Execution         500ms  │
│    └─ gRPC call + Python runtime             │
│                                              │
│ 3. Historical Insights Insights (cached)       10ms  │
│    └─ Read from cache                        │
│                                              │
│ 4. Code Analysis (regex)               5ms  │
│    └─ Pattern matching                       │
│                                              │
│ 5. AI Suggestion Generation         3000ms  │
│    ├─ Build prompt             10ms          │
│    ├─ OpenAI API call       2500ms          │
│    └─ Parse response          10ms          │
│                                              │
│ 6. Response Marshalling              50ms  │
│    └─ JSON serialization                     │
│                                              │
│ 7. Network Latency                  100ms  │
│    └─ Frontend ↔ Backend                     │
│                                              │
│ 8. Frontend Rendering                50ms  │
│    └─ React component render                 │
│                                              │
├──────────────────────────────────────────────┤
│ TOTAL:                            3,765ms  │
│ BUFFER (95th percentile):         2,235ms  │
├──────────────────────────────────────────────┤
│ TARGET:                           6,000ms  │
└──────────────────────────────────────────────┘

Optimization Strategies:
- Cache Historical Insights insights (7 days TTL)
- Async AI call with timeout (5s)
- Connection pooling for OpenAI
- CDN for frontend assets
```

---

## 9. Security & Authorization

```
┌─────────────────────────────────────────────────────────────┐
│                    Authorization Flow                       │
└──────────────────────────┬──────────────────────────────────┘
                           │
                           ↓
         Does user have valid JWT token?
                           │
                ┌──────────┴──────────┐
                │ NO                  │ YES
                ↓                     ↓
         Return 401             Extract userId
         Unauthorized           from token
                                      │
                                      ↓
                        Is user subscription active?
                                      │
                        ┌─────────────┴─────────────┐
                        │ NO                        │ YES
                        ↓                           ↓
                 Check quota:             Full access to
                 - 5 backtests/day        AI analysis
                 - Basic analysis only
                        │
                        └─────────────┬─────────────┘
                                      │
                                      ↓
                        Rate limit check:
                        - Max 10 AI analyses/hour
                        - Per user, per IP
                                      │
                        ┌─────────────┴─────────────┐
                        │ EXCEEDED                  │ WITHIN LIMIT
                        ↓                           ↓
                 Return 429                  Proceed with
                 Too Many Requests           analysis
```

**API Key Security:**
- OpenAI API key stored in HashiCorp Vault
- Never exposed to frontend
- Rotated monthly
- Rate limited per user

**Data Privacy:**
- Strategy code never logged (PII)
- Analysis results not stored (unless user saves)
- OpenAI data retention: 30 days (per OpenAI policy)

---

## 10. Monitoring & Observability

```
Metrics to Track:

┌─────────────────────────────────────────────────────────────┐
│                      Performance Metrics                    │
├─────────────────────────────────────────────────────────────┤
│ - analysis_duration_ms (p50, p95, p99)                      │
│ - alpha_mode_query_duration_ms                              │
│ - openai_api_duration_ms                                    │
│ - total_backtest_duration_ms                                │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│                      Business Metrics                       │
├─────────────────────────────────────────────────────────────┤
│ - no_signal_analysis_triggered_count                        │
│ - ai_suggestion_generated_count                             │
│ - fallback_suggestion_used_count                            │
│ - user_applied_suggestion_count (future)                    │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│                       Error Metrics                         │
├─────────────────────────────────────────────────────────────┤
│ - alpha_mode_failure_count                                  │
│ - openai_timeout_count                                      │
│ - openai_error_count (by error type)                        │
│ - parse_error_count                                         │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│                        Cost Metrics                         │
├─────────────────────────────────────────────────────────────┤
│ - openai_tokens_consumed (input + output)                   │
│ - openai_cost_usd (estimated)                               │
│ - cost_per_user_per_month                                   │
└─────────────────────────────────────────────────────────────┘

Logging Strategy:

[INFO] No signal analysis triggered: userId={}, symbol={}, timeframe={}
[DEBUG] Market context: volatility={}, trend={}, condition={}
[DEBUG] Code diagnostic: entryRules={}, exitRules={}, conditionCount={}
[INFO] AI suggestions generated: count={}, duration={}ms
[WARN] OpenAI API timeout, using fallback suggestions
[ERROR] Failed to parse AI response: {}
```

---

This architecture ensures:
✅ Clear separation of concerns
✅ Graceful degradation at every step
✅ Never shows blank screen to users
✅ Performance within acceptable limits
✅ Security & rate limiting
✅ Full observability
