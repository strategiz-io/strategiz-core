...
/**
 * Client for handling Coinbase OAuth operations
    ...
    public Map<String, Object> exchangeCodeForTokens(String authorizationCode) {
        try {
            ...
            ResponseEntity<Map> response = restTemplate.postForEntity(
                COINBASE_OAUTH_URL + TOKEN_ENDPOINT,
                request,
                Map.class
            );
    ...
    public boolean revokeAccessToken(String accessToken) {
        try {
            ...
            ResponseEntity<Void> response = restTemplate.postForEntity(
                COINBASE_OAUTH_URL + REVOKE_ENDPOINT,
                request,
                Void.class
            );
    ...
    public void validateConfiguration() {
        if (clientId == null || clientId.trim().isEmpty()) {
            throw new StrategizException(CoinbaseErrors.CONFIGURATION_ERROR, 
                "Coinbase OAuth client ID not configured");
        }
